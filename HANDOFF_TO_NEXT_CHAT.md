...
## 🔧 M5.82 추가 hotfix — 옛 PORT-MIS 데이터 정리

### 발견된 문제 (사진 분석 결과)
- DPRT 2605N PORT-MIS 카드에 시간(05/12 03:00) 표시되는데 **부두 정보 없음**
- 새 PORT-MIS 엑셀 업로드 + 시간 변경했는데 적용 안 됨
- 원인: Firebase에 M5.82 이전에 저장된 옛 데이터(berth 필드 없음)가 남아있고, 새 엑셀에 그 호출부호가 없으면 옛 데이터가 그대로

### 수정
1. **firebase.js**: `fbReplacePortMisBatch(ships, {port:'평택'})` 신규
   - 평택 PORT-MIS 데이터 모두 삭제 (`port === '평택'`)
   - 그 후 새 ships 저장
   - 다른 항만(부산/마산) 데이터는 보존
   - 반환: `{ saved, failed, deleted }`

2. **PortMisCaptureModal**:
   - 검토 화면에 `[🗑 평택 옛 데이터 삭제 + 새로 저장 (권장)]` 체크박스 (기본 ON)
   - 저장 버튼 텍스트 동적: 체크 시 "🔄 평택 데이터 전체 교체" / 해제 시 "💾 추가/갱신 저장"
   - 완료 화면에 "🔄 평택 옛 데이터 N건 삭제 후 교체" 표시

### 효과
- 검수원이 새 PORT-MIS 엑셀 업로드 → [평택 옛 데이터 삭제] 체크 → 저장
- Firebase에서 옛 평택 데이터 모두 삭제됨
- 새 데이터로 깨끗하게 교체
- 모든 검수원 화면 즉시 동기화
- voucher, HomePage, VoyagePage 모두 새 데이터로 표시
