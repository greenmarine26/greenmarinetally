# Tallyman Master M6.86.7.1 인계지침서

작성: 2026-05-23
이전 버전: M6.86.5/M6.86.6 (회귀로 폐기) → M6.80 베이스로 재시작 → M6.86.7 (카고플랜) → **M6.86.7.1 (빌드 핫픽스)**

---

## 0. M6.86.7.1 핫픽스 (2026-05-23)

### 증상
GitHub Actions CI에서 `npm run build` 실행 시 즉시 실패:
```
[vite:build-html] Failed to resolve ./assets/index-dn6za9X9.js from /home/runner/.../index.html
✓ 0 modules transformed.
```

### 원인
M6.86.7 ZIP에 들어간 루트 `index.html`이 **빌드본**이었음. `build.sh`의 [5/5] 단계 `cp dist/index.html ./` 가 빌드본을 루트에 덮어쓰고, ZIP 만들기 전에 소스형으로 되돌리는 단계가 없었음. vite 6.4.2는 빌드본 index.html을 보고 그 안에 적힌 해시 파일(`./assets/index-dn6za9X9.js`)을 다시 빌드 입력으로 찾으려 하다 실패.

이전 HANDOFF.md의 "루트 index.html = 빌드본 (GitHub Pages 직접 서빙)" 설명은 잘못. 실제로는 CI(`.github/workflows/deploy.yml`)가 push마다 `npm run build` 실행 후 `dist/`만 GitHub Pages artifact로 업로드. **루트 `index.html`은 vite 진입점(소스형)이어야 함**.

### 변경
| 파일 | 변경 |
|------|------|
| `index.html` (루트) | 빌드본 → 소스형 (`<script type="module" src="/src/main.jsx">`) |
| `build.sh` | [5/5] `cp dist/index.html ./` 제거, [6/6] 단계로 `write_source_index_html` 재호출 추가, 최종 검증 단계에서 루트 index.html이 소스형인지 확인 후 실패 시 exit 1 |
| `src/utils.js` | `APP_VERSION = 'M6.86.7.1'` |

기능/UI/베이사전/카고플랜 로직 변경 없음 — 순수 빌드 파이프라인 수정.

### 검증 방법
```bash
# 1. ZIP 풀고 push 전에 로컬 확인
grep -q '/src/main.jsx' index.html && echo "OK: 소스형" || echo "FAIL"
grep -q '\./assets/index-' index.html && echo "FAIL: 빌드본 남음" || echo "OK: 빌드본 없음"

# 2. push 후 GitHub Actions에서 "Build" 단계 성공 확인
```
...
## 8. ZIP 패키징 구조 (M6.86.7.1 정정판)

```
Tallyman_Master_M68671.zip
├── index.html                  ← 소스형 (vite 진입점, /src/main.jsx 참조)
├── assets/                     ← 빌드본 JS/CSS (검증용 누적 보존, 배포 영향 없음)
├── sw.js, manifest.webmanifest ← PWA 파일
├── src/                        ← 소스 코드 (CI 빌드 입력)
├── public/                     ← 정적 자산 (manifest, sw 원본)
├── package.json, vite.config.js, build.sh, postcss.config.js, tailwind.config.js ← 빌드 설정
├── .github/workflows/deploy.yml ← CI 정의
└── HANDOFF.md, HANDOFF_TO_NEXT_CHAT.md, SHIPMENT_MANUAL.md ← 문서
```

### 배포 흐름 (이전 설명 정정)
1. 사용자 → ZIP 풀어서 GitHub 저장소에 push
2. `.github/workflows/deploy.yml` 트리거 → ubuntu-latest 러너
3. `npm install` → `npm run build` (vite가 루트 `index.html`의 `/src/main.jsx` 진입점을 읽어 `dist/` 생성)
4. `dist/` 폴더를 GitHub Pages artifact로 업로드 → 배포

**핵심 불변식**:
- 루트 `index.html`은 반드시 소스형 (`<script type="module" src="/src/main.jsx">`)
- 루트 `index.html`에 `./assets/index-XXX.js` 형태의 빌드본 해시 참조가 있으면 **CI가 즉시 실패**
- `assets/`, `dist/`는 누적본 검증용으로만 보존, 실제 배포에는 사용되지 않음
- `build.sh`는 마지막에 반드시 루트 `index.html`을 다시 소스형으로 되돌릴 것 (M6.86.7.1에서 자동화)
