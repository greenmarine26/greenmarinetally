...
- **하루 8판을 넘기지 않는다.** `build.sh`가 최근 24시간 커밋을 세어 8판부터 **차단**한다(6판·시간당 3판에서 경고).
  임계치는 오늘 실데이터로 잡았다 — 처음 쓴 "시간당 4판"은 오늘 한 번도 안 걸리는 장식이었다(시간당 최대는 3판이었다).
- 정말 따로 올려야 하면 `TALLY_BATCH_OK=1 bash build.sh`. 넘어갔다는 사실이 빌드 로그에 남는다.
- **막혔을 때 푸시로 뚫으려 하지 마라.** 푸시할수록 배포 시도만 쌓인다. 재실행도 같은 예산을 쓴다.
  기다렸다가 **한 번만** 시도한다.
