...
if npx esbuild tools/smoke_entry.jsx --bundle --loader:.jsx=jsx --loader:.png=dataurl --jsx=automatic \
  ...
  # ConeOne 2.46: 콘앱이 «호기가 안 오는 동방»에서 검수앱과 같은 호기를 쓰는가 — 배선과 판정 한 벌을 잰다.
  SMOKE_CCM=$(mktemp /dev/shm/hometmp/_ccm_XXXXXX.mjs)
  SMOKE_CCO=$(mktemp /dev/shm/hometmp/_cco_XXXXXX.cjs)
  printf 'export { craneBaysByTime } from "%s/src/utils.js";\n' "$PWD" > "$SMOKE_CCM"
  if npx esbuild "$SMOKE_CCM" --bundle --platform=node --format=cjs --external:firebase --external:firebase/* --outfile="$SMOKE_CCO" --log-level=error; then
    node tools/smoke_conecrane.cjs "$SMOKE_CCO" || { echo "✗ 콘앱 호기 연막검사 실패 — 배포 금지"; rm -f "$SMOKE_CCM" "$SMOKE_CCO"; exit 1; }
    rm -f "$SMOKE_CCM" "$SMOKE_CCO"
  else
  ...
  SMOKE_PH=$(mktemp /dev/shm/hometmp/_smokeph_XXXXXX.js)
  if npx esbuild tools/smoke_printhub.jsx --bundle --loader:.jsx=jsx --loader:.png=dataurl --loader:.json=json --jsx=automatic \
       --external:fs --external:path --external:url \
       --outfile="$SMOKE_PH" --define:process.env.NODE_ENV='"development"' --log-level=error; then
    node tools/smoke_printhub.cjs "$SMOKE_PH" || { echo "✗ 출력 허브 연막검사 실패 — 배포 금지"; rm -f "$SMOKE_PH"; exit 1; }
    rm -f "$SMOKE_PH"
  else
    echo "✗ 출력 허브 연막 번들 실패 — 검사를 못 돌렸다. 배포 금지"; rm -f "$SMOKE_PH"; exit 1
  fi
...
node tools/smoke_conebayfit.cjs || { echo "✗ 콘앱 베이뷰 연막검사 실패 — 배포 금지"; exit 1; }
  # 2.43: 콘 트윈 — 트윈을 자료로 가리고 두 베이를 같이 적는가 · 전체 화면이 한 화면에 다 드는가(jsdom 실렌더)
  node tools/smoke_conetwin.cjs || { echo "✗ 콘 트윈 연막검사 실패 — 배포 금지"; exit 1; }
...
if npx esbuild src/utils.js --bundle --platform=node --format=cjs --external:firebase --external:firebase/* --outfile="$SMOKE_CP" --log-level=error; then
  ...
  node tools/smoke_conepos.cjs "$SMOKE_CP" || { echo "✗ 콘앱 자리 연막검사 실패 — 배포 금지"; rm -f "$SMOKE_CP"; exit 1; }   # ConeOne 2.44: 콘앱 자리 = 검수앱 자리
...
#  3.60-03: 전체 진단 뒤 기록 오염 수리 8건 — 기록지 자리 유지·startSet·42RH·key·unsub·SW 범위·엠티실 «바꾸는 칸만»(실소스 실행, 스텁 Firebase).
...
#  3.60-09: 완료는 «비었을 때만»(transaction — 이미 완료된 컨·초과 컨 덮기 금지 · 진단 T10) · 항차 삭제 확인에 완료 기록 수(T11) — 실소스 + 메모리 스텁.
node tools/smoke_fix36009.cjs "$PWD" || { echo "✗ 3.60-09 연막검사 실패 — 배포 금지"; exit 1; }
#  3.60-10: 리퍼 판정 한 벌(진단 M6) — 자체 정규식 lint · 45피트·숫자 리퍼 · 플랫랙 아님 · 마감텔리 RF condition·미르 걸러내기 실소스 실행.
...
node tools/smoke_fix36015.cjs "$PWD" || { echo "✗ 3.60-15 연막검사 실패 — 배포 금지"; exit 1; }
node tools/smoke_fix36016.cjs "$PWD" || { echo "✗ 3.60-16 연막검사 실패 — 배포 금지"; exit 1; }
node tools/smoke_fix36017.cjs "$PWD" || { echo "✗ 3.60-17 연막검사 실패 — 배포 금지"; exit 1; }
node tools/smoke_fix36018.cjs "$PWD" || { echo "✗ 3.60-18 연막검사 실패 — 배포 금지"; exit 1; }
node tools/smoke_fix36019.cjs "$PWD" || { echo "✗ 3.60-19 연막검사 실패 — 배포 금지"; exit 1; }
node tools/smoke_fix36020.cjs "$PWD" || { echo "✗ 3.60-20 연막검사 실패 — 배포 금지"; exit 1; }
node tools/smoke_fix36022.cjs "$PWD" || { echo "✗ 3.60-22 연막검사 실패 — 배포 금지"; exit 1; }
