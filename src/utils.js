...
export const APP_VERSION = 'TallyOne 2.27-01';   // **수석 권이 로그아웃 상태에서 통째로 사라졌다** — 검수사 «수석용은 어디서 보나요» → «안보입니다 바로 검수용메뉴얼만 보입니다». 실측: 10:46 로그인 → 10:54 로그아웃 → 11:01 매뉴얼 열람. 이름이 없으면 `isChief('')` 가 false 라 카드가 사라진다. 직책은 이름으로만 알 수 있으니 로그아웃 상태에서 **열어 줄 수는 없다** — 그러나 **왜 안 열리는지는 말해야 한다.** ⇒ 이름이 없으면 회색 점선 「잠김」 카드로 «이름을 고르면 열립니다 — 화면 맨 위 오른쪽 [이름] 버튼» 을 보여 준다. 누를 수는 없다. ⚠ 같은 부류의 실수다 — 조건이 안 맞으면 **조용히 없애는** 것. 그러면 쓰는 사람은 «없는 기능»으로 안다(CLAUDE.md 0-B). 연막검사가 로그아웃 경우를 같이 본다 — 사라지면·안내가 없으면·눌리면 셋 다 빌드가 멈춘다. · 앞선 내력은 git log 참조(2.27 매뉴얼 두 권 + 화면 그림 35장).
