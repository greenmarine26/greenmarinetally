...
export const APP_VERSION = 'TallyOne 2.38-01';   // **카고플랜 칠·글자 전면 정리 — 검수사가 실물 보며 한 줄씩 확정.** ①**글자는 언제나 진한 검정, 색은 «칠»이 혼자 말한다.** ②**칠하는 것은 풀뿐**이고 풀이냐는 마크가 아니라 **리스트**로 본다 «카고플랜에서 F만이 아니고 리스트에서 F여야만 합니다. 이유는 DG TK FR OT로 표기되면 풀로 안보일수 있으니까요» — cargoPlanCore 에 fulls 맵 신설, 셀마다 isFull. ③칠 색은 일반 풀 하늘색, 특수화물은 제 색. 흑백 대비로 **여섯 색 모두 회색값 190**에 맞췄다 «흑백프린터로 인쇄하면 어떻게 나오느냐가 문제입니다» — 종전엔 TK가 흰 칸과 27밖에 안 벌어져 칠했는지 안 보였고 지금은 65다. ④X(옆 짝수 40피트가 차지한 홀수 베이 자리)는 자기 셀이 아니라 40피트가 풀이어도 안 칠한다. ⑤**마크 글자 전부 1.25배**(8→10px), 소문자 e 는 1.1배·볼드 없이·가운데 보정 «B로 해서 e를 1.1 볼드 없이». ⑥그림 칸 리퍼는 **RF·RE 두 글자** «RE로 표기하면됨 엠티이니 색이 없으나 풀로 오해 안함» — 슬래시가 하던 풀/엠티 구분을 이제 칠이 한다. 두 글자가 최대라 가장 좁은 배(MCSC 한 줄 12칸·칸 17px)에서도 안 잘린다. ⚠ 리스트·서류 표기는 R/F·R/E·R/D 그대로(2026-08-04 확정). ⑦엠티 동그라미 폐지·DG·TK 2글자·선적 별첨2를 선사별→화물종류별(2.38). 실자료 시뮬 4항차 1,439칸(DXQD·KBTR·STSE 양하/선적) PASS. ⚠ 이 주석 줄에 따옴표 금지 — build.sh 버전 추출이 깨진다.
