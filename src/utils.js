...
export const APP_VERSION = 'TallyOne 2.15';   // 검수사 시안(Tallyone-Premium-Right-Panel) — 홈 항차 카드. **PC**: «카드를 좌(정보) + 우 280px 액션 패널로 분할» — 하단에 다닥다닥 붙던 «지금 처리·양하 완료·선적 완료»를 우측 세로 스택으로 옮기고 배경 #0F172A + 좌측 보더 + hover 밝아짐. ⚠ 시안의 «큰 숫자 271/254» 는 좌측에 이미 있는 값이라 **«남은 수»로 바꿨다**(작업이 진행되면 좌측 총계와 갈라진다 — 같은 수를 두 번 쓰지 않는다). **폰**: 검수사 «뱃지가 너무 작아서 현장에서 안 보였다» — 부두 뱃지 11→13px bold·높이 28px, 선박명 14→16px, 진행바 6px(PC 4px), 양하/선적 행 36px, 버튼 40→44~48px. 아이콘 규격도 폰이 더 크게 갈랐다(ico-s 16/ico 18/ico-l 24, sm 이상은 14/16/20) — 폰은 손가락, PC는 마우스.
