...
export const APP_VERSION = 'TallyOne 2.29-02';   // **로그인 화면이 옛 닻 그대로였다** — 검수사 «지금도 구버전 띄우셨네요»(로그인 화면 사진). 2.29 는 헤더만 바꿨다. 그런데 **앱을 열면 제일 먼저 보는 화면**은 로그인이다 — 거기 62px(넓은 화면)·56px(폰) 닻이 그대로 있었다. «로고를 넣었다»고 하려면 **처음 보는 자리부터** 바꿔야 한다. 전수 grep 으로 남은 닻을 확인했다 — ChiefDashboard·HomePage 의 닻은 ⚓입항/출항·작업보드 표시라 뜻이 있는 아이콘이므로 그대로 둔다. · 앞선 내력은 git log 참조(2.29 로고 신설 · -01 헤더 44px 홈 버튼 겸용).
