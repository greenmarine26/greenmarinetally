...
export const APP_VERSION = 'TallyOne 2.26-11';   // **회사명 바닥글을 용지 맨 아래에 고정** (검수사 지적 2026-08-24 — 안 그러면 위치가 계속 바뀐다). 2.26-09 는 표 바로 밑에 붙였다. 그래서 **행 수에 따라 자리가 달라졌다** — 2대짜리는 종이 중턱, 20대짜리는 아래쪽. 서류로 못 쓴다. ⇒ 쪽마다 내용 높이를 확보하고 바닥글을 아래로 밀어붙인다. A4 가로 21cm − 상 1.9cm − 하 1cm = **18.1cm** 가 한 쪽의 내용 높이다. `.pg` 를 그 높이의 세로 flex 로 두고 `.ft { margin-top:auto }` 로 남는 공간을 전부 위로 민다. 빌드가 두 값(18.1cm · margin-top:auto)을 지킨다 — 빠지면 바닥글이 다시 떠다닌다. · 앞선 내력은 git log 참조(2.26 X-RAY 양식 신설 → -01~-10).
