...
export const APP_VERSION = 'TallyOne 2.29';   // **검수사가 만들어 준 로고를 두 앱에 넣었다** — «이걸 콘앱과 검수앱 로고로 사용해줘 횐색부분은 없애고» + 헤더를 짚어 «이부분이야». 흰 바탕은 색으로 지우면 배지 아래 깔린 **회색 그림자**가 남는다(1차 실측). 배지가 둥근 사각형이니 **모양을 따서** 잘랐다 — 몸통 경계에서 상자를 잡고 1.6% 안으로 민 뒤, 모서리 반지름은 **바깥 상자에서 재서 민 만큼 뺀다**(안쪽 상자에서 다시 재면 141→100 이 되어 모서리를 깎는다). 넣은 곳 — 검수앱 헤더(홈 화면 닻 자리 · 「그린마린 검수팀 전용」 앞의 🌊 자리) · 콘앱 머리 · 두 앱 파비콘 · apple-touch · PWA 아이콘. **파비콘 링크가 아예 없어** 탭이 기본 지구본이었다 — 이번에 넣었다. 마스커블은 투명 배지를 쓰면 안드로이드가 잘라 먹으므로 남색을 꽉 채운 전용 파일을 따로 만들었다. 콘앱은 아이콘 3장을 매니페스트에 base64 로 박고 있었다(data: URL 안에서는 상대 경로가 안 먹어서) — `public/cone.webmanifest` 파일로 빼고 파일 참조로 바꿔 **cone.html 이 171KB→150KB** 로 줄었다. 로고는 `src/assets` 에서 import 한다 — vite 가 해시를 붙여 판이 바뀌면 **옛 로고가 캐시에 안 남는다.** · 앞선 내력은 git log 참조(2.28 수석 권 29장).
