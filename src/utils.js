...
export const APP_VERSION = 'TallyOne 2.29-01';   // **로고가 너무 작아 안 보였다** — 검수사 «너무 작아서 안보여요» → «로고를 홈화면 버튼으로 사용해주셔도 됩니다»(승인). 2.29 는 **홈 화면에서만** 로고를 36px 로 보이고, 다른 화면에서는 홈 버튼이 그 자리를 차지해 로고를 12px 짜리로 글줄에 끼워 넣었다 — 그 크기로는 금색 배지가 얼룩으로만 보인다. ⇒ 로고를 **늘 44px** 로 두고, 홈이 아닐 때는 그 로고가 **곧 홈 버튼**이다(오른쪽 아래 모서리에 작은 🏠 배지). 칸을 더 먹지 않는다 — 종전 홈 버튼이 있던 그 자리다(V9.15 «헤더 우측 버튼이 선박명을 42px 까지 밀어냈다» 규칙 유지). · 앞선 내력은 git log 참조(2.29 로고 신설).
