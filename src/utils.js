...
export const APP_VERSION = 'TallyOne 2.38';   // **카고플랜 출력 표기 개정 — 검수사 확정 5건.** ①엠티 동그라미(ⓔ·Ⓔ) 폐지 → 20ft는 소문자 e · 40/45ft는 대문자 E 글자만. ②DG는 D가 아니라 **DG** 2글자. ③탱크는 T가 아니라 **TK** 2글자(별첨2 범례 스와치도 같은 글자로 맞춤). ④**풀은 셀에 색을 채우고(흰 글자) 엠티는 색을 안 채운다(글자만 그 색)** — 풀/엠티가 «칠했나 아닌가»로 갈린다. ⑤포트 구분색은 그대로 살아 풀·엠티 양쪽에서 읽힌다(양하=선사색·선적=POD색), 통과화물은 회색 채움으로 셋이 안 겹친다. «선적시 선사색은 표기안함» → 선적 별첨2를 선사별 → 화물 종류별로 교체. 콘앱 카고플랜(cone-cargoplan.js)도 같은 소스라 함께 반영. (2.37 수석 질문 모음 동반 — 미르에게 «몇 항차 텔리 보관하고 있어» 류 지난 자료 조회 질문 13종.)
