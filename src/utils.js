...
export const APP_VERSION = 'TallyOne 2.12';   // 검수사 «폰용은 너무길어서 스크롤이 필요 합니다. 이유는 로그인화면에 **사용중인 사람만 보여야 하는데 사용했던 사람들이 보여서**입니다» — 실측 11명 중 최근 48시간 활동 6명(나머지 2일·5일·23일 전). 최근 이틀 활동자만 기본 표시 + 「그 밖의 검수원 N명 보기」 접기(로그인 길은 안 막는다). 최근 활동자 3명 미만이면 전체를 편다(휴무·주말). + 검수사 «반드시 컴용화면 폰용화면이 달라야 합니다. 항목이 다른게 아니고 **디자인 구성 배치**가 달라야 합니다» 1차 적용 — **폰**: 히어로 186→150(로고 70→56), 선박 보드 제거(히어로 배지가 대신), 카드 72→64·아바타 48→42, 하단 라벨 제거 → 스크롤 없이 한 화면. **PC**: 검수원 목록을 **2열 그리드**로(여백 해소, 카드 62px).
