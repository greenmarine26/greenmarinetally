...
export const APP_NOTE = '검수리스트 규격 칸은 세관에 신고된 규격을 그대로 적습니다 · XRAY 리스트는 제 규격 그대로입니다'
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    if (sl_i >= 0 && _dataRows.length) {
      ...
      if (_dataRows.every((i) => !_cell(i, sl_i))) {
        const _taken = new Set([cn_i, ...Object.keys(_cnColCnt).map(Number), wt_i, bl_i, pod_i, pol_i, fe_i, type_i, size_i, isocol_i, op_i, tmp_i, eseal_i, mlSeal_i].filter((x) => x >= 0));
