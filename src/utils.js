...
export const APP_NOTE = '완료자 표기가 터미널 글자 대신 그 조 근무자 이름으로 나옵니다';
...
//    동방(PNCT)은 적하목록에 컨별 자리·호기가 없다(수집기 `pnctpull.to_termwork` 가 pos·equip 을 아예 안 넣는다 —
//    빈 값으로 덮으면 카토스 값을 지우기 때문이다). 그래서 호기→베이를 이을 끈이 없었다.
  ...
  for (const mode of ['discharge', 'loading']) {
    ...
    for (const [cn, t] of Object.entries(tw)) {
      ...
      recs[cn] = { ...r, bay_actual: pos.bay, row_actual: pos.row, tier_actual: pos.tier,
                   _pos_src: 'catos', actual_at: t.at, actual_by: r.actual_by || '' };   // 3.16: 기록자 자리에도 업체 글자를 넣지 않는다(_pos_src 가 출처를 말한다)
      hit++;
    }
