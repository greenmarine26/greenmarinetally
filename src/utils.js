...
export const APP_NOTE = '수석 보드 호기 카드의 대수와 그림이 서로 바뀌어 보이던 것을 고쳤습니다 어느 장이 어느 호기인지 대수로 맞춥니다'
...
export function craneBaysByTime(voyage, craneNos = null) {
  ...
  const counts = cl.map((g) => g.reduce((a, k) => a + (bayCount[k] || 0), 0));
  ...
  cl.forEach((group, i) => {
    ...
    const even = (hi - lo === 2 && lo % 2 === 1) ? lo + 1 : (evenIn != null ? evenIn : lo);
