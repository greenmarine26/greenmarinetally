...
export const APP_VERSION = 'TallyOne 2.12-01';   // 검수사 확정 «**로그인한 작업자만** 보이게 하고 나머지는 분리해서 선택해서 볼수 있게 하는게 폰엔 한페이지로 남을것 같습니다» → «**항차기준이 아니고 로그인한 사람과 로그인 안된 사람**입니다» — 2.12 의 «최근 48시간» 기준을 **로그인 여부**로 교체. 판정은 inspectorStatus 한 벌(30분 신선도로 «로그아웃 없이 닫아 loggedIn 잔존» 허상을 이미 거른다 — 1.3-01). 실측: loggedIn 플래그 9명 true 지만 신선도 반영 시 로그인 1명 → 폰 한 페이지. 나머지는 「로그인 안 된 작업자 N명 보기」. 로그인 0명이면 전체를 편다(아침 첫 사람).
