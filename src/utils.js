...
export const APP_VERSION = 'TallyOne 2.22';   // **오늘 로그인한 본인은 목록에 남는다** (검수사 확정 2026-08-23). 원문 — «한번 로그인하면 그날 하루는 **본인것은 지워지지 않게** 매번이름을 넣지않게. 아까도 이야기 했듯이 **로그인중인 사람과 본인은 보이게** 해야 한다고». 2.12-01 이 목록을 «지금 로그인한 사람»만 남긴 것 자체는 맞았는데, **본인이 로그아웃하거나 30분 신선도가 지나면 본인 이름까지 사라져** 다시 들어올 때마다 이름을 쳐야 했다. ⇒ 목록 = **지금 로그인한 사람 ∪ 오늘 이 기기에서 로그인한 본인**. 본인은 맨 앞에 두고 「나」 뱃지를 붙인다(로그인 중이 아닌데 남아 있는 이유가 보여야 한다). ★ 기억은 **기기에만**(localStorage `tallyone_me_today`) — 서버에 올리면 «어느 기기에서 로그인했나»가 흐려지고 같은 폰을 두 사람이 쓰는 경우를 못 다룬다. 날짜는 **KST 기준**이라 기기 시간대가 달라도 평택 날짜로 끊기고, 날이 바뀌면 저절로 빠진다. 저장 실패(사파리 프라이빗 등)는 조용히 넘긴다 — 편의값이라 없어도 앱은 돈다. ⚠ **로그인 확정 지점이 여섯 곳**이었다(일반·세션통과·신뢰기기·비번설정·비번검증·소유자). 하나라도 빠지면 그 경로로 들어온 사람만 또 이름을 쳐야 한다 — `commitSelect` 한 줄로 모았다. ✅ 로그인 목록 연막검사를 build.sh 에 넣었다 — 검수사가 두 번 교정한 규칙이라 조용히 되돌아가면 안 된다.
