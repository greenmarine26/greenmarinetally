...
import { isViewOnlyNow } from './workChoice.js';   // 3.50: «조회만» 이면 호기 없음(문지기 한 곳)
...
export const APP_VERSION = 'TallyOne 3.50-02'   // 3.50-02 캔슬 리스트는 리스트에서 뺀다(업로드 창·진단, SWSP 2609S 13대 실측) · 3.50-01 테스터·서버 직책이 늦게 와도 역할(조회만/작업자) 단계로 올린다(업데이트 재개 실측) · 3.50 **«로그인 뒤 작업자 / 조회만» — 일반 검수원은 작업 선박(+호기)을 골라야 들어가고 그 선박 안에서만 앱이 돈다 · 수석·부수석·테스터·소유자는 조회만/작업자를 고르되 모든 선박을 본다.** 검수사 2026-09-15 «앱에 접속했다고 해서 모두 작업자는 아닙니다. 저 같은 경우는 점검차 여러선박을 조회하고 하는데 매번 호기를 기록해야 하는 경우가 있습니다» · «저와 수석 그리고 테스터를 제외하곤 모두 본인 작업 선박만 볼수있게 해주세요. 로그인후 작업선박 선택후 앱 작동» · «저와 수석이 단순 조회로 접속을 하면 호기에 기록이 안되게 하고 작업자로 선택을 하면 수석에게도 장비가 기록되어 검수가 될수 있게» · «대신 저와 수석은 작업자를 고르 더라도 모든 선박을 조회 할수 있어야 합니다» · «로그인후 작업 선박을 선택 안하면 진입이 안된다는 문구를». **상태 한 벌** `src/workChoice.js` — localStorage `tallyone_work_choice` {name, ymd(KST), mode work|view, voyageKey, equip}(meToday 와 같은 «오늘·이 기기·이 이름» 규칙, 로그아웃이 지움) · `isFreeRoamer` = canOpenChief 한 벌(isChief 는 안 건드림 — 잠금 판정이 딸려 있다) · `visibleVoyagesOf`/`canSeeVoyage`. **로그인** LoginPage 선택 단계 — 이름 확정 뒤 자유 열람은 [조회만]/[작업자], 일반 검수원은 곧장 선박 목록(로그인 현황판 board 정렬 그대로) + 부두별 호기 칩 + «⚠ 작업 선박을 고르지 않으면 들어갈 수 없습니다» 문구. 오늘 이 기기에서 이미 골랐으면 건너뛴다. **App** workChoice 상태 · 없으면 선택 화면(진입 전) · 일반 검수원은 고른 선박으로 곧장, 홈·통합검색·미르·건강점검·보조기능·항차 화면이 `visibleVoyages`(한 자리)만 받고 남의 항차 주소는 «내 작업 선박이 아닙니다» · 헤더 [변경]으로 다시 고름(«그대로 두기» 되돌림). **문지기(데이터 들어오는 자리)** — utils.getEquipNumber/setEquipNumber: 조회만이면 호기 ''·저장 안 됨 · firebase.fbSetInspectorActivity: 조회만이면 항차·호기·베이를 안 적고 workMode 'view' · fbSetInspectorChoice: 명단에 workMode/assignVoyage/assignEquip · inspectorStatus: view 는 작업중이 아니다(online 까지) · 수석 보드 검수원 카드 «조회» 배지 · 완료 문지기 4곳 문구 equipGateText(조회만이면 이유) · GuidedWorkPanel 호기 메뉴 대신 조회만 안내 · BayViewWork 호기 단추가 호기를 안 붙임. 무변경 — isChief·adminGuard·craneBoardOf 계열(읽는 계산)·tallyReport.   // 3.49-02 **베이뷰 선택 화면 — 호기 단추를 누르면 그 호기가 내 호기.** 검수사 2026-09-15 «따라가기나 장비번호가 작업중인 베이를 누르면 자동으로 호기가 지정 되어야 하는데 호기 지정 메뉴가 나옵니다». 원인 — 호기(localStorage gm_equip_no)가 빈 브라우저(컴 첫 사용)에서 «N호기 · BAY …»를 눌러도 호기를 안 정해 GuidedWorkPanel equipStep(«작업 장비(호기)를 선택하세요»)이 먼저 떴다. 수리 — BayViewWork 선택 화면: 호기 단추 = 그 호기를 내 호기로 지정(setEquipNumber + equipChanged, 헤더·작업 보고 한 벌) 후 진입 · 따라가기는 호기가 비었는데 일하는 호기가 하나뿐이면 그 호기로 지정, 둘이면 단추로 고르라고 막는다. 연막 smoke_bayview: 호기 비운 채 «2호기 · BAY (08)09» → gm_equip_no 2호기·호기 메뉴 없음.   // 3.49-01 **베이뷰 컴 화면 — 장이 통째로 한눈에·두 베이 같이(칸이 넓으면 나란히, 아니면 위아래).** 검수사 2026-09-15 «컴화면 좌우로 나뉘긴 했는데 베이 화면이 한눈에 안들어 옵니다. 베이가 통째로 들어 오게 하고 트윈일경우 두베이가 같이 보여야 합니다». 원인 — BayViewWork 의 FitBox 가 폭에만 맞춰(높이 상한 없음) 넓은 우측 칸에서 2.5배까지 키워 밑이 잘렸다(하나뿐). 수리 — FitBox `fill`+`boundsRef`(칸 바닥까지 높이에도 맞춤, 수석 보드와 같은 갈래 · 스크롤 칸이면 scrollTop 을 뺀다) · BayPlan `onlyLayout` prop(row = 가로 나란히·라벨 안 붙게 gap 48, 기본 col = 수석 보드·폰 무변경) · 컴에서는 col/row 중 **더 크게 그려지는 쪽을 자동으로**(감사 실측 1280×800 은 col 0.72 > row 0.60, 초광폭·손잡이를 끌면 row). 연막 smoke_bayview 에 배치 검사, 실렌더 Playwright 컴 1280·1920·폰 375.   // 3.49 **«베이뷰 — 컴은 좌우 분할 · 따라가기면 해치커버를 앱이 판정해 적는다».** 검수사 2026-09-15 «폰화면은 수직 화면 이지만 컴은 수평화면으로 보여야 할듯 합니다. 컴은 좌측에 컨테이너 자료가 우측에 베이가 보여지는것이 좋겠습니다» · «폰화면에서 따라가기를 누르면 커버 열고 닫는 질문도 자동으로 처리해야 합니다» · «보통은 데크 작업이 끝나고 베이 이동이 바로 이루어 지고 양하가 시작되면 커버가 닫힌상태이며 데크 작업 완료후 시간 공백이 어느정도 있고 난후 다음 베이를 작업 한다면 그시간에 커버를 열었다고 판단 해야 합니다» · «검수원 입력이 있으면 그건 우선 적용이고 입력이 없다면 따라가기 모드가 아닌 경우엔 알림을 보내야 합니다» · «네 그렇게 입력해도 됩니다. 틀리면 마지막에 수석이 수정할테니까요». **① 화면** BayViewWork 가 useIsWide(기존 한 벌)로 넓으면 좌(자료)/우(베이) flex-row, 폰은 위/아래 그대로. 경계 손잡이도 가로↔세로. **② 해치 자동 판정 한 벌** utils.hatchEventsOf — 터미널 실적(termWork.at·pos)을 크레인별 «런»(같은 장·단·모드 연속)으로 세워 장 단위로 판정. 열림 = 그 장 홀드 첫 런 직전에 끝난 양하 데크 런 뒤 공백 ≥3분(HATCH_GAP_MIN, 검수사 규칙 — 다음 베이로 갔어도), 없으면 홀드 첫 런 직전 공백. 닫힘 = **홀드가 끝났다는 증거가 있을 때만**(감사 치명 지적 — 실시간엔 지금 하는 홀드 런이 늘 마지막 런이라 홀드 첫 컨에 닫힘이 나왔다): 그 장 위 선적 데크 런 시작 또는 홀드 잔여(EDI 가 아는 평택분 홀드 컨 − 앱 완료 ∪ 시각 있는 실적, Booking 행 제외) 0 이고 그 장 EDI 홀드가 있고 양하 EDI·선적 자료를 앎(리스트만 온 선적 없음) 또는 항차 끝 표식(workEndAt·inspectorDone·양쪽 완료). 시각 = 마지막 홀드 런 뒤 공백 ≥3분, 크레인 마지막 런이면 «지금»까지 3분 지나야 +5분 추정(src est), 아니면 선적 데크 런 직전 공백. 동방(컨별 호기 없음)은 3.47-02 바구니 규칙으로 호기를 붙이고(낱개 오귀속 런은 «홀드 직전 마지막 데크 런»만 봐서 거른다) 못 붙인 구간은 장 단위 추정. 미래(+9h 오염) 실적·workEndAt+2h 뒤 실적 제외, 표시 끝은 15분 캡, 한 장에 열림·닫힘 각 한 번(재오픈은 인계함). 해치 제외 배(isHatchSkipShipInfo)·베이사전 없는 배는 판정 없음. 연막 tools/smoke_hatchauto.cjs — SWTD 9013E 16건·KKLC 2608N 8건 + 실시간 전진 재생(처음 나온 사건 = 최종)·Booking 행·양하 EDI 없음 갈래. 실측(보관소 6척) — SWTD 9013E 1호기 데크 끝 11곳 중 10곳 3~16분 공백, KKLC 2608N 25&27 닫힘 추정 04:42~04:47 vs 타임시트 04:45-04:50. **③ 기록** hatchReport.recordHatchEvent → fbAddWorkReport(type hatch, auto:true, eventTs=공백 첫머리) — 수석 보드·타임시트(tallyReport 는 auto 행이면 eventTs 로 시각)에 그대로 실린다. 보고 상태 한 벌 hatchReportedOf(reports type hatch → 장|동작) — 검수원 보고가 있으면 자동 기록도 알림도 없다. 내 호기 사건만(호기 미지정이면 전부). 보고의 사건 시각은 hatchReportTs 한 벌(auto 면 eventTs) — 타임시트·카톡 대조(kakaoWorkLog)·미르 답(chiefAnswers)·GuidedWorkPanel 마지막 보고 판정이 같이 쓴다. firebase.fbAddWorkReport 키는 단조 증가(같은 밀리초 겹침 방지, 감사 실측 9건 → 경로 2개). **④ 흐름** SearchPanel — 따라가기(bayView.follow)면 묻지 않고 적는다(GuidedWorkPanel hatchAuto 로 «오픈할까요/닫을까요» 프롬프트 끔) · 따라가기가 아니면 HatchAlertBanner(한 번 소리 + [보고+카톡]/[보고만]/[무시]). 자동 기록은 항차별 순차 await(키 겹침 방지)·모듈 수준 «적음» 표(재마운트에도 두 번 안 적음). 장수는 GuidedWorkPanel.hatchPanelsOf 본문을 utils.hatchPanelCountOf 로 옮겨 한 벌. 무변경 — 검수원이 직접 누르는 해치 보고(WorkReportModal)·기존 방식 프롬프트(따라가기 아닐 때 그대로).   // 3.48 **«베이뷰 작업» — 양하·선적을 고른 뒤 «▶ 작업 시작»에서 기존 방식과 베이뷰 중 하나를 고른다(기존 화면은 그대로, 추가).** 검수사 2026-09-15 «폰의 1/3은 양하와 선적 컨테이너 자료화면 이고 2/3는 작업중인 베이를 실시간으로 보여주는것이다. 작업할 베이와 데크인지 홀드 인지 자동인지 수동인지 수동이면 싱글인지 트윈인지를 선택하면 바로 화면 전환» · «기존껄 바꾸는게 아니고 추가하는 기능입니다. 베이뷰작업» · «작업 시작시 기존모드로 할것인지 새로운 베이뷰 모드로 시작할건지 선택할수있게» · 목적 «기존 작업이 편한지 베이뷰 작업이 편한지 테스트 해보고 장점과 단점을 찾으려고» · «크레인이 위치한 자리와 베이가 맞는지 보려는것입니다. 빠진칸도 자동적으로 보일것» · «본인이 완료하든 터미널 자료가 완료하든 색은 칠해져야 합니다. 베이가 중복 된다면 다른 검수원이 한것도» · «선택화면을 먼저 보여줘야 하고 만약 사용자가 선택을 안한다면 자동모드로 터미널이 선택한 베이로». **구조** — 전면 덮개(`components/BayViewWork.jsx`) = 맨 위 띠(모드·호기·장·데크⇄홀드·✕) / 위 칸 = 지금 «작업 시작» 탭의 SearchPanel 그대로(`bayView` prop 으로 모드 줄 숨김·초기값 주입·작업 맥락 보고) / 아래 칸 = `BayPlan onlyBay`(수석 보드와 같은 한 장 갈래) + `FitBox`. 칸 경계는 손으로 끌어 조절(폰에 기억). **완료 지도·불일치 한 벌** `bayViewOverlayOf` — 칠하는 것 = 앱 completed ∪ 터미널 termWork(메모리 합집합, 쓰기 없음 · 우선순위는 종전대로 검수원 > 터미널 `applyCatosPos`). 실측 — 터미널 자리는 계획이 아니라 실제다(DXQD 2636E 선적 260 중 160이 계획과 다른 자리 · DJCT 0224W 60 중 27) 그래서 «터미널이 계획대로 넣어 충돌» 은 안 일어나고, 남는 것은 ①같은 컨 다른 자리 ②같은 자리 다른 컨 ③검수원이 찍었는데 터미널이 10분 넘게 모르는 컨 — 셀 빨간/회색 테두리 + 띠 한 줄로 **보이기만** 한다(자동으로 안 고침 — 3.47 규격 불일치와 같은 원칙, 실물이 정본). **따라가기** `bayViewFollowOf` — 수석 보드와 같은 `craneBoardOf` 한 벌(PCTC 는 컨별 호기라 즉시 · 동방은 두 호기가 같은 시각 다른 장에 찍힌 마지막 바구니라 실측 10분 지연) → 내 호기 자리가 없으면 `boardBaysOf` 최근 장. 따라가기 중에는 활동 기록에 베이를 안 싣는다 — 수석 보드가 «사람이 확인한 자리(live)»로 읽어 터미널 추론을 덮는 되먹임을 막는다. ⚠ 발견(인계함) — DXQD 2636E 선적 completed 19/260 의 at 이 termWork 보다 정확히 +9h(수집기 apply_completed 는 추가만 하고 뒤에 고쳐진 at 을 안 따라감) → boardBaysOf 창 기준이 튄다. 앱 코드 무변경 부분 — 기존 리스트·작업 시작·베이 탭 동작, firebase.js(새 쓰기 없음), 판정 함수들.   // 3.47-02 **동방 배에서 «호기별 자리가 안 와»가 사라진다 — 가까운 베이는 버리는 게 아니라 한 장으로 묶는다.** 검수사 2026-09-15 «호기 판단은 선수 즉 2호기 4호기 작업 이었다면 판단을 하면 되었습니다. 그시간에 2호기는 어느 위치 였고 4호기는 어느 위치 였는지». **원인** — 3.38-01 이 «번호 차 2 이하면 같은 그림이니 그 바구니를 버린다»로 문지기를 세웠는데, 한 크레인이 40ft 장 하나를 잡으면 20ft 베이 둘(11·13)이 같은 초에 찍히는 것이 **정상 모양**이라(실측 DXQD 2636E 시각 바구니 165개 중 163개) 판정이 한 번도 성립하지 않았다 — 언제나 «호기별 자리가 안 와 베이로 묶었습니다»로 빠지고, 묶을 때는 EDI 계획 자리를 써서 실제(11·13)가 아닌 계획(07·09)을 그렸다. **수리** — 버리지 말고 차 2 이하를 이어 붙여 **장(해치) 묶음**을 만들고 묶음 수가 호기 수와 같을 때만 쓴다. 대표 베이는 그 장의 짝수(25·27 → 26). **실물 근거** — KKLC 2608N 타임시트가 커버를 «BAY# 25&27 · 17&19 · 29&31 · 01&03»으로 적는다(홀수 둘이 한 장). **실측 검증** — DXQD 04:48 «2호기 11·13 · 4호기 19·21» = 동방 호기별 QC102 선적 42대와 11·13 묶음 42대가 일치 · KKLC 04:18 «1호기 25·27 · 2호기 38» = 타임시트 04:45-04:50 커버닫음 25&27 → 04:50 COMPLETED LOAD 와 일치. 남은 것(인계함) — 한 호기가 두 배에 겹치면 버리는 교차검사(2호기가 KKLC 04:22 → DXQD 04:33 으로 옮긴 실측).   // 3.47-01 **수집기 내장 헬퍼가 30판 넘게 안 구워지고 있었다 — 이제 build.sh 가 판마다 굽고 막는다.** 3.47 배포 직후 라이브 헬퍼(`collector/autoreg_helper.html`)를 열어 보니 파서가 **TallyOne 3.13**(2026-09-06)에 멈춰 있었다 — 3.14~3.46-01 동안 앱 파서가 바뀐 것을 수집기는 하나도 못 봤다. 지침서에 «파서를 고친 판마다 재생성» 이라고 적혀만 있고 **훅이 없어 아무도 안 지켰다**(규칙을 사람 기억에 맡긴 대가). 3.47 로 재생성해 올렸고(백업 `_claude/backup/autoreg_helper_TallyOne3.13_bak_20260914_1905.html` · 계약 grep GMautoPayload·ptkCount·iso_edi · node 적재로 GM_HELPER_VERSION=TallyOne 3.47 확인), 이 판에서 **build.sh 에 게이트**를 넣었다 — 헬퍼 번들을 굽고 ①버전이 APP_VERSION 과 다르거나 ②계약 셋 중 하나라도 없으면 **배포를 막는다**. ⛔ merge_helper 는 굽지 않는다(그쪽 GMmerge 는 수집기 계약이 정본). 앱 코드 무변경 — 도구만 고쳤다.   // 3.47 **규격이 자료마다 다르면 알린다. 실물을 보고 고른다.** 검수사 2026-09-14 «세관자료와 EDI 또는 선사 자료와 규격이 틀렸는데 어느곳에서도 알림이 없었습니다. 예를 들면 ATPR 10번 홀드에 40 스탠다드 10대가 있는데 세관리스트에는 40 스탠다드가 없었다는것입니다.» · «이건 신규기능이 아니고 규격 불일치 알림이 전부터 있었습니다.» · «확인대상이 EDI : 선사리스트 : 세관리스트 3곳입니다. 만약 불일치가 나온다면 이들은 실물을 보기전에는 확정할수 없습니다. 그러므로 알림을 띄우고 그컨테이너가 양하 되거나 선적될때 세개중에 어느것이 맞는지 선택할수 있어야 합니다.» **원인** — 세관 파서 변환표에 세관이 실제로 쓰는 45GP·22RE·42PC 가 없어 규격을 빈칸으로 내보냈고, 그 빈칸을 «빈 값은 기존을 안 덮는다» 규칙이 EDI 값으로 메웠다. 세 자료가 records.iso 한 칸을 돌려써서 마지막 것만 남았고 진단은 EDI 와 EDI 를 비교해 영원히 0건이었다. **수리** ①표에 없으면 구조로 푼다(ISO 6346 크기2+종류군2 → 45GP=45G1) — 처음 보는 규격도 안 버린다 ②출처마다 제 칸(선사 iso_carrier · 세관 iso_customs) ③진단을 3자 대조로(utils isoTriad·isoConflictOf 한 벌 — 진단·작업카드·컨 상세가 같은 것을 부른다. 문지기도 거기 한 곳) ④작업카드·컨 상세에서 셋 중 하나를 누르면 확정되고(fbPickIso — 누가·언제 기록) 리스트 재업로드가 못 덮는다(iso_pick 있을 때만 iso 를 지킨다). 고르기 전에도 완료는 막지 않는다 — 미확정 표시만(검수사 통보). 3.46 이하 저장분은 제 칸이 없어 안 뜬다 — 파일을 다시 올리면 뜬다(통보). **실측 ATPR 2641E 실 3종**(선사 CLL · 세관 적하목록 · RTDB 실 EDI) — 세관 규격 버린 대수 64/263 → 0/263 · 불일치 알림 0건 → 10건(전부 10번 베이 · EDI 42GP / 선사 42GP / 세관 45GP) = 검수사가 말한 그 10대. 표기만 다른 것(45RF↔45RE) 0건. 업로드 순서 무관. **독립 감사 지적 수리(전부 이 판에서)** — ①새 칸을 VoyagePage 화이트리스트에 안 넣어 «진단 10건인데 상세엔 고르기 없음» ②화면마다 병합 규칙이 달라 «EDI» 단추에 EDI 아닌 값이 뜰 수 있었다 → **EDI 도 제 칸(iso_edi)** 을 갖는다, 없으면 그 자리를 비운다 ③**정규화를 모듈 한 벌(normalizeSpecIso)로** 꺼내 파서와 「고르기」가 같은 값을 쓴다 — 갇혀 있어서 «세관 것이 맞다»를 누르면 파일로 올렸을 때보다 나쁜 값(25GP→20DC · 46RE→40HC드라이)이 잠길 뻔했다 ④대조 잣대는 **정규화한 라벨**(길이+높이+종류) — 표기(44GP↔45GP)로 대조하면 «없는 불일치 49건»이 뜬다(감사 실측). 표시는 자료가 적은 말(spec), 대조는 라벨 — 자리를 나눴다 ⑤확정값이 인쇄·컨상세 경로에도 EDI 를 이긴다(병합 셋 다) ⑥확정을 풀 수 있게(안 그러면 오확정 한 번에 그 항차 내내 침묵) ⑦검수원 없이 확정 금지(§5-3) ⑧확정이 리퍼·FR·OT·탱크 표시를 강등까지 고치고, 그 판정은 utils 한 벌(isFlatRackIso·isOpenTopIso·isTankIso 신설 — 손으로 다시 적은 정규식이 4283·FR40·PL40 을 FR 이 아니라 해 3.43-03 사고가 되살아날 뻔했다) ⑨저장 실패를 알린다(§4-3). **2·3차 감사 지적 추가 수리** — ⑩확정이 규격만 지키고 표시(rf/fr/ot/tk)는 리스트가 덮어 «확정이 모순을 고정»하던 것 ⑪EDI 노드에 확정 표시를 쓰는 바람에 「다시 고르기」 뒤 리퍼가 영구 강등되던 것(이제 EDI 노드에는 표식만) ⑫`_SPEC_SZ` 의 `49`(ISO 6346 둘째 자리 9 = 4피트 미만)를 하이큐브로 잘못 올리던 줄 삭제. ⑬진단이 `ediContainers` 를 직접 읽는 탓에, 드라이로 확정해도 EDI 의 rf 가 남아 «풀 리퍼 온도 미입력» 이 안 꺼지던 것(승격은 되고 강등만 안 되던 비대칭 — extractReefers 가 확정 컨에서는 records 를 본다). 연막 smoke_isotriad **77항** — 이제 **진짜 runDiagnostics 를 부른다**(종전엔 호출부 인자식을 검사 안에 베껴 재서, 진단이 딴 값을 봐도 통과했다). 픽스처 EDI 에 평택 판정 칸(pod·pol·fe)을 넣어 «0건으로 조용히 통과»도 막았고, **오직 EDI 때문에만 갈리는 한 대**를 따로 세워 진단·화면이 갈리면 깨지게 했다(사보타주 확인).   // 3.46-01 **베이 상세 — 칸이 안 움직이게 바로잡음.** 검수사 2026-09-14 «셀크기가 바뀌면 안되는데 바뀌었네요 셀크기는 절대 변경하면 안됩니다. 어렵게 맞춰 논것이라». 원인 — 3.46 이 4번째 줄 글꼴만 줄였는데 두 격자의 line-height 가 «수»(1.15 · 1.1) 라 자식이 **자기 글꼴로 다시 계산**해 그 줄 상자까지 작아졌고, space-evenly 가 남는 자리를 나머지 네 줄에 나눠 줬다. 칸 바깥 크기는 그대로였지만 **칸 안 다섯 줄의 높이·자리가 움직였다**(크로뮴 실측 MCSC 8.8px→9.28px · 자리 1.45px). 수리 ①line-height 를 «길이»(1.15em · 1.1em)로 — 계산된 절대값이 상속돼 글꼴과 무관하게 같은 높이를 쓴다 ②축소 문턱을 **그 배의 실제 칸 폭**에서 계산(MID_CAP) — 고정 문턱은 넓은 배에서 필요 없이 작게 만들었다(ATPR 116px 에서 리퍼 4칸이 6.3pt 로 떨어졌다). 실측 ATPR 2641E 실항차(양하 262 · 검수사 실봉인 7) — X-RAY 켠 종이 대 끈 종이 **칸 크기·줄 자리 다른 칸 0/262** · 장수 14 동일 · 잘린 칸 0 · 7대 전부 8.5pt. ⚠ 3.46 을 통과시킨 내 검사는 칸 크기를 style 문자열로만 봤다 — 이제 연막이 **X-RAY 칸과 그냥 칸의 칸 크기·다섯 줄 높이·자리를 픽셀로 대조**한다(사보타주로 잡히는 것 확인).   // 3.45 · 3.44 · ConeOne 2.50 · MailPilot 2.33 **선사 시프팅 목록(RESTOW LIST)이 오면 그것이 시프팅 정본** — 검수사 2026-09-12 «카토스에 있는 MCAT 시프팅 자료를 찾아서 검수앱과 맞춰주세요 2개 차이납니다» · «둘 다»(서류 정본 · 카토스 대조는 다음 판) · «둘 다 보이기». 실측 MCAT 635N — 선사 서류 14대(배정목록 이적 28모브 ÷ 2 와 일치)인데 앱 추정 12대. 빠진 둘은 ①제자리 재적재 MSKU5260201(33/09/04 → 33/09/04 — 자리가 안 바뀌니 EDI 대조로는 못 잡는다) ②«평택 작업에 안 걸린다»로 걸러진 이적 MRKU9040939(17/01/82 → 3/03/88). 수리 — 수집기(MailPilot 2.33 collector/restowlist.py)가 항차 폴더의 RESTOW 서류(PDF·엑셀)를 읽어 `voyages/{키}/restowList` 에 넣고, 앱은 시프팅 캐시 창구(computeShiftingMapCached) 한 곳에서 서류가 있으면 그것을 정본으로 쓴다(추정 규칙 computeShiftingMap 은 그대로 두고 표시·집계만 바꾼다). 화면은 «선사 서류 정본 · 앱 추정 N — 차이 M» 을 같이 적고 제자리 재적재는 목록·인쇄·엑셀에 «제자리» 로 적는다. 콘앱도 같은 판정 한 벌(ctShiftMap → ConeParse.restowMapFromDoc). 연막 smoke_restow(MCAT 635N 실서류 14 · 실 BAPLIE 두 벌 — 서류 있으면 14, 없으면 종전 12).   // 3.43-03 · ConeOne 2.49-03 **카고플랜 FR 이 OT 로 그려지던 것** — 검수사 2026-09-11 «SWTD 카고플랜에서 FR을 OT로 오류 표기 수정바람». 원인 — ASC 파서가 장비코드 FR40·FR20 에 oog 만 켜고 fr 은 안 켜, SWTD 9013E 양하 FR 4대(42PF 3 · 22PF 1)가 칸 판정(getMarkV2 — fr 플래그만 봄)에서 OT 가지로 떨어졌다. 같은 카고플랜 별첨은 iso 셋째 자리 P 를 따로 봐 FR 로 세어 칸과 별첨이 서로 달랐다(베이플랜은 OOG·△, 자리 고르기 창은 OT 배지, 맞교환 안내는 OOG(규격초과), 콘앱 베이뷰·카고플랜도 OT). 수리 — utils isFlatRackContainer 한 벌(플래그 → 파서 규칙 셋째 자리 P·F·6 · 4583·4584 · isoCategory FR · 장비코드 FR·PL·FP)을 카고플랜 칸·별첨 · 실시간 별첨(legendLiveOf) · 베이플랜(글자·막대·테두리) · 자리 고르기 창 · 맞교환 안내 · 콘앱(_ediRowOf, 번들 ConeParse 로 같은 함수)이 같이 부른다. 연막 smoke_frmark(SWTD 9013E 실데이터 양하 859·선적 105 렌더 — 전 OT 4칸 → 후 FR 4칸, 진짜 OT 42UT 1대는 OT 그대로).   // 3.43-02 **씰 잘림·파손 «대처법»은 현장 절차(원장)로** — 검수사 2026-09-11 «씰 잘림(씰 파손) 대처법을 물었는데 씰번호 틀림을 알립니다. 이걸보면 ai가 적용된건지 의심이 갑니다». 원인 셋 — ①mirKnowledge 씰 원장 패턴이 «잘려·잘렸·끊겼» 동사꼴만 받아 «잘림·파손·훼손» 명사꼴·«봉인·실» 을 못 받고, 묻는 말 게이트(ASKING)에 «대처·처리 방법·조치» 가 없었다 ②nlSearch 가 «파손» 을 무조건 데미지(컨 손상) 인텐트로 세워 «씰 파손 대처법» 이 «등록된 데미지 없음» ③howToQuery 가 원장보다 기능 색인(「실번호 입력」)을 먼저 봐 «씰 잘림 대처법» 이 실번호 입력 설명으로. 수리 — 원장 패턴·ASKING 확장 · 씰·봉인·실의 파손은 데미지 인텐트에서 제외 · «대처법·처리 방법·조치» 를 사용법 강신호로 · howTo 도 원장 먼저(answerHowCore 와 같은 순서). 규칙이 답하니 모델은 안 부른다(공용 키 값 0). 연막 smoke_mirsame ⑦.   // 3.43-01 · ConeOne 2.49-01 **콘앱 미르가 검수앱과 같은 재료를 본다** — 검수사 2026-09-11 «몇시에 끝나하고 콘앱과 검수앱에 물어보면 실제 작업한량이 틀리거나 작업시작도 안한걸로 답하는 경우가 있고 끝나는 시점을 답을 못함» · «검수앱과 콘앱에 공통되는 질문이라면 답은 같아야 합니다». 원인 셋 — ⓪**콘앱 loadMirCtx 의 g 가 fbFetch 의 Response 를 JSON 으로 안 풀었다**(2.48~2.49) → info·완료·기록·X-RAY·보류 재료가 전부 빈 객체 — 콘앱 미르는 늘 «아직 시작 전»·«자료 없음»(감사 재현 `mc.info instanceof Response`) ①mirAsk 가 `terminalWork: info.termWork`(카토스 자리 노드, info 에 없음)를 넘겨 늘 null; shipSpeed·pilotForecast 도 안 넘김 ②엔진 ①(콘 이야기)이 «실제 진행 상황»(nlSearch 가 briefingQuery 로도 세움)을 콘앱에서만 브리핑으로 가로채고, 콘 작업표가 있으면 «얼마나 남았어» 를 콘 «남는 곳(반납)» 으로 가로챔. 수리 — g 가 ok 면 json · loadMirCtx 가 terminal_work/{배}·shipSpeed·pilot_forecast/{배} 를 30초 호흡으로 같이 받아 넘김 · 엔진 ①은 진행 물음(_progressLike 한 벌)이면 브리핑을 안 내고 콘 계산 답은 콘 낱말이 있을 때만. 실측 KBTR 2606E·OBWH·SWTD 공통 질문 60문장 — 같은 답 37 → 57(남은 3은 콘앱 도움말 폴백·양쪽 다 약한 답). 연막 `smoke_mirsame`(cone.html 의 실제 loadMirCtx 를 Response 스텁으로 돌린다).   // 3.43 **판 C — 제미나이 여섯 창구(검색패널 AI·선박 소개·PDF 베이·사진 리스트·리퍼 사진·PORT-MIS 캡처)가 미르와 같은 공용 키(검수사 부담, RTDB mir_config)로 돈다** — 종전엔 여섯 곳이 각자 `_storage.get(SK.geminiKey) || GEMINI_API_KEY` 로 키를 찾아 개인 키 없는 검수원은 전부 «키 없음». 이제 `gemini.js resolveAiKey`(공용 → 개인) 한 벌 + `aiCall`(타임아웃 25~120초 · `ai_call_log/{YYMMDD}` 한 줄 — 누가·어느 창구·몇 초·HTTP 코드). 🔑 설정창은 공용 키가 있으면 «차단됨» 빨간 상자 대신 «공용 키 사용 중». 덤 — 검색패널 AI 버튼이 클릭 이벤트 객체를 질문으로 넘기던 것(`onClick={handleAskAI}`). 모델은 종전 그대로 gemini-2.5-flash(공용 키 실호출 확인). 콘앱 무변경(2.49).   // 3.42 **판 B — 미르가 못 알아들은 말을 모델이 번역하고, 규칙이 없는 것은 자료를 실어 답한다** (검수사 2026-09-10 «어떤질문이 들어 올지는 저도 모릅니다. 제가 원하는건 그질문들에 적당한 답을 해주길 원합니다» · «1번인데 회사 유료키가 아니고 제가 부담합니다»). 규칙(answerOneRaw)이 먼저이고 **약한 답**(null · 잡아채는 길 `_trace.via`: 사용법 매뉴얼·현재 시각·베이사전 타령·진행 잡답·지식 추측·콘 안내 · 모르는 낱말 남음)일 때만 `src/mirModel.js` 한 함수가 공용 키(RTDB `mir_config`, 검수사 부담, 3.5-flash-lite)로 ①질문만 보내 «미르 말»로 번역 → 규칙 재실행(답하면 `mir_lexicon` auto 등록, `mir_misses` 기록) ②없으면 요약·계산해 둔 사실·질문에 맞는 컨 30줄만 실어 3문장 답(AI 표시, **답 속 숫자가 자료에 없으면 버린다**). 문지기 — 같은 문장 한 번·10초·하루 300회(`mir_model_log`)·키 없으면 침묵·월 지출 상한 ₩15,000(구글 콘솔). 검수앱 작업창(음성 교정 fixQuestionWithAI 대체, 타이핑도)·홈·떠 있는 미르·콘앱(하드코딩 키·coneFixQuestionWithAI 삭제) 네 입구 한 벌. 덤 — «2호기 11:15 시작했어» 가 갱 배분에 가로채이던 것. 관문 4 실호출 79문장(번역→규칙 28 · 자료 답 29 · 값 $0.038)   // 3.41-01 떠 있는 미르 셋(검수사 라이브 신고 2026-09-10) — ①묻고 나면 칸을 비우고 물은 말은 답 위에 남긴다 ②말하기 전에 쌓인 발화를 끊고 플랜 명령은 말부터 하고 열며 시트를 내린다 · 작업창은 «카고플랜 보여줘»(listQuery)에 양하/선적 단추가 같이 뜨던 것을 막고 단추를 누르면 되묻는 발화를 끊는다 · 플랜 덮개를 열 때 발화를 끊는다 ③«작업중인 선박 언제 끝나» — 이름 대신 «작업중인 배» 면 utils.isWorkingNow 한 벌로 지금 일하는 배, 여럿이면 «어느 배?» · 덤: paceQuery(«작업 속도»)가 «조건 있음»에 빠져 null 이던 것(3.24부터)   // 3.41 **미르 한 벌 + 어디서나 + 창구 15건** — 검수사 2026-09-10 «미르를 하나로 만들고 싶습니다» · «미르를 앱 어디에든 항상 띄워서 모든 질문을 받을수 있게» · «앱이 갖고 있는 자료를 막힘없이». 답 고르는 자리 넷(작업창·양하선적 탭 카드·홈·콘앱 엔진)을 `src/mirAnswer.js` 한 벌로 내리고(어디서 물어도 같은 답), 오른쪽 아래 떠 있는 미르(`MirFab`, 콘앱과 같은 얼굴·시트·마이크·목소리)를 모든 화면에 두고, 값은 있는데 말이 없던 창구 15건(끝네자리+온도·실번호·중량·다음항·B/L·치수·X-RAY·상태 / 마감텔리 수치·해치·접안 현측·시작종료 실적·규격초과·엠티실·특수제작컨·임시창고·커트씰·보류·수화물확인·환적)을 `src/mirFacts.js` 로 연다. 「3426 온도」가 답을 못 내던 직접 원인 — 온도·톤·피트 낱말이 끝네자리를 통째로 지우던 것(nlSearch skipDigits) — 홀로 선 네 자리는 살린다. «N호기» 숫자는 끝네자리가 아니다(«1호기 선수 2호기 선미» → 12 오답). 콘앱(2.48)은 xrayList·held·reports 를 미르에게 싣고 30초마다 다시 받는다.   // 3.38-01 같은 그림에 그려지는 베이(차 2 이하)를 두 호기로 가르던 것 — 라이브에서 1·2호기가 «BAY 17 BAY (18)19» 한 그림을 함께 그렸다(검수사 화면)   // 3.38 **동방(PNCT) 배도 수석 보드에 «호기별 작업 베이»가 뜬다** — 터미널이 컨별 호기를 안 줘도 **같은 시각에 함께 찍힌 베이는 서로 다른 호기**이므로 선수(작은 베이)부터 호기 번호 오름차순으로 잇는다(검수사 «동시간대 컨이 두대가 양하가 되었는데 하나는 02베이 하나는 10번베이라면 1호기 2번베이 2호기 10번 베이»). 실측 OBWH 2735E — 1호기 02·03·06 24대 · 2호기 10·14 28대 = QC101·QC102 완료와 일치   // 3.37 특수제작컨(32ft·34ft) 리퍼 온도 제외   // 3.36 카고플랜 해치커버 중앙   // 3.35 **실시간 작업 보드 — 베이 표기 글자를 배율 밖으로 빼 크게.** 검수사 2026-09-09 «수석대시보드 실시간 작업현황… 베이표기 부분이 너무 작다고 합니다 한눈에 들어오게 크게» · «베이가 작은거는 확대하면 되는데 **베이표기 글자** 이야기 입니다» · «그걸 한눈에 들어 오게 하되 **베이크기 확대 축소에 상관없이** 크게». 뿌리 — 보드용 한 장(`BayPlan onlyBay`)은 제목을 `fontSize+4` = **11px** 로 그리는데(`compactZoom` 0.36 → 본문 7px), 그 위에 `FitBox` 가 `transform: scale()` 을 한 번 더 걸어 화면에서는 **6~8px** 이 됐다. 확대해도 그림과 같은 비율로 커질 뿐이라 «확대 축소에 상관없이» 가 안 됐다. ⇒ **제목만 `scale()` 밖으로 뺀다** — `BayPlan` 에 `titleOut`·`onTitles` 를 달아 그림 안 제목은 감추고 그 장들의 제목을 부모에게 올려 주며, 보드는 새 `BoardBayCell` 이 그것을 **고정 px**(호기 3대까지 24 · 4대 18 · 5대 16)로 그린다. 배율을 아무리 줄여도 이 글자는 그대로다. ⚠ 제목은 **BayPlan 이 만든 그것**을 그대로 쓴다(«BAY (12)13» 짝 표기 포함) — 보드가 `c.bay` 로 다시 지으면 짝 표기가 갈린다(규범 §4-4). 제목을 올리는 셈은 `onlyBay` 갈래와 같은 벌이다. ⛔ **훅은 조기 반환보다 앞이어야 한다** — 첫 판이 `containers.length === 0` 반환 **뒤**에 두어, 자료가 «없다 ↔ 있다» 로 바뀌는 순간 렌더마다 훅 수가 달라져 React 가 통째로 죽었다(감사 실측 — «Rendered more hooks than during the previous render» · 화면 0자). EDI 가 늦게 오거나 **양하↔선적 탭을 누르는 것만으로** 걸리는 길이라 보드뿐 아니라 베이플랜 탭·미르 오버레이가 같이 죽었다. 연막검사 `tools/smoke_bphooks.cjs` 두 방향(없다→있다 · 있다→없다)이 그것을 잰다. ⚠ **격자 코드·CSS 는 한 글자도 안 바꿨다**(보드 DOM 이 3.34 와 바이트 동일 · CSS 해시 그대로 · 확대 단추 무수정). 다만 제목 줄이 그림 **위**에 새로 생기므로 그림이 조금 작아질 수 있다 — **폭이 먼저 닿으면 그대로이고, 높이가 닿을 때만 제목 줄 높이(30px)만큼** 줄어든다(`FitBox` 는 `min(폭비, 높이비)` 다). 폰 줄 예산(`fitMaxH`)에서도 그 30px 을 함께 뺐다 — 안 빼면 칸이 예산을 넘어 스크롤이 생긴다. ⚠ 그 대가로 폰에서는 칸이 그만큼 작아진다(실측 — 800px 화면 2척 −11% · 3척 −19%). 빼는 쪽이 맞지만 «조금» 은 아니다. ⚠ 빌드 CSS 에 **안 쓰는 유틸 한 줄**(`.py-0`)이 늘어 해시가 `index-DYf5yYNn.css` → `index-DX3GcWsO.css` 로 바뀐다(36바이트 · 그 클래스를 쓰는 요소 0개 · 화면 무영향). ⚠ 제목 폴백(`BAY ${c.bay}`)은 걷어냈다 — 그것이 **제목 둘째 벌**이었다(감사 지적 §4-4). 못 받으면 아무것도 안 적고 그림이 사정을 말한다. ✅ 연막검사 — `tools/smoke_liveboard.cjs`(배율 밖·16px 이상·옛 제목 없음·크기 자 한 벌·`hideTitle`·`titleOut` 기본값 보호·`fitMaxH` 보정) · `tools/smoke_liveboard_pnct.cjs`(동방 갈래도 배율 밖·16px 이상·옛 제목 없음) · `tools/smoke_bphooks.cjs` + 하네스 둘(신설, 훅 순서 두 방향 · build.sh 등록). 사보타주 **10종**(제목 10px·`hideTitle` 제거·`titleOut` 제거·`onTitles` 죽이기·동방 갈래만 옛 FitBox·크기 자 8px·`hideTitle` 기본값 뒤집기·`titleOut` 기본값 뒤집기·`fitMaxH` 보정 제거·훅을 반환 뒤로) 전부 빨개진다. ★ 다른 클로드 감사 «부» 셋을 그 자리에서 수리했다 — ①훅을 조기 반환 뒤에 둔 것(화면이 죽는다) ②연막검사 구멍 셋(동방 갈래·4·5호기 크기·`hideTitle` 기본값) ③판 기록의 «한 픽셀도 안 바뀐다» 가 참이 아니던 것.   // 3.34 **콘앱 2.43 — 트윈인데 베이가 하나만 보이던 것 + 콘 트윈 전체 화면.** (검수앱 화면은 한 픽셀도 안 바뀐다 · 콘앱 `cone.html` 만 바뀌지만 `sw.js` 버전이 안 바뀌면 폰이 새 화면을 안 받으므로 판을 올린다.) 검수사 2026-09-08 «콘앱에서 실시간 작업현황을 보는데 **트윈 작업인데 베이가 하나만 보이는** 문제가 있습니다». 뿌리 — 트윈은 40ft 한 칸에 20ft 둘을 같이 드는 것이라 **베이가 둘**(17 과 19, 그 사이 18 이 그 40ft 칸)인데 콘앱의 짝은 «짝수+뒤홀수»(16 과 17)라 두 묶음이 어긋난다. 그래서 트윈 두 대가 **언제나 다른 짝**으로 갈리고, `ctCompute` 가 호기마다 마지막 한 대만 남기므로(`cranes[eq]=r`) 나머지 한 베이가 카드·위 띠·줄 그림에서 통째로 사라졌다. 실측 2026-09-08 작업 중 네 척(STSE 2669E·SWBT 2614N·XTPG 539E·DPRT 2610N) — 트윈 **79쌍**이고 **베이 차이는 전부 2**(다른 차이 0건), 두 대 사이 간격 중앙값 2초·90% 4초·최대 70초. ⇒ ①**트윈 판정 한 벌** `ctTwinOf` — 같은 호기·같은 열·같은 단·베이 차 2·2분 안. ⚠ 계획(EDI)으로 «여기는 트윈 자리다»를 미리 판정하지 않는다 — 자리가 둘이어도 따로 들 수 있다, **든 것만 트윈이다**. ②카드·위 띠에 «베이 17+19» 로 **두 베이를 같이** 적는다(전체 화면을 안 켠 사람도 반쪽만 보지 않게). ③「⤢」 로 **콘 트윈 전체 화면** — 검수사 «글자 없이 전체 세로 화면으로 그림만 두베이 빼는곳 반짝임 닫기 버튼 좌우 스크롤 예를 들면 3 4호기 일떄 첫화면 3호기 좌로 밀면 4호기» · «베이표시는 21 (22)23» · «위에 데크홀드 아래도 데크홀드 쉽게 카고플랜 1칸씩 보여준다고 생각하시면 됩니다». 카고플랜 한 칸(위 반쪽 21 · 아래 반쪽 (22)23)을 통째로 띄우고 반쪽마다 갑판·선창을 다 그린다(가는 금이 가른다). 문장·배 이름·시각은 없고 위에 호기 번호·베이 표시·✕ 뿐이다. ⚠ **안 잘린다** — 칸 크기를 화면에서 거꾸로 낸다(`twFit`). 검수사 «로우가 11개 13개라도 한화면에 보이게 해주세요 잘리지 않게». 실측 SWBT 2614N 양하 «17 (18)19» 는 **20줄·11열**이 폰 세로 한 화면에 든다(칸 25×28px). ⚠ 자리·완료 판정은 콘 타이밍 줄 그림과 **같은 한 벌**이다(규범 §4-4) — 검수원이 앱으로 찍은 자리 → 터미널이 준 자리 → 계획 자리, 완료는 `completed`(앱 확정)·`termWork`(터미널, 초록 점선). 열은 베이사전 골격(`bvBaySkeleton` deckRowPos·holdRowPos)이 먼저이고 골격에 없는 열도 반드시 그린다(2.35 와 같은 규칙). ✅ 검수사 미리보기 확인 — «원한 그림입니다». 연막검사 `tools/smoke_conetwin.cjs` **128항**(build.sh 등록) — 소스에서 꺼낸 그 함수를 돌리고 화면은 jsdom 실렌더로 읽는다. 검사 자(여백)는 **CSS 에서 뽑아** 상자 셈을 따로 한다 — `twFit` 의 식을 베끼면 twFit 이 틀려도 통과한다(감사 지적, 처음 판이 그랬다). 사보타주 **23종**(가운데 40ft 를 앞쪽으로·앞 반쪽을 사전 없이 붙이기·트윈 가운데를 사전에 묻기·잰 값을 :root 에 적기·가로/세로 밀기 안 켜기·아래 반쪽에서 40ft 빼기·twFit 고정값·폭만 고정·골격 무시·골격 밖 열 버리기·컨번호 통째로·쪽을 자리번호로·동방 문지기 빼기·그림자 없애기)이 **전부 빨개진다**. 칸 판정은 실사전 **여덟 척 195베이 전수**로 재되, 코드의 «어느 40ft 를 고르나» 식을 베끼지 않고 **답이 갖춰야 할 여섯 성질**로 잰다(가운데는 그 배의 짝수 · 앞뒤는 사전에 있는 홀수이고 가운데∓1 · 앞<가운데<뒤 · 물어본 베이가 칸 안 · 가운데는 이웃 · 제목이 값 그대로). 식을 베끼면 그 자체가 항등식이다(3차 감사 지적). 매뉴얼 2줄 동반. ★ 다른 클로드 감사를 **두 번** 받아 «부» 여덟을 그 자리에서 수리했다. 1차 — ①`ctTrioOf` 가 **사전을 안 보고 짝을 지어냈다**(실측 STSE 순간의 12.2%·SWBT 4.5% 가 배에 없는 40ft 베이를 가운데로 삼았다 — 부록 A 그대로다) ②30초 갱신이 **보던 호기를 말없이 바꿨다**(31회 중 19회 — 자리번호로 잡았다) ③동방(«베이 18»)을 «18호기» 라 적었다(2.38 문지기가 새 화면에 없었다) ④`twFit` 이 상자 안여백 14px·틈 하나·갑판선창 금 9px 를 빼먹어 320px 폰에서 12~16px 넘쳤다 ⑤연막검사가 사보타주 6종을 통과했다. 2차 — ⑥**`pairEven` 의 뜻을 거꾸로 읽었다.** 그것은 «그 20ft **앞**의 40ft» 가 아니라 «그 20ft 가 **속한** 40ft» 다(DXQD `{3:'04'}`). 그래서 트윈 아닌 칸이 «(04)03» 처럼 앞뒤가 뒤집혔다 — 감사 실측 실적 순간의 **25.6%**, 옛 판보다 나빴다. ⇒ 가운데 40ft 를 **사전의 짝수 베이 목록**에서 고르고(앞뒤 둘 다 있을 때만 `pairEven` 이 갈래를 잡는다), 앞·뒤 20ft 는 사전에 있는 것만 붙인다. 실측 — 같은 베이 17 이 SWBT 는 «17 (18)19», STSE 는 «15 (16)17» 이다. ⑦**잰 칸 크기가 화면에 안 닿았다** — `twFit` 이 `:root` 에 적는데 `.tw{--twc:26px}` 가 상속을 이겨 브라우저는 늘 26×26px 로 그렸다(감사 Chrome 실측). ⇒ `#ctTwin` 자신에게 적는다. 가로 하한에도 밀기를 연다. ⑧연막검사가 그 둘을 못 잡았다(사보타주 5종 통과). 3차 — ⑨연막검사 정답표가 **코드 식을 전사**해 항등식이었다(«답이 갖춰야 할 성질» 검사로 다시 짰다) ⑩판 기록 항수가 어긋나고 «건너뜀»을 ✓ 로 찍는 항이 있었다 ⑪갈래를 모를 때 **말없이 뒤쪽으로** 골랐다(실측 SWTD 33 한 자리 — 이제 콘솔에 밝히고 `src:'dict?'` 로 남긴다). 덤으로 셋을 더 막았다 — 트윈인데 사이 40ft 가 그 배에 없으면(이웃 홀수쌍의 46.6%) 지어내지 않고 두 20ft 만 그린다 · 뒤 20ft 가 없는 40ft 전용 칸은 «33 (34)» 로 40ft 임을 남긴다 · `twFit` 이 전체 화면 없이 불리면 `:root` 에 적지 않고 물러나며 그 사실을 밝힌다. 4차 — ⑫검사 문구 오기(«일곱 척»)와 늘 참인 항 하나를 걷고, 감사가 짚은 **그물 구멍 여섯**을 막았다(트윈을 트리오에 넘기는 배선 · 앞 베이가 마지막일 때의 반짝임 · 선창 열을 갑판 열로 갈아 끼우기 · 누른 호기를 여는지 · 열 차례 · 갑판/선창 폭이 다른지). 배선 항은 **사전만 STSE 로 바꿔** 트윈 답과 사전 답을 갈라 놓고 잰다 — SWBT 사전으로는 둘이 우연히 같아 안 걸린다.   // 3.33 **콘앱 2.42 — 카고플랜 베이 차례가 뒤집힌다(낮은 베이부터 · 우측으로).** 검수사 확정 2026-09-08 «콘앱의 카고플랜을 뒤집어 주세요» · «검수앱의 카고플랜과 역방향으로» · «콘앱은 검수 카고플랜을 뒤집은 형태» · «우측으로» · «낮은베이부터 높은베이로». 이유 — «콘 인력이 보던 것이 검수앱과 반대였음». 검수앱은 `cargoPlanCore.autoPageLayout:303` 이 «각 행 내부: 큰 번호 좌측 (카스피 정답)» 이라 윗줄이 17 → 01 로 간다. 콘앱은 그 반대다. ⇒ `PrintableCargoPlanV2` 에 `flipBays` 문 하나(기본 **false**)를 달고 **줄 하나를 통째로 뒤집는다 — 별첨 상자·빈 칸까지 함께**(검수사 «거울에 비친것 처럼»). 콘앱 진입점만 켠다. 실측 ATPR 2640W — 윗줄 «13 (14)15 · 09 (10)11 · 05 (06)07 · 01 (02)03» → «01 (02)03 · 05 (06)07 · 09 (10)11 · 13 (14)15», 아랫줄도 같다. ⚠ **안 건드리는 축 둘** — ①칸(열) 좌우(10 08 06 … 09 그대로) ②한 칸 안 위·아래(17 위 · (18)19 아래). ⚠ **별첨은 «표»지만 줄 안의 한 칸이라 같이 간다** — 베이 상자만 뒤집던 첫 판은 별첨이 아랫줄 왼쪽에 남아 배 그림이 1~2칸 들여쓰기 됐고, 검수앱에서 «읽기가 끝나는 쪽»이던 자리가 콘앱에서는 «읽기가 시작되는 쪽»이 됐다(감사 실측 — `topCount=⌈(N+1)/2⌉` 이라 남는 칸이 언제나 1~2개, 두 줄짜리 배 전부 해당). 실측 SWBT 2614N 선적 아랫줄 — 첫 판 «별첨·빈칸·25·29·33·38» → 이 판 «25·29·33·38·빈칸·별첨». ★ **내가 세 번 헛짚었고 검수사가 세 번 바로잡아 주셨다** — ①칸 좌우를 거울 반전 ②베이 전체 차례를 뒤집어 재배치 ③칸 안 위·아래 맞바꿈. 뿌리는 하나다 — **검수앱 카고플랜이 어떻게 서 있는지를 안 보고 «오름차순이겠지»로 시작했다.** `autoPageLayout` 을 먼저 열었으면 첫 번에 맞았다(지침서 부록 A «있는 데이터를 안 보고 이렇겠지 하고 만드는 것»). ✅ 연막검사 `tools/smoke_coneflip.cjs` **67항**(build.sh 등록) — 소스 문자열이 아니라 **jsdom 실렌더의 칸 차례**를 읽어 잰다. ⚠ 베이 상자만 세면 안 된다 — **별첨·빈 칸도 그 줄의 한 칸**이라, 상자만 세는 검사는 위의 결함을 못 본다(감사 지적). ⚠ 제목을 낱개로 세도 안 된다(한 상자가 «13» 위 · «(14)15» 아래 둘을 낸다). ATPR·MCSC **두 배 × 선적·양하** 넷을 검수앱/콘앱으로 나란히 그려 줄 통째 반전·별첨 자리·낮은베이→높은베이·칸 수 보존·열 라벨 불변·상자 안 그림 바이트 동일을 재고, **`flipBays` 를 아예 안 넘긴 그림**이 검수앱과 바이트까지 같은지(하네스가 늘 명시하면 기본값이 뒤집혀도 못 본다 — 감사 지적), 검수앱 **호출부 다섯 곳**(인쇄허브·BayPlan·선적플랜편집·항차화면·planedit)이 하나도 안 켜는지까지 잰다. 항등식이 아님을 사보타주로 실측 — 옛 «상자만 뒤집기»로 되돌리면 **20항**이 빨개지고, 기본값을 `true` 로 바꾸면 **3항**(그중 둘은 실렌더)이 빨개진다. ★ 다른 클로드 감사 «부» 둘을 그 자리에서 수리했다 — ①이 별첨 자리(뒤집기가 거울이 아니었다) ②판 기록이 실제와 정반대이던 것(«선수가 오른쪽» · 목표 윗줄이 검수앱 현행 차례 — 세 번 헛짚던 때의 잔재, 규범 §10-6). 감사가 실측한 «가» — 검수앱 카고플랜 DOM 이 라이브 3.32-02 와 **6쌍 전부 바이트 동일**(ATPR·MCSC × 선적·양하 + BayPlan + BayDetail) · RTDB 활성 **8쌍 7척** GET 재현 · 상자 1~6개 끝단 무오류.   // 3.32-01 **콘앱 2.41 — 베이뷰에서 컨번호가 잘리고, TOP 이 묻히거나 겹치고, 늘 100% 로 열리던 것.** (검수앱 화면은 안 바뀐다 · 번들이 `normPortCode` 를 새로 내보내므로 판을 올린다.) ①**컨번호가 조용히 잘렸다** — 실화면 STSE 2670W 베이플랜 **918칸이 전부**, 잘려 나가는 것이 하필 **끝 4자리**다(줄 끝 + overflow:hidden · 말줄임표 없음). ⚠ 한 줄이 **두 크기**다 — 앞 7자 `.bvc-cnh` 와 끝 4자 `.bvc-cn4`(11px). 첫 판은 그 11px 을 빼놓고 «11자 한 크기»로 재서 «고쳤다»가 거짓이었다(감사가 실화면 전수로 잡았다). 자폭은 **안드로이드 기본 모노스페이스 0.60011em** 으로 잡는다(폰이 현장이다 · Consolas 0.54981 은 더 좁다). ⇒ 좌우 여백 4→**2px**(가용 48→52px) + 두 크기를 **8px 로 통일** → (7×8+4×8)×0.60011−3.0 = **49.81px ≤ 52px**, 안드로이드에서도 2.19px 남는다(Consolas 45.38px). 끝 4자리는 색(#ffd9a0)·굵기로 계속 도드라진다. ②**TOP 단추 둘** — ⓐ전체화면에서 **아예 안 보였다**(감사 실측 — 전체화면 판 z-index 60 인데 TOP 은 40 이라 밑에 깔린다. 같은 줄의 불편신고·새로고침·닫기·미르는 다 위에 뜬다) ⇒ z-index 9998. ⓑ좁은 폰에서 「전체화면 닫기」와 좌표가 겹친다 — TOP right:74 w:48 이라 360px 에서 238~286px, 닫기 left:202 w:44 라 202~246px(320px 에서는 198~246 대 202~246). ⇒ ≤420px 에서 TOP 을 한 줄 위로(bottom 70). 겹치는 폭은 277~367px 뿐이라 이 규칙이 전부 덮는다. 좌하단 줄은 검수사가 «언제든 쓸수 있어야 한다»고 정한 자리라 안 건드렸다. ③**늘 100% 로 열렸다** — ⛶ 단추·가로모드·전체화면은 이미 `applyFit()` 을 부르는데 **첫 화면만 빠져 있었다**(`applyZoom(100)`). 못 재면 종전대로 100% 다(null 경로가 2.40 과 같음을 감사가 확인). ④**항구 코드를 제 손으로 잘랐다** — `short()` 가 «앞 5자 중 2~4번째»를 그냥 잘라 부두코드 `PTK02` 가 «K02», `PYEONGTAEK` 가 «EON» 이 됐다. 실측(활성 17항차 GET) — 바뀌는 행 **339**(PTK02 283 · PYEONGTAEK 53 · 쉼표형 3) · **회귀 0**. 보관까지 넣으면 PTK02·PTK04 만 11,775건이다. 번들이 `normPortCode` 를 내보내고 콘앱이 그것을 지난다 — 1.9(평택 판정)·2.23(시프팅)과 같은 처방, **판정 두 벌 금지**. ✅ 연막검사 `tools/smoke_conebayfit.cjs` **25항**(build.sh 등록) — 치수를 소스에서 뽑아 **두 크기 그대로** 다시 계산하고, «끝 4자리를 11px 로 되돌리면 넘친다»·«2.40 그대로면 넘친다»를 같이 재 항등식이 아님을 보인다. ★ 다른 클로드 감사가 «부» 셋을 잡아 그 자리에서 수리했다 — ①수리가 실은 안 됐던 것(cn4 11px 누락) ②검사가 같은 오산을 복제해 그 결함을 구조적으로 못 잡던 것(+`|| true` 항등식 1항·못 걸리는 항 2항) ③APP_NOTE 가 거짓이던 것. ⚠ 남은 것(인계함) — `INC07`→«C07»(정규화 게이트가 `/^PTK\d*$/` 라 평택만 본다) · `PYONGTAEK`(철자 다름) 80행 · `KRPYOTM` 551행(3.7-07 이 미뤄 둔 값) · 진입 배율이 열 많은 배에서 23~30% 라 8px 글자가 2px 로 그려지는 문제(칸을 넓힐지 소유자 4자를 접을지는 검수사 확답 사안).   // 3.32 **리퍼 실제온도가 종이에서 앱으로 못 넘어오던 길.** 검수사 2026-09-08 «온도가 기록이 안됨 실제온도 SWBT 2614N 배포 끝나고 점검바람» + 본선 리스트 사진 3장. 실측 — SWBT 2614N 풀 리퍼 **37대**(화면 기준 · EDI 만 세면 55대) **전부 기준(세팅)은 EDI 에 있고**(종이 인쇄값과 뽑아 본 6대 전부 일치) **실측은 0대 · `rfCheckedAt` 이 한 번도 안 찍혔다.** 보관소까지 훑으니 사진 판독 자체는 다른 배에서 151대가 성공했는데, 이 배에서만 안 된 이유가 **종이 양식**이었다. 3.25 프롬프트는 SITC 양식 하나를 «표 구조는 보통 이렇습니다» 로 못 박았는데 (NO.│CONTAINER│SLOT│PLUG-IN DATE│POL│POD│**SET TEMP**│시각격자), SAWASDEE BALTIC(Sinokor) 실물은 NO.│LOCATION│CONTAINER NO.│**SETT.TEMP.**│**PLUG IN TEMP.**│POL│POD│REMARK│시각격자 로 ①설정온도가 POD **앞**에 있고 ②프롬프트에 없는 `PLUG IN TEMP` 칸에 손글씨 숫자가 들어 있다(사진 셋째 장 12.6·10.0·20.5·21.0·13.6·15.9·−1.5·7.1) ③REMARK 칸에도 손으로 적은 자리 번호(180184 = 18-01-84)가 있다. 프롬프트는 «손글씨 우선» 이라고만 해서 그 칸을 실측으로 집을 여지가 그대로였다. ⇒ ①**열 순서를 가정하지 않는다** — 열 제목을 읽어 칸을 정하게 하고, `PLUG IN TEMP`·`REMARK`·`LOCATION` 을 이름으로 막았다. 손글씨 앞자리가 칸 경계를 넘어 «14.0» 이 «4.0» 처럼 보이는 줄도 알려 준다(실물에 여럿 있다). ②**여러 장을 한 번에** 받는다 — 종전엔 `files[0]` 하나뿐이라 3장을 세 번 찍어야 했다(어제 RZOR R097E 도 3장이었다). 장마다 읽어 누적하고, 한 장이 실패해도 나머지를 읽고 실패한 장을 밝힌다. ③**판독 검산**(`ocrSetMismatch`) — 읽어 온 «설정온도»를 **EDI 온도와 맞춰 본다.** 설정온도는 잣대가 있는 유일한 값이다. 어긋나면 그 줄에 «읽은 값 ≠ 자료 값» 을 붙이고, 절반 넘게 어긋나면 «판독기가 다른 칸을 읽은 것 같습니다» 를 크게 띄우며, **어긋난 줄이 남은 채로 「확인 완료」를 누르면 한 번 더 묻는다** — 문구뿐인 경고는 그대로 찍히기 때문이다(감사 지적 · 규범 §4-3). ⚠ 자료 온도에 **단위가 붙어 온다**(`parseAscFile` 이 `-18.0℃` 로 쓴다 — 보관소 8,110대 중 280대, 17개 항차·모드는 전량). `Number('-18.0℃')` 는 NaN 이라 그냥 견주면 **잘 읽은 판독에 전량 경보**가 뜬다 — `tempNum` 으로 단위·유니코드 음수부호를 걷고 견준다(감사가 잡은 회귀). ⚠ **실측은 여전히 사람이 넣는다** — 3.25 확정대로 「세팅온도 채우기」는 기준 칸만 채우고 실측 칸은 안 건드린다. ✅ 연막검사 `tools/smoke_reeferphoto.cjs` **34항**(build.sh 등록) — 프롬프트 문구·여러 장 누적·검산을 **실제로 돌려** 잰다(꽂을 때 온도 12.6·10.0 을 읽어 오면 2대가 걸리는 것 · ℃ 표기 · 숫자 자료까지). 3.25 회귀 가드 두 항은 **주석이 아니라 코드 모양**을 보게 고쳤다(감사가 항등식이라 잡았다). 매뉴얼 2줄 동반. ★ 다른 클로드 감사 «부» 셋을 그 자리에서 수리했다 — ①℃ 표기로 전량 헛경보 ②여러 장 누적이 앞 장 실측을 지움 ③연막검사 두 항이 항등식. 덤으로 «몇 대 채움» 안내가 늘 0 이던 것(3.25 부터 있던 결함)·경고가 안 지워지던 것도 고쳤다. 실패 문구 중복은 **전량 실패 갈래만** 합쳤다 — 일부만 실패했을 때는 «어느 장이 실패했는지»가 필요해 장 번호를 남긴다. 덤으로 감사가 잡은 잠복 결함 둘도 막았다 — 모달을 연 뒤 리퍼로 승격된 컨을 빈 칸으로 읽어 오면 화면이 통째로 죽던 것(3.31 에도 있던 것)과, 대화상자가 막힌 환경에서 저장이 조용히 죽던 것.   // 3.31 **마감텔리 선사 칸 — 선적 DWS 를 CSC·DSL 로 가르고, 40HC 를 HC 칸에 넣는다.** 김명보 부장 메모 2026-09-08 09:07 «선적 dws ==csc dsl로 구분» · 검수사 «김부장꺼 작업해줘». 정본 마감텔리 22건을 다시 읽어 **부모 선사 아래 자식이 있다**는 것을 확인했다 — STSE·STMJ 는 OPERATOR 칸이 «DWS» 한 번이고 그 아래 PORT 칸이 «(CSC) TAO» · «(DSL) TAO» 두 줄로 갈린다(TMPZ 는 «TJM» 아래 «(DWS)» · «(MAS)»). 그런데 앱 사전에는 DWS 자리가 **아예 없어** 그 20대가 맨 뒤 «순서 미확정»으로 밀렸고, 자식 구분도 없었다. ①`subOps` 로 부모·자식을 적고 `matrixRows` 가 부모는 한 번, 포트 칸은 «(자식) 항구» 로 낸다. ⚠ 표에 찍는 글자는 `opLabel`·`portLabel` 로 따로 내고 `op`·`port` 는 **원래 값 그대로 둔다** — 변형 양식(TMPZ)이 `pairRows` 의 «자식|항구» 로 행을 찾기 때문이다(라벨을 박으면 그 배 Final Work 가 4행을 통째로 버린다 — 감사 실측). ②`opAlias` — **같은 세 글자가 배마다 다른 선사다.** XTPG·DXQD 정본은 DWS·WDF 를 그대로 쓰고 STSE·STMJ 정본은 그 자리에 DSL·WDG 를 쓴다. 그래서 공용 변환표가 아니라 배별 사전이다(공용으로 바꾸면 DXQD 마감텔리가 깨진다). ⛔ **근거 없이 가르지 않는다**(`opAliasNeeds`) — 선적 자료는 선사별 CLL 파일이 CSC 를 따로 주고 나머지를 DWS 로 뭉쳐 주는데, CSC 가 한 대라도 있으면 CLL 이 온 것이니 남는 DWS 는 DSL 이다. CSC 가 하나도 없으면 CLL 이 안 온 항차라 가를 근거가 없다 — 그때는 DWS 그대로 둔다. 실측 STSE 2653E 선적 20대는 정본이 CSC 8 + DSL 12 인데 자료에 그 구분이 없다. «DSL 20» 으로 적으면 8대가 아무 표식 없이 틀린 줄에 실린다(감사 지적). ③덤으로 **규격 칸 오류**를 잡았다 — 자료에 코드 계열이 둘인데(`45GP`=ISO 6346 40ft 하이큐브 · `40HC`=하역사 약식) 종전 `/^4[5-9]/` 가 앞엣것만 잡아 `40HC`·`40HR` 이 «40'» 칸으로 떨어졌다. 같은 배인데 선적은 HC 칸, 양하는 40' 칸에 찍혔다. 실측 STSE 2653E 양하 — 정본 HC 204 인데 앱은 40' 204. ⚠ HC 칸은 **40ft 짜리만** 간다 — `20HC`(실측 ZXJU0130421, OBWH 세 항차)는 앱 확정 규칙대로 20' 칸이다(감사가 잡은 회귀). `436E`·`4310`(9'0") 도 정본이 40' 칸이라 약식으로 읽는 것은 `20xx`·`40xx` 둘뿐이다. ④선사 판정을 **한 벌**로 배선했다 — 제 목록을 따로 만드는 곳이 일곱이라 하나만 고치면 같은 워크북·같은 인쇄허브 안에서 코드가 두 벌이 된다(감사 실측). 마감텔리(`ptkContainers`·Act.Cntr-Seal·SHIFTING)·작업리포트·검수리스트·항차 화면·카고플랜 색·수석 보드·통합검색이 전부 `shipOpMapper` 를 지난다. 선사로 나가는 선적 EDI 회신(`loadingEdiExport`)은 **일부러 안 건드렸다** — 거기 NAD+CA 는 원자료여야 한다. ✅ 실데이터 — STSE 2653E&2654W 를 **정본 엑셀 Final Work 와 행 단위 대조**: 선사 순서 SIT → DWS → TJM → EAS → WDG → SKR 일치, **양하는 16행 전부 수치 일치**(CSC 3/0/3·DSL 4/2/4 포함). 선적은 CLL 이 안 온 항차라 «DWS 20» 한 덩어리로 두었고 합계가 정본 20 과 같다. CLL 이 오는 지금 항차 2669E 는 선적이 «(CSC) 5 · (DSL) 12» 로 갈리고 CLL(CSC).xls 5대·CLL(DWS).xls 14대와 맞는다. 연막검사 `tools/smoke_opalias.cjs` **38항**(build.sh 등록) — 문자열 대신 **동작으로** 재고, 근거 없을 때 안 가르는지·20ft 하이큐브·TMPZ 매칭 키까지 잰다. ★ 다른 클로드 감사 «부» 넷을 그 자리에서 수리했다 — ①20ft 하이큐브 회귀 ②TMPZ 템플릿 키 파손 ③별칭 누출 다섯 자리 ④판 기록 과장.   // 3.30 **수석 보드에서 4·5호기가 조용히 사라지던 것.** 검수사 확답 2026-09-08 «③3 유지하고 «+N호기 더»를 누르면 펴지게». «최대 3갱까지 보이게»는 2갱 시절 요구인데 부두 정본은 **PCTC 4호기 · 동방 5호기**(`utils.equipNumbersForPier`)다 — 4갱이면 한 갱(25%), 5갱이면 두 갱(40%)의 작업 베이 그림이 통째로 없었고, 동방 경로(`boardBaysOf`)는 «+N호기 더» 안내조차 없이 **조용히** 빠졌다(감사 실측). ⇒ 기본은 3칸 그대로 두고(칸이 좁아지는 것을 검수사가 싫어하셨다) 머리줄의 «+N호기 더»를 **누를 수 있게** 만들었다 — 누르면 갱 수만큼 펴지고(격자 4·5칸) 동방 베이 칸도 그만큼 나온다. 다시 누르면 접힌다. 카드마다 따로 기억한다(`LiveShipCard` 안 상태라 한 배를 펴도 다른 배는 안 펴진다). ★ 감사(다른 클로드) «부» 를 그 자리에서 수리했다 — 처음 판은 단추를 **호기 갈래에만** 달아 동방 배(「지금 작업 중인 베이」)는 여는 길이 아예 없었는데 판 기록만 «동방 베이 칸도 그만큼 나온다»고 말하고 있었다(조용한 실패를 기록이 덮은 꼴). ⇒ 단추를 **두 갈래가 함께 쓰는 한 벌**(`openBtn`)로 뽑고, 베이는 총량을 먼저 뽑아 화면에서 자른다(접힘 결과는 3.29 와 동일). 동방 격자도 4·5칸을 알게 하고, 펼침 상한 5를 두어 6칸 이상에서 격자가 두 줄로 접히며 그림이 뭉개지는 것을 막았다. 연막검사의 «부두 정본에 4·5호기가 있다» 항이 **APP_VERSION 주석을 물어 언제나 참인 항등식**이던 것도 함수 본문을 보게 고쳤다.   // 3.29 **종이로 나가는 서류에서 숫자가 «조용히» 사라지던 네 자리.** 검수사 확답 2026-09-08 «①행 수에 맞춰 글자를 줄여 한 장 유지». 화면이 잘리면 검수사가 보고 알지만 서류에서 없어진 줄은 **보내는 쪽도 받는 쪽도 모른다** — 그래서 이 판을 따로 뽑았다. ①**FINAL WORKING REPORT** — 머리말에 «A4 풀 1페이지 강제»라 적어 놓고 행 수를 정하는 축(선사 × 포트 × F/E)을 어디서도 안 봤다. `PAD_TARGET=45` 는 **채우는** 코드만 있고 **넘칠 때**가 없었다. 실측 SWBT 2614N 양하가 **72행**이라 27행이 서명란과 함께 둘째 장으로 밀렸고, 최대 조건(선사 10 × POD 5)이면 102행 = 414mm 로 가용 200mm 의 2.07배다. ⇒ 행 높이·글자를 **행 수에서 거꾸로** 낸다 — 45행 이하는 종전 11.5pt 그대로, 그 위는 줄여 **한 장에 69행까지** 담는다. ⚠ 하한은 **본문 6pt**다(검수사 확정 «6pt 는 선내 조명에 장갑 낀 손으로 못 읽는다» · «최소 크기가 넘어가면 2장이 되면 됩니다»). 그래서 SWBT 2614N 양하 72행은 **두 장**이 되고, 서류에 «읽을 수 있는 가장 작은 글자로도 한 장에 안 들어갑니다» 를 한 줄로 밝힌다 — 종전(45행 고정)에도 두 장이었으니 퇴행이 아니고, 이제는 한 장에 69행까지 담고 남는 것만 넘어간다. 가용 높이도 감사가 바로잡았다 — 머리부를 하나씩 세니 **77.9~81mm**(주석의 «73» 이 틀렸다)라 본문 가용은 190mm 다. ②**탤리 엑셀 REMARKS** — 창이 실측 8~9줄 고정인데 remarks 는 **선사 1종당 1줄**이라 10종이면 두 선사분 규격 요약이 통째로 빠졌다. ⚠ 이 자리는 **병합 행**이라 duplicateRow 가 부순다(513행 실측) — 그래서 넘친 줄을 **마지막 칸에 이어 붙인다**(한 줄도 안 사라지고 양식도 안 깨진다). ③**탤리 엑셀 Time Sheet** — 창이 28~33줄인데 해치 open/close 만 36베이 배에서 36줄이라 12줄 이상 유실됐다. 처음엔 Seal 시트처럼 duplicateRow 로 늘리려 했는데 **감사가 그것을 막았다** — 템플릿 17개를 열어 세어 보니 **전부 C:H 병합이 있고 15/17 은 복제 대상 행 자체가 병합의 주인**이라, ExcelJS 가 병합 등록부를 안 옮겨 서명 칸 한복판에 빈 병합 상자가 생긴다(같은 파일 521행이 이미 «duplicateRow 는 REMARKS 병합 행을 파괴» 라고 적어 둔 함정이다). ⇒ 이 판은 REMARKS 와 같은 «마지막 칸에 이어 적기»로 **한 줄도 안 사라지게만** 하고, 양식을 제대로 늘리는 것은 insertRows + 행별 스타일 보존 mergeCells 로 가되 **실물 엑셀을 만들어 열어 본 뒤** 별도 판으로 한다(인계함). ④**검수 리스트** — «75행/단»은 비고가 한 줄일 때만 참이다. FR 최악 비고는 7pt 로 세 줄(3.4 → 7.4mm)이고 별첨은 **전 행이 특수화물**이라 75행이 가용의 1.57배였다. ⇒ 단을 «행 수»가 아니라 **«차지하는 줄 수»**로 채운다(`_noteWeight`·`packCols`) — 한 줄도 안 자르고 넘치면 페이지가 는다. 비고가 짧으면 종전대로 75행. ✅ 실데이터 — 활성 8모드 중 WORKING REPORT 가 바뀌는 것은 SWBT 2614N 양하 하나(72행), 검수 리스트는 셋 다 75행 그대로(회귀 0). 연막검사 `tools/smoke_docfit.cjs` 신설 **23항**(build.sh 등록 · 머리부 상수를 **소스에서 뽑아** «머리부 + 본문 ≤ 273mm» 를 직접 재고 + 소스에서 꺼낸 `packCols` 단위 시험). ★ 감사 «부» 넷을 그 자리에서 수리했다 — ①가용 200 → 190mm(머리부 실측) ②글자 하한 4.5 → 7.7pt(본문 6pt) ③Time Sheet duplicateRow 철회 ④검사가 **항등식**이라 BODY 를 400 으로 바꿔도 통과하던 것. 덤으로 죽은 renderPage(75/75 고정)도 걷었다.   // 3.28 **화면·인쇄·매뉴얼에서 외부 시스템 이름을 걷어냈다.** 검수사 지시 2026-09-08 «앱 내용안에서 특수한 단어 사용을 제한한다. 카스피, 카토스, 트레드링스의 터미널 카운트등» · 확답 «①카스피도 전부 지운다». 3.16 이 리터럴 「터미널(CATOS)」 **하나만** 막아 카스피 31곳·카토스 4곳이 그대로 남아 있었다(전수 실측 — 빌더 라벨·경고창·수석 매뉴얼·카고플랜 설명줄·미르 답변·검색 별칭·콘앱 콘 타이밍). ⇒ 화면에 나가는 말을 현장 말로 바꿨다 — «CASP 플랜(PDF)»→«선박 도면(PDF)» · «CASP 코드»→«선박 약자» · «카스피 양식 그대로»→«표준 도면 양식 그대로» · «카스피에서 읽을 수 있습니다»→«표준 형식이라 그대로 읽힙니다» · «카토스 배»→«컨별 기록이 오는 배»·«PCTC 배» · «터미널 앱(CATOS)»→«터미널 앱» · 콘앱 «카토스(PCTC) 실적»→«PCTC 터미널 실적». ⚠ **주석과 내부 데이터 필드는 남긴다** — 주석은 설계 근거(«카스피 도면이 그렇다»)라 지우면 다음 클로드가 그 양식을 어기고, `methodology:'CASP_DEF_…'`(베이사전 메타)·`meta.rcpt||'CASP'`(선사로 나가는 EDI 의 UNB 수신자 코드)는 **화면 글자가 아니라 데이터**다 — 수신자 코드를 바꾸면 상대가 못 읽는다. 연막검사 `smoke_termby` 가 주석을 걷어 낸 «코드가 쓰는 글자»만 훑어 네 낱말을 막는다(예외는 그 둘뿐).   // 3.27 **플랜 칸에서 목적지 글자가 화물 표기와 붙던 것 + 별첨이 빈 칸을 두고도 잘리던 것.** 검수사 2026-09-08 «셀안에 표기 문자 와 목적지 표기와의 어울림» · «잘림인지 겹칩인지 구분이 안감» · «최대한 한곳에 모으되 넘치거나 잘림 겹침이 생기면 빈곳을 이용해 보기 좋게 만든다» · «XTPG는 별첨 1.2 한칸 3은 두번쨰 칸 상단에» · 목적지 표기는 «1안». ①**칸** — 3.7-02 가 코드 배율 0.70 을 **ATPR 한 척**으로 정했고 연막검사도 그 한 척만 쟀다(`for (const ship of ['ATPR'])`). 실렌더 실측 — ATPR 여유 **+0.02px**(유일 통과) · STSE 2669E **−4.16** · SWBT 2614N **−3.34** · MCSC 633N 은 칸이 좁아 코드를 아예 접는다. 열×박스 32조합 중 안 닿는 것은 4개뿐이고 사전 40척 중 39척이 8열 이상이다. ⇒ **표기자만 왼쪽으로 비켜 세운다**(`cpv2-podsplit`) — 코드 자리(오른쪽 위)는 한 글자도 안 건드려 ★X-RAY(오른쪽 아래)와의 사이가 그대로다. 실측 SWBT 표기자 0~5.18 · 코드 8.93~11.95 로 **3.75px 이 빈다.** 두 글자 표기(DG·RF·RE·FR·OT·TK)만 **0.78배** — 코드를 그리는 **37조합 전수 실측**에서 가로 여유 최소 **+0.31px**(코드 한 글자 기준), 비 0.897. ⚠ 칸 네 구석은 이미 다 쓰인다(◆시프팅 좌상·▲긴급 좌하·★X-RAY 우하·목적지 우상) — ◆·▲ 가 있는 칸과 세 글자 표기 칸은 종전 가운데 그대로 둔다(옮기면 표식 위에 글자가 얹힌다). 양하는 코드를 안 그리므로 한 픽셀도 안 바뀐다. ②**별첨** — 3.7-05 문턱 `_fAll >= 7.0` 이 너무 낙관적이었다. XTPG 539E(선사 9 + 미상 1 = 20줄)가 7.2px 로 «읽을 만하다» 판정을 받아 **빈 칸을 바로 옆에 두고도** 셋을 한 칸에 몰아넣었고 별첨1 합계 줄이 잘렸다(검수사 실화면). **계산으로 미리 정하는 것을 폐기했다** — 한 칸으로 그려 보고 **줄이지 않은 크기로 넘칠 때만** 나눈다(검수사 «나눔건은 조건이 한칸에 표기 불가능할 경우만입니다» · «축소기능 사용 안하고 평상시대로 사용하면서»). 나눌 때는 «별첨1·2 한 칸 · 별첨3 둘째 칸 상단»이고, 선사가 줄어 한 칸에 들어가면 종전대로 셋이 한 칸이다. 실측 XTPG **7.5 → 9.5px**. ③**죽어 있던 안전망** — `fitLegend` 는 상자의 «첫 자식 하나»만 보고 넘침을 물었는데 그 자식이 `overflow:hidden` 이라 **안엣것이 잘려도 «다 들어갔다»로 읽혀 한 번도 발동한 적이 없었다.** 이제 상자와 별첨 칸(`cpv2-legend-slot`)을 전부 훑는다. ✅ 실데이터 파급 — 활성 8모드 전수 대조에서 바뀌는 곳은 **둘뿐**(XTPG_d 한 칸→두 칸·7.5→9.5px · STSE_d 나누는 짝이 1|2+3 → 1+2|3), 나머지 6모드 무변화. 연막검사 `smoke_podpat`·`smoke_legendfit` 에 항 추가(좌우 가르기·구석/왼쪽변 예외·코드 자리 유지·그려 보고 정하기·leg12·fitLegend 회귀·평상시 크기 판정 단위시험·양하 무변화). ★ 다른 클로드 감사를 **세 번** 받아 지적을 그 자리에서 수리했다 — ①두 글자 칸 배율: 0.72 로 줄였더니 코드(0.70)와 «거의 같은 크기»(3.7-01 이 되돌린 상태)가 됐고, 코드를 0.55 로 줄였더니 이번엔 **인쇄 문턱 3.0pt 아래**(SWBT 2614N 2.970pt)로 떨어졌다 ⇒ **코드 배율은 0.70 그대로 두고 표기자만 0.78** 로 정했다 — 그래야 게이트(`podLen` 3.0pt)와 CSS 가 한 벌이 된다(규범 §4-4). ②왼쪽 **변**에 이미 그려지는 수화물 띠(.cpv2-lugg)·폭 초과 막대(.cpv2-oog-W/HW) 칸을 예외에서 빠뜨린 것 → 예외에 더함(2.98-10 «굵은 테두리 때문에 FR 글자 구분이 안됩니다» 재발 방지). ③**별첨3 이 한 장에 두 번 그려지던 것**(2.56-03 의 «반쪽으로 옮긴다»를 3.7-03 이 반만 지운 잔재) — 실측 사전 35척 중 NBTD·NSFR·SWSP 셋. 검수사 «별첨 1, 2, 3은 중첩되지 않아야 하며 최대한 같이 있어야 합니다» 위반이라 이번 판에서 걷었고, 죽은 legendFont(별첨 글자 크기 두 벌)도 같이 치웠다. ④낡은 주석·판 기록 정정.   // 3.26 **부킹 자리(예상 EDI)와 실번호 리스트를 두 번 세던 것을 한 벌로 막는다.** 검수사 2026-09-07 «상습적인 구리스트 보관과 예상EDI 컨자리 넘버와 컨넘버 중복계산». 실측 SWBT 2614N 선적 — 자리 316 + 실번호 316 이 별첨 632·검증박스 316/316·목록 543 이었다. 판정 한 벌 utils.isSlotEntry(cn 빈 PRE 자리도 자리)·bookingFillOf/bookingFillOfSec(채움 상태)·dropFilledBookingSlots(목록·대수, 총수 기준)·legendItemsOf(별첨은 자리=계획, 채운 실번호 제외 — B안)를 세는 자리 12곳(목록·인쇄허브·별첨·검증박스·홈·수석 카드·현황요약·마감점검·검색 대수·미르 대수·수석 보드 별첨)이 부른다. ediContainers 는 손대지 않는다(감사 «부» — 자리 그림·가이드·보관소가 그것을 읽는다). 상세는 utils 3.26 머리말과 기록부 «2026-09-07 밤».   // ★ 3.25 **리퍼 온도 — 시작할 때 큰 화면으로 안 막는다.** 검수사 확정 2026-09-07 «큰화면으로 보이게 하면 또 바로 리스트 처리 할수 있습니다. 상단에 리퍼 검수여부를 알림으로 작게 보여주고 완료시 다시 한번 묻는걸로 대치 했으면 합니다». 자동 팝업을 없애고 항차 화면 맨 위 한 줄로 늘 보이게 했다 — 컨이 배 위에 있어 시작할 때는 잴 수가 없고, 길을 막으면 급하니 «리스트대로»를 누른다(2026-09-07 에 두 사람이 각각 52대·19대를 그렇게 굳혔다). 판정을 한 벌(`reeferTempOf`/`reeferTempSummary`)로 모아 열 곳의 서로 다른 셈을 없애고 상태를 셋으로 갈랐다 — A 기준없음 · B 검증안됨(사진 필요) · C 확인됨(세팅↔검사 차이 표). 「전부 리스트대로」는 「세팅온도 채우기」가 되어 **기준 칸만** 채우고, 기준이 없으면 안 눌린다. `rfSrc:'list'` 로 베낀 값은 실측으로 세지 않는다(화면·마감·브리핑·인계·CSV·종이 서류 전부). 「선원 리스트」는 「검수 수기 리스트」로(검수사 정정). 수석·부수석·테스터는 자동 로그아웃 대상에서 뺐다(«일반 검수원들만 로그아웃되게»). 연막검사 `tools/smoke_reefertemp.cjs` 20항.   // 3.24 **미르가 «시간당 몇 개»를 알아듣고, 페이스 분모가 «검수 시작»이 된다.** 검수사 2026-09-07 «미르는 오답을 주었습니다 … 시간별 몇대를 했냐고 물었는데 엉뚱한 대답을 했습니다» · «작업 시작 시간은 검수 시작이 시작입니다». ①**시간당 인텐트가 없었다** — 페이스 계산은 3.6-01 부터 있었는데 «몇 시에 끝나?»로만 불렀고, «시간당 몇개 했어»는 대수 질문으로 떨어져 «작업한 대수 260/260»을 답했다. `paceQuery` 신설(시간당·시간별·시간마다·속도·페이스) + `formatPace` — 페이스 한 벌로 답하고 **시간대별 분포**를 작업 순서대로 펼친다(자정 넘김 정렬). ②**페이스 분모가 접안시각이었다** — `workWindowOf` 가 `atbActual`(ATPR 00:00)을 시작으로 써서 «시작 3시간 전에 66대를 내린 것»이 됐고 시간당이 부풀었다. 검수원이 「작업 시작」을 누르면 `reports` 에 `discharge_start` 로 남는데(실측 20:45:33 — 검수사 카톡 보고와 초까지 같다) 그것을 한 번도 안 봤다. `voyageReportSpan` 신설 → `workWindowOf` 가 **맨 먼저** 본다. 보고가 없으면 종전 폴백. 실측 ATPR 2640E — **94.5대/시간 → 50.2대/시간**(분모 165분 → 311분 · 20:45:33~03:26:36). 통계탭·미르가 같은 한 벌을 쓴다(§4-4).   // 3.23 **베이를 골라 놓았으면 끝 4자리가 겹쳐도 그 베이 것으로 카드가 선다.** 검수사 2026-09-07 05:40 «베이를 지정해서 양하중입니다. 그러면 리퍼 3290만 보였어야 합니다» · «베이를 지정하지 않았더라면 중복이 많겠지만 베이를 지정한 상태라면 그베이껏이 우선이 되어야 합니다» · «자동 가이드 문제가 아닙니다. 수동양하도 마찬가지 입니다. 항상 베이를 선택하고 양하 합니다». 실측 ATPR 2640E — «3290» 이 FBIU5373290(14-08-82 리퍼)·SKHU8933290(06-04-86) 둘인데 검수사는 B14 를 골라 두고 있었다. 그런데 화면은 «자리를 확인하고 고르십시오 / 전체 번호를 치면 바로 확인 카드가 뜹니다» 로 11자리를 요구했다 — 4자리로 끝나는 빠름이 사라진다. ⇒ 3.3-01 의 금지(작업분 둘 이상이면 큰 카드 없음)를 **고른 베이·단에 정확히 하나일 때만** 연다. 그 사고(KSKM 7075 — 하나를 완료하면 남은 하나가 큰 카드로 승격)는 «고른 베이 안에 둘»일 때 나므로 이 문으로는 안 난다. 베이를 안 골랐으면 종전대로 나란히 보인다. 가려낸 나머지는 접힌 목록에 그대로 남고 카드 위에 «다른 N대는 다른 자리»를 밝힌다. 판정(`inManualTier`)은 `results` 정렬과 카드가 **같은 한 벌**을 쓴다(규범 §4-4).   // 3.22 **실시간 작업 보드가 동방 배를 빠뜨리던 것.** 검수사 2026-09-07 00:30 «수석대쉬보드 실시간 작업현황에 왜 ATPR작업현황이 안보이죠 3척이 다보여야 하는데». 동방(PNCT)은 배정목록 상태를 늦게 바꾼다 — ATPR 2640E 는 터미널 실적 149대가 들어와 있는데 `terminalStatus` 가 여태 `planned` 이고 작업시작일시도 빈칸이라 `isWorkingNow` 가 첫 줄에서 잘라 냈다(보드에 두 척만). 같은 뿌리를 어제 **수집기에서만** 고쳤고(MailPilot 2.29) 검수앱 게이트는 그대로였다. ⚠ 두 곳은 언어가 달라 한 벌이 못 된다 — 수집기(`pnctpull.py:495`)는 `_done>0` 하나만 보고, 여기는 문지기 넷을 다 본다. 터미널이 호기별로 적어 준 `info.qcWork`(disDone/lodDone)가 있으면 작업 중으로 본다. `terminalStatus`·`workStartAt` 은 한 글자도 안 덮는다(추측으로 채우지 않는다). 실적 칸에는 시각이 없어 과거에도 참이 되므로 **계획 시작 시각**을 함께 본다. 떠난 배(departed/done)·터미널이 «남은 게 없다»고 적은 배(qcRest 0)는 뺀다. 떠난 배·터미널이 «남았다»를 0 으로 적은 배·계획 끝을 반나절 넘긴 배는 열지 않는다(감사가 짚은 유령 구멍). 실데이터 활성 17척 전수 — 바뀌는 것은 ATPR 하나뿐(허수 0), 라이브 재현 3척(ATPR·MCAP·DJCF). 전수 회귀 20척×14시각 280판정 통과.   // 3.21 **미르가 「선미 ○○ 선수 ○○」 를 알아듣는다.** 검수사 2026-09-06 19:09 «야간근무 DJCF 선미 김판석 선수 이종부» 가 **통째로 사라졌다** — `parseCraneCrew` 첫 줄이 «호기» 라는 글자가 없으면 등록 문장으로 아예 안 봤고, 못 알아들었다는 표시(`mir_misses`)조차 안 남겼다(조용한 실패 — 규범 §4-3). 검수사가 «1호기 김판석 2호기 이종부» 로 고쳐 말해 넘어갔지만, **그 번호가 실제와 어긋났다** — 그날 야간 실적은 DJCF 가 GC103·GC104, MCAP 이 GC101·GC102 였는데 검수사는 DJCF 를 «1·2호기», MCAP 을 «3·4호기» 라 부르셨다(정확히 역순). 검수사 «그건 제 실수 입니다. 저도 실체를 안보고 경험으로 봐서 그렇습니다. **그래서 선미 선수로만 이야기 했던거고요**» · «**처음에 호기를 이야기 안한 이유입니다**». ⇒ **선수·선미가 흔들리지 않는 축**이다. ⓵`craneBowSternOf(voyage)` 신설 — 검수사 «양하작업이 어떻게 이뤄졌는지 보면 어디가 선수 선미인지 압니다» 그대로, 호기마다 찍은 베이의 **가운데값**으로 가른다(작은 번호 = 선수쪽 — cargoPlanCore 머리말). 평균이 아니라 가운데값이다(한 대가 남의 베이를 도우면 평균이 끌린다). ⓶`parseCraneCrew` 가 «선수·선미» 를 받는다 — 어순 두 꼴을 **아는 이름이 몇 번 걸리는지로** 가른다(«김판석 선미 이종부 선수» 에서 쪽-먼저 그물이 «선미 이종부» 를 물어 김판석이 사라졌다). ⓷`resolveCrewSides(cs, voyage)` 한 벌 — 저장하는 자리가 넷이라(검색 패널·항차 화면 둘·전체 검색) 각자 유도하면 갈린다(§4-4). ⓸**못 가리면 되묻는다** — «이 배는 아직 작업 기록이 없어 어느 호기가 선수·선미인지 못 가려요» + 가렸으면 «선수 N호기 · 선미 N호기 — 지금까지 내려놓은 베이로 가렸어요» 로 근거를 밝힌다. ✅ 실데이터(그날 야간 실적) — DJCF 선수 3호기·선미 4호기(가운데값 22/34) · MCAP 선수 1호기·선미 2호기(22/34). 검수사 첫 말 «선미 김판석 선수 이종부» 가 그대로 재현된다. 연막검사 `smoke_bowstern` **22항**(실데이터 픽스처 · 확신 없으면 null 다섯 가지 · 어순 두 꼴 · 되묻기 · 3.8 호기 꼴 회귀). ★ 같은 날 RTDB 정정 — 어긋나 등록된 09-06 야간 조를 실적에 맞게 고쳤다(DJCF 3호기 이종부·4호기 김판석 / MCAP 1호기 최원형·2호기 최관식, D급 백업 `_claude/backup/*_craneCrew_bak_20260906_1922.json` · 되읽기 확인).   // 3.20-05 **양하 리스트에 컨번호 없는 한 줄이 서서 «미완 1» 로 남던 것.** 검수사 2026-09-06 실화면 «전체 231 · 미완 1 · 완료 230» · «26-01-88 · 실:미입력» · «선적 시프팅 리스트에 옮기셨으면 양하 리스트에서 지워 주셔야 됩니다» · «미완1이 남습니다». 정본(RTDB)은 양하 EDI 230 = 리스트 230 = 완료 230 으로 이미 깨끗했는데 화면만 231 이었다 — 뿌리는 3.7-08 `applyCatosPos` 다. 터미널 실적에는 **시프팅(재적부)이 같이 온다**(실측 MCAP 634N 실적 326건 중 96건이 양하 리스트 밖). 그런데 그 함수가 `recs[cn] = { ...(recs[cn] || {}), bay_actual … }` 로 **리스트에 없는 컨에도 행을 새로 만들었고**, 새 행에는 `cn` 이 없었다. 화면(VoyagePage containersBase)은 `merged[r.cn]` 으로 담으므로 그 96 건이 전부 **`merged[undefined]` 한 칸**에 덮어써지고, 마지막 한 건(26-01-88 · UETU8177334 자리)만 살아남아 **컨번호 없는 유령 한 줄**이 섰다. 규격·풀엠티도 없어 «⚠기타 ISO · ?미정» 으로 보였고, 완료 도장은 컨번호로 찍히니 그 줄은 **영영 미완**이었다. 같은 뿌리로 머리글 «총 작업 422대 = 리스트 326 + 쉬프팅 96» 이 시프팅을 두 번 셌고, «리스트에 있는데 EDI 없음 96대» 라는 거짓 경고도 떴다. ⇒ **리스트에도 EDI 에도 없는 컨은 행을 만들지 않는다**(자리는 termWork 원본에 그대로 있고, 시프팅은 제 경로가 계획 좌표로 그린다) + 만드는 행에는 **`cn` 을 반드시 박는다**(EDI 에는 있는데 리스트가 아직 안 온 배에서 여기서 행이 처음 생긴다). ✅ 실데이터 — MCAP 634N 목록 327 → 326 · 전체 231 → **230** · 미완 1 → **0** · 리스트 326 → 230 · 컨번호 없는 행 96 → 0. 활성 **17항차 34모드 전수 대조** — 바뀌는 곳은 이 한 항차뿐이고 **리스트·EDI 에 있는 컨의 자리 손실 0건**(3.7-08 기능 무손실). 연막검사 `smoke_catospos` 에 이 사건을 박았다 — 3.7-08 이 못 박았던 «EDI 에 없는 컨도 실적은 남긴다» 항을 **뒤집고** 이유를 적었다. ★ **문지기를 `applyCatosPos` 에 세운 이유**(감사가 실측으로 확인) — `bay_actual` 을 읽는 소스가 22개이고, 그중 `loadingEdiExport.collectActualLoading` 은 records 를 **키로** 돌아 3.20-04 에서는 리스트 밖 실적 3건이 **선사로 나가는 선적 EDI 회신에 «실렸다»고 올라갔다**(회신 58 → 61행). 화면 쪽에 세웠으면 이 길은 그대로 열려 있었다(규범 §4-4 — 문지기는 데이터가 들어오는 자리에). ⚠ **`computeTermApply` 에는 같은 게이트를 두지 않았다 — 없는 것이 맞다.** 리스트 밖 96건에도 `completed/{컨}` 을 쓰지만 그것이 곧 시프팅 모브 수다(`progressOf` 수정 전후 바이트 동일 — moves 96 · done 326). 다음 감사가 «여기도 막아야 한다»고 고치면 모브가 통째로 사라진다. ⚠ **검수사 원문 둘이 부딪힌다** — 1.76-05 «TCLU9762509는 앱에서 양하처리 되어야 합니다»(시프팅을 검수 리스트에 올림) 대 오늘 «선적 시프팅 리스트에 옮기셨으면 양하 리스트에서 지워 주셔야 됩니다». 이 판은 **유령 한 줄만** 지웠고 시프팅 96건은 종전대로 「◆ 시프팅 96/96」 별도 칸에 남는다(총계에는 안 섞임 — 1.76-05 확정). 어느 쪽인지 검수사에게 확인받을 것. ✅ 감사(다른 클로드) «가» — 지적 하나(`boardBaysOf` 의 조용한 탈락)는 같은 판에서 수리했다.   // 3.20-04 **수석 보드에 「⤢ 크게」 단추 — 트윈 작업을 크게 본다.** 검수사 2026-09-06 «트윈 작업할때 크게 보고 싶은데 … 선박을 클릭하면 전체화면으로 잘 볼수있고 닫으면 전체선박을 볼수있게 해달라고 했는데 그것만 안되었습니다» · «크게 하는 버튼이 없고 닫는 버튼도 없습니다». 3.10 이 포커스를 «카드를 아무 데나 누르면» 으로만 만들었는데, 카드 안은 별첨 표와 베이 그림으로 꽉 차 있고 **그림 칸을 누르면 컨 상세가 뜬다**(3.11) — 누를 빈 곳이 사실상 없어 포커스가 한 번도 안 걸렸고, 「✕ 닫기」 는 포커스일 때만 뜨니 **영영 안 보였다.** 기능은 있는데 들어가는 문이 없었다. ⇒ 「항차 열기 →」 옆에 **늘 보이는** 「⤢ 크게」(배가 둘 이상일 때). 누르면 그 배만 화면 가득, 그 자리가 「✕ 닫기」 로 바뀐다 — 검수사 «두척이니 작게 보이고 원할때는 한척 작업하는것처럼 크게 보고 닫음 처음처럼 두척이 보이게». ★ 그리고 **카드를 눌러 항차 화면으로 넘어가지 않는다** — 검수사 «가고 싶을때 가야 하는데 클릭한번 실수로» · «수석에서 일반 검수앱을 왔다 갔다 하면 안됩니다». 3.10 의 onOpen 폴백을 걷었다. 카드 클릭은 크게 보기 하나이고, 항차로 가는 길은 [항차 열기 →] 버튼뿐이다.   // 3.20-03 **콘앱 2.36 — 폰을 눕히면 머리를 줄인다**(검수앱 화면은 안 바뀐다 · 콘앱 라벨 동기화용 판 올림). 검수사 2026-09-06 «폰에선 콘계산기 헤더가 자리를 너무 많이 차지합이다 1/4 정도». 머리는 77px 고정인데 폰을 눕히면 화면 높이가 812 → 375 가 되어 같은 77px 이 10% 에서 **21%** 로 커진다(브라우저 실측). 콘 작업 중에는 눕혀서 그림을 보므로(검수사 실화면) 낮은 화면(`max-height:520px`)에서만 로고·글자·여백을 줄이고 부제를 감춘다 — **77px → 30px(8%)**, 화면의 13% 를 그림에 돌려준다. 세로는 종전 그대로다.   // 3.20-02 **콘앱 2.35 — 홀드도 그림을 보여준다**(검수앱 화면은 안 바뀐다 · 콘앱 라벨 동기화용 판 올림). 검수사 2026-09-06 «콘앱을 보니 홀드는 그림을 보여주지 않습니다. 몇몇 선박은 40피트도 홀드 콘 작업을 합니다. 복합선 같은 선박 그래서 홀드도 보여 줘야 합니다» · «그냥 보통선박처럼 그려주시면 됍니다. 이유는 인력들이 선박들을 다알기 때문에 홀드 진행상황을 볼수 있으면 됩니다» · «어디를 진행중인지 글자로는 눈에 잘 안들어 오기때문입니다» · «데크와 홀드는 열 폭이 다릅니다 … 이거 맞추는법 알고 있을것입니다. 많은 시간을 베이구조에 투자했었기 때문입니다». 2.33 이 문 셋으로 홀드를 버리고 있었다 — ①홀드에서 작업 중이면 그림을 통째로 안 그림 ②`tier>=80` 체가 홀드 컨을 안 담음 ③`filter(t>=c.t1)` 이 남은 홀드 단을 또 자름. 그래서 2.29 가 넣은 «홀드 1단(02)» 라벨은 **화면에 한 번도 뜬 적이 없었다**(죽은 코드). ⇒ 셋을 열고, **열은 베이사전 골격(`bvBaySkeleton` 의 `deckRowPos`·`holdRowPos`)에서** 가져와 데크와 홀드가 각자 제 폭을 쓴다(실측 MCAP 베이 22 — 데크 12열·홀드 10열, 34베이 중 30베이가 다르다). 줄마다 가운데 정렬하고 갑판·선창 사이에 얇은 금 하나. ⚠ **콘 개수 판정(선종)은 안 건드렸다** — 검수사 «2는 선종구분을 안하겠다는것이니 질문자체도 필요 없습니다». 한때 «선종을 골라 주세요» 문구를 넣었다가 그 지시로 빼냈다. 이 판은 **그림만**이다. ✅ 연막검사 `tools/smoke_coneholdview.cjs` 14항 — 소스에서 그림 블록·골격 함수를 그대로 꺼내 실데이터(MCAP 634N 양하 베이 22·77행)로 돌린다.   // 3.20-01 **수석 카드에서 「🏗 터미널」 줄을 없애고, 보드가 모니터 한 화면을 쓰게 했다.** 검수사 2026-09-06 실화면 «🏗 터미널 지연 ←????» → «없애주세요» · «그거 없애면서 작업중인 작업 보드가 모니터 한 화면에 차 보였으면 좋겠습니다» · «크롬하고 엣지하고 틀리게 보입니다. 크롬은 그나마 차보이는데 엣지는 많이 비어 보입니다». ①3.20 이 대수를 빼자 꼬리(「지연」·«← 방금»)만 남아 뜻이 안 통했고 카드마다 한 줄을 그냥 먹었다 — 줄을 통째로 없앴다. 조용히 사라지는 것이 아니다(규범 §4-3) — 터미널이 멎은 것은 보드 머리의 수집기 경고(«수집기가 멈춰 …»)와 healthcheck 가 말한다. ②**브라우저마다 달리 보인 뿌리는 배율이 아니라 그릇이었다.** 보드 높이는 «화면 바닥 − 보드 위 끝» 실측인데(3.11-01) **재는 때가 창 크기 변화뿐**이라, 아래 접이식 섹션을 펴거나 페이지를 스크롤해 보드 위치가 바뀌면 옛 값에 굳었다. 실화면 실측 — 같은 2척인데 크롬은 화면을 거의 다 쓰고 엣지는 **440px 에 굳어 위 카드가 잘렸다.** ⇒ 스크롤·문서 크기 변화에도 다시 잰다(scroll + ResizeObserver(document.body)), 보드 머리가 화면 위로 나가면 top 을 0 으로 눌러 화면보다 큰 그릇을 안 만든다. 그릇이 커지면 그림 배율은 FitBox 가 저절로 따라 키운다 — 좌우 잘림 없이 채워진다. 매뉴얼 4줄 정합.   // 3.20 **시프팅 대수가 작업 중에 스스로 움직이던 것 + 수석 화면에서 터미널 대수 빼기.** 검수사 2026-09-06 «시프팅 갯수가 스스로 줄어들었습니다. 96에서 94로» → «시프팅은 96이 맞을것입니다». 실측 MCAP 634N — 카토스 자리가 한 대씩 들어올 때마다 «동형 공컨 순열» 필터의 고리가 깨졌다 이어졌다 하며 96 → 94 로 내려갔다(셋이 빠지고 하나가 들었다). 뿌리는 3.7-08 `applyCatosPos` 가 구독에서 얹은 실적 자리를 시프팅 재료로 같이 쓴 것 — 일부는 실적·나머지는 계획이라 섞였다. 시프팅은 «작업 전에 몇 대를 들어야 하나»를 보는 숫자라 작업 중에 값이 움직이면 못 쓴다. ⇒ `_mergeRecPos` 가 **터미널이 채운 자리를 건너뛴다** — 계획 + **사람이 손으로 고친 자리**로만 센다(2.96 «검수원이 자리를 고치면 다시 계산한다»는 그대로다). ★ **문은 두 짝이다**(감사 «부» 지적, 다른 클로드) — 실체(`_pos_src`)만 막았더니 3.13 `applyAutoSwap` 이 구독에서 바로 다음에 돌며 터미널 자리에서 유도한 기계 배정을 `bay_assign`(assign_by 'auto')으로 넣어 **옆문이 그대로 열려 있었다**. 실측 재현 — 카토스가 MRKU4002140 을 남의 계획칸(41-11-84)에 실었다고 말하자 95 → **66**(신고된 96→94 보다 크다). ⇒ assign 문도 «사람이 정한 것»만 통과시킨다. 덤으로 ①`stripCatosPos` 가 'catos' 만 벗기고 3.16-01 이 더한 'pnct' 를 안 벗기던 것 ②사람이 자리를 고쳐도 `_pos_src` 표식이 남아 다음부터 «터미널 자리»로 오인되던 것(`fbSetActualPosition` 이 걷는다)도 같이 고쳤다. 실측 MCAP 634N 96 = 96 · DJCF 0150N 0 = 0 · 연막검사 `smoke_shiftberth` [8] 은 **구독 경로(applyCatosPos→applyAutoSwap)를 태워** 재고, **문 둘을 각각** 잰다 — 실체 문을 빼면 95 → 65, 배정 문을 빼면 95 → 66 으로 그 자리에서 걸린다. 덤으로 컨 상세의 기록자 표기도 'catos' 만 보던 탓에 **동방 배에서 조용히 빈칸**이던 것을 고쳤다. 같은 판에 **수석 카드의 터미널 대수 대조를 뺐다** — 검수사 «카토스(앱) 카운트는 놔두고 터미널 카운트는 이제 없어도 될듯합니다». 컨별 터미널 실적이 앱 완료로 들어오므로 진행 막대가 곧 그 숫자다(대조가 두 벌). 지연 배지·자료 시각·미수신 표시는 남긴다. ⚠ 검수사 확정 후속 — «양하가 끝나면 시프팅 대상인데 안내린 컨이 생길 것입니다. 그때 맞바꿈 해야 합니다»는 별도 판(관문 4 대기).   // 3.19-03 **콘앱 2.34 — 위 고정 띠를 한 줄로, 상자만 절반**(검수앱 화면은 안 바뀐다)   // 3.19-02 **확대하면 짧은 베이가 유독 잘리던 것 + 콘앱 2.33-02.** 검수사 «3개는 잘나오는데 홀드가 없는 베이는 많이 짤립니다» — 3.17 이 확대 시 칸 높이를 «제 그림 높이 ÷ 배율»로 잡아, 홀드 없는 짧은 베이는 그림이 짧아 상자까지 작아지고 확대분이 거의 다 잘렸다(실화면 4호기 베이38). ⇒ 확대하면 칸이 **줄 높이를 다 쓴다**(칸마다 같은 높이 · 안에서 스크롤), 확대 안 했으면 종전대로 그림에 딱 맞춘다(밑에 여백 없음).   // 3.19-01 **콘앱 2.33-01 — 쌓은 줄이 한 덩어리로 밀린다**(검수앱 화면은 안 바뀐다)   // 3.19 **수석 보드 — 호기 칸 균등 크기 · 확대 시 가운데 · 확대 100~300%.** 검수사 2026-09-06 «이화면만 균등크기로 조정해주고 확대시 중앙정렬되게 해주시고 확대는 100-300%로 해주세요» · «수석화면 입니다». ①**균등** — 칸마다 제 그림에 맞춰 배율을 따로 잡아 그림 크기가 제각각이었다(실화면 넷 중 4호기만 크게 보였다). 각 칸이 «혼자면 이 배율»을 알려 주고(`FitBox onFit`) 카드가 그중 **가장 작은 것**을 모두에 되돌려 준다(`force`). ②**가운데** — 3.17 은 확대하면 왼쪽에 붙였다(`x=0`). 이제 칸보다 좁으면 여백을 반씩 나누고, 넓으면 **스크롤을 가운데로** 민다. ③**100~300%** — 1·1.25·1.5·1.75·2·2.25·2.5·3 여덟 단. 줄이는 쪽(0.8·0.9)은 없앴다 — 작아서 생긴 문제였다(기기에 남은 옛 값은 100%로 돌아온다).   // 3.18-02 **콘앱 2.33 — 줄 그림을 단별로 쌓는다 + 베이 그림 보기 전용**(검수앱 화면은 안 바뀐다)   // 3.18-01 **콘앱 2.32-01 — 콘앱의 베이 그림은 보기 전용**(검수앱 화면은 안 바뀐다)   // 3.18 **콘앱이 수석 보드와 같은 베이 그림을 쓴다** — `coneCargoPlan.entry` 가 `window.ConeBayPlan.mount`(검수앱 `BayPlan` 컴포넌트 그대로)를 내보낸다. 검수사 2026-09-06 «콘앱의 베이플랜을 가져오면 실시간으로 볼수가 없습니다. 그리고 콘앱의 베이플랜은 양하와 선적이 같이 있습니다» — 콘앱 2.32 가 그것을 쓴다. 검수앱 화면 자체는 안 바뀐다.   // 3.17-04 **콘앱 2.31-02 — 카드 안 베이 그림의 헛도는 스크롤 제거**(검수앱 자체는 안 바뀐다)   // 3.17-03 **콘앱 2.31-01 — 카드 안 베이 그림 크기 맞춤**(검수앱 자체는 안 바뀐다)   // 3.17-02 **콘앱 2.31 — 콘 타이밍 카드 안에 그 베이 그림**(검수앱 자체는 안 바뀐다 · 콘앱 라벨 동기화용 판 올림)   // 3.17-01 **콘앱 2.30 — 베이플랜이 실제 실린 자리로**(검수앱 자체는 안 바뀐다 · 콘앱 라벨 동기화용 판 올림). 실측 DJCT 0223E — 계획과 다른 자리 28대   // 3.17 **실시간 작업 보드 확대.** 검수사 2026-09-06 «작업이 2척이다 보니 수석 화면이 작아 졌습니다. 확대기능 가능한지요?». 보드는 높이를 척수(최대 4)로 등분하므로 2척이면 그림이 절반이 된다 — 3.10 이 «최대 4척»을 위해 그렇게 짠 것이고, 실제로 두 척이 붙자 바로 걸렸다. ⇒ 보드 머리에 **확대 단추**(− · 배율 · +) — 0.8·0.9·1·1.15·1.3·1.5·1.75·2 여덟 단, 배율을 누르면 기본으로. 맞춤 배율(FitBox)에 그 값을 곱하고, 1보다 크면 **그 칸 안에서만 스크롤**한다(보드 전체가 밀리지 않게). 기기에 기억한다(localStorage `gm_board_zoom` — 읽기·쓰기 모두 try, 사생활 모드에서 던진다). 왼쪽 별첨·오른쪽 그림 둘 다 같이 커진다.   // 3.16-02 **콘앱 2.29 — 단 표기(홀드 1단(02) · 데크 1단(82))**. 검수앱 자체는 안 바뀐다(콘앱 라벨 동기화용 판 올림)   // 3.16-01 **동방 자리를 «카토스»라 적지 않는다 + 콘앱 2.28.** 수집기 2.26 이 동방 컨별 선내위치를 받아 오면서 «동방 termWork 에는 자리가 없다»는 전제가 깨졌다(다른 클로드 감사가 잡았다). ①`applyCatosPos` 의 출처 표식을 원천대로(`_pos_src` pnct/catos) ②콘앱 2.28 — 자리는 오는데 호기가 안 오므로 `ctEquip('')` 로 두면 갱 둘이 한 칸으로 뭉치고 검수원 이름 갈래가 건너뛰어진다 ⇒ 호기가 없으면 **그 해치를 칸 이름으로**(«베이 18») · 원천 표기를 «동방 터미널 실적 + 검수앱 기록»으로 ③낡은 주석 정정   // 3.16 **«터미널(CATOS)» 글자를 화면·서류 어디에도 안 내보낸다.** 검수사 2026-09-06 «by:'터미널(CATOS)' 어디든 이 문구는 없어야 됩니다». 3.9 는 X-RAY 봉인자만 막았고 CSV 검수자 열·컨 목록·컨 상세·보고 탭·통계 탭·수석 실시간 표·활동 로그·미르 음성 **열 곳**이 무필터였다. 게다가 동방(PNCT)은 카토스가 아닌데도 그 글자가 붙었다(실측 OBWH 2731E 470대 전부). ⇒ ①쓰기 — `computeTermApply` 가 `by` 를 비우고 `src:'term'` 표식만 남긴다(`applyCatosPos` 의 `actual_by` 도 비운다). ②판정 한 벌 `isTermApplied` — 새 표식과 옛 글자를 둘 다 알아본다(보관소 수천 행은 `completed/{컨}` 이라 덮지 않는다 — 규범 §5-1). ③표시 한 벌 `completedByLabel` — 사람이 찍었으면 그 이름 → 터미널 반영이면 **그 시각·그 호기의 조 등록 근무자**(3.9 X-RAY 봉인자와 같은 규칙) → 없으면 «터미널 반영». 업체 이름(카토스·동방)은 어느 경우에도 안 나간다. ④열 곳 전부 그 한 벌로 배선 + 토스트·툴팁·매뉴얼 문구 정정 + 콘앱 사본(`ctIsTerm`). ✅ 연막검사 `tools/smoke_termby.cjs` 21항 — 소스 전수 스캔(주석 제외)으로 그 글자가 남은 곳 0 확인   // 3.15 **동방(PNCT) 배도 «지금 작업 중인 베이» 그림.** 검수사 2026-09-06 «그림도 보여주세요. 양하는 맞게 내려오고 있고 선적도 계획된 선적이면 맞게 보일테이고 검수앱을 사용하면 맞게보이니까요» · «오늘 ATPR로 테스트 할것입니다. 동방자료로만 살펴보고 다음 선박은 앱과 동방자료 그다음은 앱으로만». 3.10~3.14 는 동방 칸에 «컨별 자리가 안 와서 그림이 없습니다»라고만 적었다 — 동방 적하목록에는 컨별 자리·호기가 없어(수집기 `pnctpull.to_termwork` 가 pos·equip 을 안 넣는다) 호기와 베이를 이을 끈이 없었기 때문이다. ⇒ **자리는 계획(EDI)이 안다.** `utils.boardBaysOf` 신설 — 방금 완료된 컨들의 계획 자리를 해치(짝수+뒤홀수)로 묶어 많이 찍힌 순으로 갱 수만큼 낸다. 실적 자리(`bay_actual`)가 있으면 그것이 먼저다(검수앱으로 찍은 것이 계획을 이긴다). 창의 기준은 «지금»이 아니라 **가장 늦은 완료 시각**이다(쉬는 시간·자료 지연에 그림이 사라지면 안 된다). 30분에 없으면 1·2시간·전체로 넓힌다. 화면 — 그런 배는 오른쪽 칸 제목이 «호기별 작업 베이»에서 **«지금 작업 중인 베이»**로 바뀌고 칸이 호기가 아니라 베이로 선다(호기별 대수 QC 합계는 머리줄에 남는다). 게이트는 «동방이냐»가 아니라 «호기 자리가 하나도 없느냐»다(규범 §4-2). 호기 자료(QC)가 아직 없어도 그린다 — 동방은 완료가 먼저 오고 QC 합계가 늦다. ✅ 실측 OBWH 2731E(동방 470대) — 30분 창이 갱 두 대만큼 또렷이 갈렸다(16:40 (18)19×16·(10)11×16). 계획 자리 못 찾은 완료 0/470. 렌더 PASS(베이 그림 4장 · 완료 초록 · 끝4자리 128칸). 연막검사 `tools/smoke_boardbays.cjs`(21항) · `tools/smoke_liveboard_pnct.cjs`(jsdom 실렌더)   // 3.14 **선사 목록이 데려온 통과화물을 선적분에서 뺀다 + 동진 오픈탑 규격.** 검수사 2026-09-06 «DJCF 선적에 이상한점이 많습니다 … 26개의 컨테이너가 정말 선적분인지와 20여개의 선적위치가 이상한곳에 있습니다» → 분석 보고 → «EDI가 조금전에 들어 온것이니 그게 맞을거라는 저의 판단입니다. 수정바랍니다» · «EDI 없는 컨넘버도 삭제 바랍니다». 실측 DJCF 0150N 선적(0151S) — 앱 388대인데 배정목록·EDI·합본은 362대. 초과 26 = 선사 CLL(배 전체 목록)이 데려온 **통과화물 24**(인천·방콕·사이공 적재 → 부산 양하) + 구판 파일의 캔슬 2. 리스트 등재라는 이유로 선적분이 되어 EDI 가 준 «남의 짐 자리»(베이 5·7·10·17·19·25·27 — 평택 작업 없는 곳)에 21대가 그려졌다. 합본은 «[제외분(EDI밖)] 24건»으로 이미 걸렀고 등록(autoreg)만 안 걸렀다. ⇒ ①`utils.isPtk` 한 벌 — 선적에서 **EDI 가 «남의 항구 적재 → 남의 항구 양하»라고 말하고 자리까지 준 컨**(`isTransitByEdi`)은 리스트 등재라도 선적분이 아니다. 근거 셋(POL 남·POD 남·EDI 자리)이 다 맞을 때만 뺀다 — 엠티 선적 리스트는 항구 칸이 목적지로 오염되지만 EDI 에 없어 자리가 없다(M6.94.29 285대 사건 회귀 가드). `getContainerColorKey`·`legendLiveOf` 도 이 한 벌로. ②규격 — 동진 EDI 의 오픈탑 `225E`·`435E` 를 20OT·40OT 로(선사 리스트는 같은 컨을 `22UE`·`42UE` = ISO 6346 U 로 적었고 EDI 자체 OOG 플래그도 있다). 실측 35대(20ft 24·40ft 11). ✅ 활성 18항차 36모드 전수 대조 — 바뀌는 곳은 DJCF 선적 하나뿐(388→364), 나머지 무변화. 규격도 그 35대만. 연막검사 `tools/smoke_ptk.cjs`(실데이터 사본 13항) · 뿌리는 MailPilot 2.25   // 3.13 **밀려난 계획 컨은 비운 자리로 자동 맞교환** — 검수사 확정 2026-09-06 «속성 같으면 비운 자리로 자동 맞교환» · «지금처럼 검은곳이나 흰곳이 있으면 안됩니다. 그리고 컨테이너가 겹쳐 있어도 안됩니다» · «이유는 화면상에서 남은 컨 넘버를 확인 할수가 있어야 해서 입니다». 실린 컨 A 가 B 의 계획 자리에 앉으면 B 는 A 가 비운 계획 자리로(사슬이면 사슬 끝 빈 자리) — `utils.applyAutoSwap`(구독에서 applyCatosPos 다음, 메모리만 · `bay_assign`/assign_by 'auto' · 보관 직전 strip). 속성(iso·F/E·POL·POD) 다르면 같은 자리로 보내되 ⚠(`_assign_warn`, 주황 테두리). 빈 자리를 못 찾으면 제 칸에 ⊕ 로 남긴다(숨기지 않음). 베이플랜 탭·보드 그림은 «정해 준 자리»(assign)를 실체 다음으로 승격해 그린다(하늘색 테두리 ↔). 실측 DJCT 0224W 선적 — 겹침 2칸·구멍 27칸 → 0. 연막검사 `smoke_liveboard`   // 3.12-01 **한 칸에 둘이면 실린 컨이 앞** — 검수사 2026-09-06 00:0x «아직도 자리 버그가 있습니다 지금 선적중 3개째 보는데 방법이 없을까요?» · «이베이 홀드에서만 3개째인듯». 실측 DJCT 0224W 선적 60대 중 27대가 계획과 다른 자리에 실렸고(같은 해치 안 20ft 자리 바꿔 싣기), 그중 2칸(19-06-04·21-05-04)은 «실린 컨 + 아직 안 실린 계획 컨»이 한 칸에 겹쳤다. BayPlan 이 칸의 첫 컨(계획 컨)을 그려 실린 컨이 ⊕ 뒤에 숨었다 → 완료·실적 자리 있는 컨을 앞으로(BayPlan cellList 정렬 한 줄). 2.95 «이름만 걸린 컨은 밀어내지 않는다» 그대로 — 그림의 앞자리만   // 3.12 **선적 별첨4 «목적지별(POD)»** — 검수사 2026-09-05 «통계화면에서 선적은 포트도 표기 해야 한다는걸 전달 못드렸습니다» · «선적은 표4개가 필요합니다 양하처럼 + 포트별». 보드 왼쪽 선적 별첨이 양하처럼 선사별·화물 종류별·규격별 F/E 셋에 **별첨4 목적지별(POD, 완료/전체)**을 더한다(`utils.legendLiveOf` pods — 인쇄 별첨 POD 표와 같은 셈, PTK 제외). 실측 DJCT 0224W 선적 274대 = HPH 274   // 3.11-01 **보드가 화면 바닥까지 · 그림이 잘리지 않게** — 검수사 실화면 «밑에 다른 항목만큼 화면이 줄어 들었습니다». ①보드 높이를 고정 9rem 추정에서 «보드 위 끝~화면 바닥» 실측으로 ②FitBox 의 ×1.05³(미리보기에서 정한 값)이 실제 줄에선 높이를 넘어 밑 두 단이 잘렸다 → 칸에 딱 맞춤 ③호기 칸 여백 최소   // 3.11 **실시간 작업 보드 — 호기별 «보이는 베이» 그림 + 별첨 표 실시간.** 검수사 2026-09-05 «실시간 작업보드 원한게 앞에 베이를 원한게 아니고 보이는 베이를 원한것이고 통계는 선박 밑에 빈공간에 별첨 자료처럼 실시간으로 채워 지는걸 원한것입니다» · «좌우 여백을 최소화 하여 충분히 크게» · «통계에서 / 가 수직으로 같아야 합니다. 아니면 숫자 보기가 힘들어 집니다». 3.10 은 «BAY 21» 글자였다. ⇒ ①오른쪽 호기 칸 = 그 호기가 지금 작업 중인 짝 베이 그림 — 베이플랜 그 자체(`BayPlan onlyBay·compactZoom` — 도구 없이 BayPage 한 장, 칸은 커도 끝4자리 `compactCells`, 완료 초록은 completed 한 벌). 컨 목록은 ChiefBayEdit 와 같은 길(fullEdiMapOf + records 실적 자리 — 카토스 자리 포함). ②왼쪽 = 별첨1 선사별·별첨2 화물 종류별·별첨3 규격별 F/E, 칸마다 «완료/전체»(`utils.legendLiveOf` — 인쇄 별첨과 같은 분류, 리퍼는 3.6-02 isReeferContainer 한 벌 — 인쇄 별첨도 같은 한 벌로 고쳐 2230 을 리퍼로 센다). «/» 는 완료·전체 두 칸으로 갈라 한 세로줄. ③보드는 화면 폭 전체(max-w 밖으로), 여백 최소, 줄 수에 따라 칸 배율(1척 0.42 → 4척 0.16, 포커스 0.5). 실측 DJCT 0223E 22:20 — 1호기 BAY (12)13 · 2호기 BAY (04)05 · 완료 초록 98칸 · 별첨1 DJS 61/83 42/42 103/125 · 합계 77/110 140/141 217/251(인쇄 별첨 분모와 같음). 연막검사 `smoke_liveboard`(jsdom 실소스·베이사전 DJCT 사본)   // 3.10 **실시간 작업 보드 — 지금 일하는 배만 · 한 척 한 줄(왼쪽 통계 · 오른쪽 호기별 작업 베이) · 세로 4등분(5척부터 스크롤).** 검수사 2026-09-05 «실시간 작업보드는 지금 손봐야 합니다. 작업중인 선박만 보여야 하는데 16척을 다 올려놓고 있습니다» · «지금 작업하는 선박의 2갱이 작업하는 베이가 보이도록 고쳐야 할것입니다. 좌측에 통계가 우측에 작업화면 2갱이면 1호기 작업베이 2호기 작업배이 그런식으로요» · «화면을 세로로 4등분해서 최대 4척이 보이게 2척이면 2등분 5척이면 스크롤 되게». ①보드에 남는 배 = 터미널이 «작업 중»이라 적은 배(`isWorkingNow`) ∪ 검수원이 앱에 붙어 있는 배 ∪ 30분 안에 앱 완료가 찍힌 배 — 실측 20:59 활성 16척 중 DJCT 0223E 한 척. 예정·출항은 안 올린다. ②`utils.craneBoardOf(voyage, workers)` 한 벌 — 호기마다 termWork(GC10x·pos·at) 최신 컨의 자리가 «지금 베이», 앱 완료(equip)·접속 검수원(equip·bay)·조 등록 이름(crewNameAt)을 호기 번호로 합친다(같은 컨 두 번 안 셈). 동방은 컨별 호기가 없어 `info.qcWork` 로 호기별 대수만(자리 없음). 실측 DJCT — 1호기 BAY 20(07-84) 56대 · 2호기 BAY 4(08-82) 70대. ③카드 = 한 줄(왼쪽 진행·터미널 대조·검수원 · 오른쪽 호기별 «N호기 이름 · BAY · 로우-단 · 대수 · N분 전»), 보드 높이를 척수(최대 4)로 등분, 5척부터 스크롤. 연막검사 `tools/smoke_craneboard.cjs`   // 3.9 **X-RAY 봉인자 = 그 조·그 호기의 근무자 — 터미널 표기는 봉인자가 아니다.** 검수사 2026-09-05 «카토스 조회하면 XRAY 넘버도 보일 것입니다. HLHU6412125 조회해보면» · «입력된 실번호가 있을것입니다» · «그건 빈곳으로 놔두시면 됩니다. 수기로 기록할수있게» · «미르에게 어제 야간조 1호기 누구 2호기 누구 하고 알려주면 그때 봉인자가 기록 될것입니다» · «조등록이 그때 근무자입니다» · «카토스라는 문자는 들어 가면 안됩니다». 실측 DJCT 0223E 양하 — X-RAY 8대 중 내려진 3대(HLHU6412125·DJLU5150170·DYLU2150705)의 세관봉인 실번호(521321·521305·521361)를 카토스가 `sealNo2` 로 주고 있었고(수집기는 2.8 부터 `termWork.customsSeal` 로 받아 둠) 앱 X-RAY 탭은 사람이 친 `xraySeals.seal` 만 읽어 빈칸이었다. 그리고 카토스 실적이 completed 에 «터미널(CATOS)»로 들어오자 `xraySealerOf` 의 «완료자 = 봉인자» 규칙이 **그 글자를 봉인자로 찍고 «사무실 제출 가능»으로 셌다**(3대 전부). ⇒ ①수집기 2.24 가 세관봉인 실번호를 `discharge/xraySeals/{컨}.seal` 에 넣는다(사람이 친 번호는 안 덮음 · 봉인자는 안 씀) ②앱 — `xraySealerOf(xs, comp, info)`: 사람이 등록/손입력한 봉인자 최우선 → 완료가 터미널 표기면 **그 시각·호기의 조 등록 근무자**(`crewNameAt` — 3.8 조 등록 `info.craneCrew`, 통계 `crewWorkStats` 와 같은 구간 `crewSegments` 한 벌) → 등록 없으면 **빈칸**(수기). 터미널 표기는 어떤 경우에도 봉인자로 안 나간다. 저장하지 않고 볼 때마다 계산하므로 «어제 야간조 …»를 오늘 등록해도 어제 컨의 봉인자가 채워진다. 화면·인쇄·엑셀·CSV·컨 상세·검수 리스트가 전부 이 한 벌(호출부 3곳에 info 전달). 연막검사 `tools/smoke_xraysealer.cjs`(DJCT 실데이터 사본)   // 3.8-01 **휴지통에 넣은 배가 목록에 되살아나던 것** — 검수사 2026-09-05 «베이메트릭스에서 휴지통에 넣은게 자꾸 목록으로 다시올라옴 영구삭제 바랍니다». 실측 보관소 73키 중 BSDU·MARS·SAWA 가 휴지통(38키) 사본과 한 글자도 안 다른 채(updatedAt 까지 같음) 되살아나 있었다 — 새로 저장한 것이 아니라 기기 로컬 사본(userBayDict)이 「☁ 전체 동기화」로 그대로 올라간 것(1.57-01 이 updatedAt 을 존중). 3.5 휴지통은 «내리기»만 있고 «다시 못 오게 하는 문지기»가 없었다(1.60-01 은 부팅 자동 업로드만 없앴다). ⇒ `fbIsTrashedShipBayDict` 신설 + `fbSaveShipBayDict` 입구(보관소 쓰기 한 곳 — 전체 동기화·EDI 자동 등록·def 업로드·빌더 저장이 전부 지난다)에서 휴지통에 있는 코드는 되살리지 않는다. 휴지통 노드가 곧 «되살리지 말라는 표식»이라 지우지 않는다(= 영구 삭제). 전체 동기화는 «휴지통에 있는 배 N척은 되살리지 않았습니다»로 센다. 되살아난 3키는 보관소에서 다시 내렸다(백업 `_claude/backup/ship_bay_dict_v3_revived3_bak_20260905_1840.json` · 휴지통 사본과 동일 확인). 연막검사 `tools/smoke_baytrash.cjs`   // 3.8 **호기–검수원을 조 단위로 기억한다 — 미르에게 «주간 1호기 김판석 2호기 송제욱».** 검수사 2026-09-05 — *«미르에게 검수앱 OWBH 1호기 이인철 3호기 최관식 이렇게 불러주면 앱에 등록해주기 그래야 누가 몇개나 작업 했는지 알수 있고 어디까지 작업한걸 알수가 있음 즉 선박이름 호기와 검수원이름 앱에 등록하는 기능»* · *«등록자는 등록할 필요가 없습니다»*. 실측 — 김성일이 09-04 SWMM 에서 22:11 «주간 1호기 김판석 2호기 송제욱» · 22:12 «야간 1호기 박진우 2호기 김성일» 을 이미 치고 있었는데 앱은 1·2 를 끝자리 «12»로 읽어 컨 조회로 흘렸다(miss 로도 안 남았다). ⇒ ①`utils.parseCraneCrew` — «(주간|야간) N호기 이름 …»(이름-먼저 어순·조사·같은 호기 두 번이면 뒤엣것). 이름은 **명단(코드+서버 `staffList`)에 있는 것만**(음성이 이름을 자주 틀린다) — 아는 이름이 하나라도 섞여 있으면 모르는 낱말은 «명단에 없어 못 적었어요»로 알리고, 아는 이름이 하나도 없으면 등록 문장으로 보지 않는다(«2호기 양하시작» 류). 되묻기(«맞아?»)·취소(«빼줘»)는 등록이 아니다(취소는 «지우기는 아직 없어요»). ②`fbSetVoyageCraneCrew` → `info.craneCrew/{MM-DD 주간|야간}/{N호기} = {name, at}` PATCH(조 키는 갱 수 `gangsShift` 와 같은 벌 `crewShiftKey` — 기준일은 gangKeyFromWords 처럼 **지금 조의 시작일**이고 그 날의 «주간»·«야간»이다. 22:11 «주간» = 방금 끝난 09-04 주간(검수사 실측), 정오 «야간» = 오늘 밤. 조를 안 대면 지금 조. 등록자는 안 적는다). ③집계 한 벌 `utils.crewWorkStats` — **등록된 조의 시작부터 다음 등록 조의 시작(없으면 그 조의 자연 끝 — 주간 19:00·야간 익일 08:00, 카페리는 끝까지)까지가 그 사람 구간**, 창 밖은 «조 미상». 카토스 배는 `termWork.equip`(GC10x)+`at`+`pos` 로 컨별(조별 대수·마지막 자리), 앱 완료(`by`·`equip`)도 컨번호로 합치되 **앱으로 직접 찍은 사람(`by`)이 등록보다 앞**이다. 동방 배는 컨별 호기가 안 오므로 `info.qcWork` QC 합계가 호기별 정본이고(앱 완료가 섞여도 «직접 찍은 것 N대»로만 덧붙인다) «조별로는 못 나눠요·자리도 안 와요»라고 밝힌다. ④미르 — 등록 «…로 기억할게요»(저장 뒤 음성 «기억했어요»), 조회 «김성일 몇 개 했어»·«1호기 누구야»·«누가 몇 개 했어»·«이인철 어디까지 했어»·«2호기 몇 개». 배선 네 화면(작업 시작 탭·양하·LOLO·홈은 배 이름 필수) — 답은 `answerCraneCrew` 한 벌을 ctx 클로저(gangShift 와 같은 방식)로. ✅ 실데이터 — SWMM 2609S 선적 693대가 주간 1호기 김판석 **209** · 2호기 송제욱 **236** · 야간 1호기 박진우 **129** · 2호기 김성일 **119**(합 693, 마지막 38-06-90) / OBWH 2731E 동방 QC101 257·QC103 213. 연막검사 `tools/smoke_crew.cjs` 신설(픽스처 실데이터 `crew_swmm.json`). ★ 2차 시뮬(부 3)·코드 감사(부 7) 지적 전부 수리 — «남았어» 오인·화면 선점 게이트(진행·브리핑·위치·소개)·동방 혼합 QC 소실·되묻기가 정본을 덮던 것·씨/님 호칭·by 우선·자연 끝·조 키 기준일·콘앱 «기억할게요» 거짓말. ⚠ 통계탭·마감 점검의 사람×호기 표는 이 판에 없다(미르 답만) — 인계함   // 3.7-08 **터미널 앱(CATOS)이 찍은 «실제 실린 자리»를 실적 자리로 쓴다.** 검수사 2026-09-04 — *«왜 실제로 올린것을 안받고 예정된 것을 받았나요? 앱은 22번 베이 완료인데... 실제는?»* · *«카토스도 우리 검수원이 찍은것입니다»* · *«결국은 결과는 같습니다 우리앱으로 찍든 PCTC앱으로 찍든 그래서 카토스 자료로 우리앱에 덮어 사용하는 것입니다. 다만 오늘은 계획에 맞춰 했기 때문에 제가 추궁을 한것이고 실제 입력된 데이터로 고치라고 한것입니다.»* 실측 SWMM 2609S 선적 — CATOS 가 `termWork/{컨}.pos` 로 실제 자리를 **전량** 보내 주고 있었는데(`{"pos":"220782","status":"Delivered"}`) 앱이 한 번도 자리로 안 썼다. 그래서 22번 베이가 «EDI 계획 60대 전부 실림 = 완료» 로 보였고 작업 중 실측은 **25대**뿐이었다(나머지는 6·10번에서 옮겨 온 컨이 채웠다). 검수사가 종이 플랜에 손으로 정정해 쓰고 계셨다 — 앱이 해야 할 일이었다. ⇒ `utils.applyCatosPos` 신설 — `records` 의 실적 자리가 **비어 있을 때만** CATOS `pos`(6자리 BBRRTT)를 `bay_actual/row_actual/tier_actual` 로 채운다(`_pos_src:'catos'`). 주입은 `fbSubscribeVoyages` 구독 콜백 **한 곳**뿐이다 — 전 화면이 그 한 벌을 props 로 받으므로 베이플랜·베이상세·검수 리스트·텔리·검색이 코드 수정 없이 따라온다. ⚠ **우선순위 다툼이 아니다** — 우리 앱으로 찍든 PCTC 앱으로 찍든 같은 검수원의 같은 기록이다(검수사 정정). 그래서 «누가 세냐» 를 만들지 않고 빈 자리를 채우기만 한다. 우리 앱으로 이미 찍어 둔 `bay_actual` 은 덮지 않는다(현장 기록 보호). ⚠ **카고플랜은 계획 그대로다**(검수사 확정 2026-08-12 «카고플랜은 변함이 없어야 한다») — VoyagePage·BayPlan 이 `_edi_*` 로 계획 좌표를 되돌려 넘기므로 그림이 안 바뀐다. ✅ 실데이터 — SWMM 2609S 693대 전량 반영, 지금 시점 베이 14(64→66)·18(80→73)·38(44→49) 셋이 바뀐다. 22번은 작업이 끝나가 수는 60으로 같지만 **그 60칸 중 44칸이 계획과 다른 컨**이다(HLHU6405640 EDI10→실제22-07-82 등) — 검수사가 손으로 적으신 그 번호들이다. ⚠ **동방(PNCT)은 이 판에 못 넣었다** — 자리가 적하목록에 없고 컨 조회(c006/m129)에만 있는데 `CNTR_SEQ`(그 컨의 이 터미널 기항 순번)가 정확히 맞아야 답이 온다. 빈칸·0·99·반출일 조건 전부 0행이고 실측 CAAU4286479 는 seq 12 가 OBWH079 라 이번 기항은 더 위다. 한 배 470컨이면 수천 회가 된다 — seq 를 얻는 다른 길을 찾아 별도 판으로.  ★ **감사 «부» 지적 전부 반영**(2026-09-04 다른 클로드) — ①**보관 오염** `fbArchiveVoyageBeforeDelete` 가 구독 덧칠본을 통째로 set 해 «CATOS 가 채운 자리»가 보관본에서 «우리 앱으로 찍은 자리»로 굳고, 되살리면 게이트가 영영 건너뛰어 CATOS 가 나중에 고쳐도 못 고친다(HomePage 의 1주일 자동 정리가 사람 손 없이 그 길로 간다) ⇒ `stripCatosPos` 신설, 보관 직전에 벗긴다(termWork 원본은 남아 되살리면 다시 얹힌다). ②**게이트 누락** — `at`(반입시각) 없는 행은 Booking 이라 자리를 붙이면 **선사로 나가는 선적 EDI 회신**이 안 실은 컨을 실렸다고 말한다(검수사 «선적확인이 눌려야 실물이 실린거란 말입니다» 2026-08-31) ⇒ `computeTermApply` 와 같은 `!rec.at` 게이트. ③**지운 자리가 되살아남** — `fbClearActualPosition` 은 `bay_actual: null` 로 지우는데 RTDB 에서 null 은 키 삭제라 「비었다」와 「지웠다」가 같아진다 ⇒ 그 함수가 남기는 `moves` 끝 `why:'cancel'` 로 가른다. ④**베이 문자열 두 벌** — CATOS 는 `'01'`, 앱은 `String(parseInt)` 라 `'1'`(실측 693대 중 113대가 한 자리) ⇒ `parseCatosPos` 가 앱 모양으로 맞춘다. 검사도 한 자리 표본을 넣어 이 변이를 잡게 했다. ⑤완료자 표기를 `TERM_APPLY_BY` 한 벌로(화면·매뉴얼이 그 문자열을 쓴다) ⑥빈 catch 에 로그 ⑦매뉴얼 한 줄(`helpData`) ⑧3.7-07 이력 복원(주석 슬라이스 때 지웠던 것). 연막검사 14 → **24항**인계함 기록   // 3.7-07 **머스크 리스트에서 규격·풀엠티·무게를 통째로 못 읽던 것** — 검수사 «아직도 리스트 파서에 문제가 있네요?» (MCAP 634N StandardLoadList 실측 — 19대 중 «?(미정) 17 · ⚠기타 ISO 17 · 풀 2»). 한 파일에서 원인 셋이 겹쳤다. ① F/E 열 패턴 `soc.*[ef]` 가 헤더 «Isocode» 에 부분일치한다(i·**soc**·od·**e**) — findCol 은 왼쪽부터 첫 일치를 잡으므로 F/E 열이 6번(Isocode)으로 정해졌고 거기 값은 «45G1» 이라 F/FULL/E/EMPTY 어디에도 안 맞아 **전건 F/E 미정**이 됐다(리퍼 2대만 온도로 F 회복). ⇒ 「SOC」는 낱말일 때만 F/E 열 후보다(`\bsoc\b`). ② «Type»(DRY/REEF)이 «Isocode»(45G1) 보다 왼쪽이라 규격을 DRY 로 읽었다 — **DRY 는 높이를 못 담고** composeIso 에 DRY 항도 없어 iso 가 빈칸이 됐다. 바로 옆 열에 ISO 6346 정본이 있는데 안 본 것이다. ⇒ ISO 전용 열(`isocol_i`)을 따로 찾아 **그 값으로 먼저 판정**하고, 비었을 때만 종전 Size+Type 으로 내려간다. ③ «Gross weight» 가 무게 패턴(…wt)에 안 걸려 **머스크 리스트 전건 무게 0** 이었다. ⇒ `gross.*weight` 추가. ✅ 파급 — MAILBOX 리스트 **344개 전수 드라이런**: 바뀐 것 20개(머스크 양식만) · 그대로 324개 · **iso +2,577대 · F/E +1,197대 · 무게 +2,589대 · 실손실 0**. 달라 보이던 6건은 전부 40RF→40RH 정정이었다(EDI 45R1/45R8 = 40HC 리퍼 — 옛 Size+Type 경로가 표준높이로 낮춰 읽던 것). ⚠ **시프팅(배정 정본)은 이 판에서 뺐다** — 검수사 «이번건은 선적 플랜을 받고 결정하겠습니다». MCAP 634N 의 예측 94대는 허수가 아니었다. 머스크 본사 일정표 실측 — 634N 은 평택 09-06 → 다롄 09-08 → **신강 09-09** 로, 신강은 평택 **뒤**다. 앱이 «신강은 평택 앞»으로 읽은 것은 `lane_routes/IA8` 의 출처인 PCTC 서비스Lane 표의 **갱신일이 2025-05-09** 이기 때문이다. 항로표로 앞뒤를 가르는 것 자체가 못 미덥다 — 항차별 일정표 수집으로 따로 간다. ⚠ **pol·pod 도 이 판에서 안 건드렸다** — 머스크 양식은 선적항이 «Receipt»(KRPYOTM)이고 목적항은 값이 빈 «Discharge» 열에 가려 있는데, 그 값들이 **UN/LOCODE 가 아니라 터미널 코드**다(실측 MCAP 631S — PHDVOKT→PHDVO 는 앞 5자가 맞지만 CNDAIDT→**CNDLC** · CNTSTQQ→**CNTAO** 는 안 맞는다). 그대로 넣으면 `isPyeongtaekPort('KRPYOTM')` 가 어긋나 **선적 평택분이 통째로 빠질** 수 있다. 터미널코드→LOCODE 사전을 실측으로 만든 뒤 규격 판(3.8)과 같이 간다   // 3.7-06 **«점심 먹었어»는 미르에게 묻는 안부다** — 검수사 정정 2026-09-04 *«이건 명백한 오류 입니다. 질문의도는 우리가 뭘 먹을까의 의도가 아닙니다. 순수하게 미르에게 묻는것입니다»* · *«먹었으면 먹었다고 하던지 먹을것이라든지 뭘 먹을 예정이라던지 … 반대로 해당 검수사에게 검수사님은 드셨습니까? 말고 되 물을수도 있고 먹었다 하면 뭘 드셨는지 맛있었는지 그런류의 답»*. 원인 둘. ① `mirChat.js` 의 «점심 먹…» 그물이 **과거형까지 삼켜** 앞으로 드실 분께 하는 «식사 맛있게 하세요»(mirMeal)로 받았다 ⇒ 지난 끼니 갈래 `mirAte` 신설 — 제 끼니를 답하고(먹었다·아직인데 뭘 먹을 참이다) 되묻는다, 이어지는 «맛있었어»·«별로였어»도 받는다. 미래·권유(«먹으러 가자»·«뭐 먹을까»)는 손대지 않았다. ② **그래서 미르가 엉뚱한 것을 배웠다** — `nlSearch.js` 의 «알아들었나» 판정이 조회 조건만 보고 **잡담 답을 안 봐서**, 미르가 화면에 답을 내놓고도 뒤로는 miss 로 적고 3분 안에 이어진 말과 짝지어 자동 별칭을 만들었다. 실측 260903 «점심 먹었어» → «점심 먹으러 가자»(맛집 돌림판)가 사전에 굳어 있었다(RTDB 에서 걷어냄, 백업 `_claude/backup/mir_lexicon_점심먹었어_bak_20260904.json`). ⇒ `_understood` 에 mirSmallTalk 한 벌을 물렸다 — 정말 못 배운 말(«천정이 뭐야»)은 그대로 결산에 남는다. 덤으로 «성일 아저씨은 드셨어요?» 조사 오류도 받침 판정으로 고쳤다. 연막검사 12항 신설   // 3.7-05 **별첨이 최대 발생조건에서도 안 겹친다** — 검수사 «크기를 맞춰도 이런 조건이면 선사가 많거나 포트가 많거나 특수 화물이 많으면 겹칩이 일어납니다. 최대 발생조건을 생각하셔야 합니다» · «중구난방이 아니고 정렬된 방식을 원하는것입니다». 뿌리는 `legendFont` 가 **«한 줄에 박스 몇 개냐»만 보고 표가 몇 줄인지는 식에 아예 없던 것** — 그래서 포트 다섯인 항차에서 별첨2·3 의 합계 줄이 잘렸다. ⇒ ① `cargoPlanCore.legendFontFor(줄수, 표수, 페이지줄수)` 신설 — 페이지 높이(195mm)에서 머리글·바닥글·테두리를 빼고 **줄 수로** 글자 크기를 낸다. ② 한 칸으로 6.5px 아래로 떨어지면 **상자를 두 칸으로**(왼쪽 별첨1 · 오른쪽 별첨2+3) — 높이 요구가 «셋의 합»이 아니라 «둘 중 큰 쪽»이 돼 오히려 9.5px 로 커진다. 자리를 옆으로 흩지 않으니 «최대한 같이»도 지킨다. ③ 빈 자리가 둘 이상이고 그래도 좁으면 같은 줄 옆 칸으로 나눈다(위가 맞아 정렬된다). ④ 안전망 — `src/fitLegend.js` 가 그린 뒤 브라우저에 실제 높이를 물어 넘치면 한 눈금 더 줄이고, 하한(4.6px)까지 줄여도 넘치면 콘솔에 알린다(조용히 안 넘어간다). **«최대»는 상상이 아니라 실측이다** — 2026-09-04 활성+보관 전수로 한 항차 목적지 최대 5(SWMM 2609S)·선사 최대 10(XTPG 0526E)·화물 종류 최대 5(PCSZ 2623E) = 24줄. 그 1.7배인 40줄까지 안 넘치고 못 읽을 크기도 안 나오는 것을 연막검사가 훑는다   // 3.7-04 **업데이트해도 로그인이 안 풀린다** — 검수사 «업데이트 마다 자동 로그아웃이 됩니다. 이느 버그라면 버그입니다. 작업중 업데이트 하면 로그인 부터 다 다시해야 합니다. 캐시가 비워져서 그런다고 하지만 검수사들은 많을땐 수십번 적게는 한두번을 격어야 합니다». **원인은 캐시가 아니었다** — `App.jsx` 의 inspector 는 화면 상태(useState)뿐이고 «자동 로그인 없음»이 확정 사양이라, 업데이트가 부르는 `location.reload()` 한 번에 그대로 날아갔다(캐시를 안 지워도 똑같이 날아간다). ⇒ 새 `src/updateResume.js` — 업데이트가 **스스로** 새로고침하기 직전에 검수원과 보던 해시를 sessionStorage 에 맡기고, 뜨자마자 한 번만 꺼내 종전 로그인 흐름(handleSelectInspector)을 그대로 태운다. 활동 로그·역할 게이트·라우팅이 다 붙고 보던 화면으로 돌아간다. **«자동 로그인 없음»은 그대로다** — 맡긴 자리가 sessionStorage 라 탭을 닫으면 사라지고(폰을 넘겨받은 다음 사람은 로그인부터), 60초 시한이 있고, 한 번 쓰면 지운다. 두 새로고침 길(버튼·다른 탭의 controllerchange) 모두에서 맡긴다. 연막검사 16항 신설   // 3.7-03 **종이 배치 넷** — 검수사가 3.7-02 인쇄본을 보고 짚은 것. ① *«설명이 길어서 상단이 두줄 또는 세줄로 되어 있습니다. 매번 같은 설명은 하단 아래 바닥글로 처리 바랍니다»* → 고정 설명을 `.cpv2-page-footer` 로 내렸다. 머리글에는 이 항차의 것(항차·대수·사전·날짜)만 남는다. 바닥글 문구도 3.7-02 의 «엠티일 때만 E» 규칙에 맞게 고쳤다 — 그림은 고쳤는데 설명이 아직 `DGF·FRF` 라고 말하고 있었다. ② *«베이 넘버가 베이 마다 정 중앙에 위치 하여야 하는데 왼쪽으로 조금 치우쳐 있습니다»* → `.cpv2-bay-title-row` 의 `padding: 0 50px 0 4px` 가 오른쪽만 50px 이라 가운데가 23px 왼쪽으로 밀렸다. 그 50px 은 오른쪽 대수 표시 자리를 비우려던 것인데 그 표시는 `position:absolute` 라 애초에 흐름을 안 쓴다 ⇒ `0 4px` 로 대칭. ③ *«상단/하단 로우 표기 위치가 일정해야 합니다. 티어가 많고 적음에 따라 올라갔다 내려갔다 합니다»* → 로우 라벨이 여백칸(tier-spacer) **안쪽**에 있어 단이 적은 베이는 그만큼 따라 움직였다. 라벨을 여백칸 바깥으로 빼 **언제나 맨 위·맨 아래**에 세웠다(카스피 도면이 그 모양이다). ④ *«별첨 1, 2, 3은 중첩되지 않아야 하며 최대한 같이 있어야 합니다. SWMM은 너무 각 각 떨어져 있으며 OWBH는 2, 3이 중첩되는 부분이 보입니다»* → 빈 자리가 셋 이상이면 셋을 **따로 흩어** 놓던 분기를 없애고 언제나 한 상자에 붙인다. 한 상자에 넣을 때 별첨3 만 `0 0 auto` 라 자리가 모자라면 밖으로 밀려 잘리던 것도 셋 다 `0 1 auto` 로 고쳤다 — 모자라면 같이 줄고 남으면 아래를 비운다. 남는 자리는 카스피처럼 그냥 비운다   // 3.7-02 **검수사 실물 지적 셋** — ① *«FR표기는 자체가 풀입니다. 그런데 FRF라고 보입니다.»* → DG·FR·OT·TK 는 **글자만으로 풀**이고 **엠티일 때만 뒤에 E**(`FR` / `FRE`). 3.7 은 넷 다 F·E 를 붙여 당연한 것을 덧붙였다(선적 EDI 실측 — DG 400대 전부 적). 덤으로 흔한 경우가 두 글자로 돌아가 글자를 줄일 일도 없다. ② *«목적지 표기자 크기를 줄이고 위치 이동 시켰더니 얼마나 작은지 사라진건지 안보입니다»* → 원인 둘. 3.7-01 이 배율을 0.95→0.62 로 내리면서 **인쇄 문턱 3.5pt 는 그대로 둬 --mf 7.5px 아래 배는 코드가 통째로 사라졌고**, 흰 후광 3겹이 4.5pt 글자의 획을 먹었다. ⇒ 후광 제거·색 진한 검정·문턱 3.0pt·배율 **0.70**(종이 실측 대문자 1.13→1.28mm, 글자 상자 아래끝 1.87mm 가 표기자 대문자 띠 1.88mm 에 딱 안 닿는 최대치). 세 글자 칸에서는 코드를 0.55 로 더 줄여 «포트 표기가 화물표기 보다 작아야 합니다»를 항상 지킨다. ③ *«세째 별첨이 중첩되어 잘려 보이는것도 있습니다»* → `.cpv2-legend` 가 `height:100%` 라 한 상자에 표가 셋이면 **셋 다 상자 전체 높이를 요구해 3분의 1씩** 나눠 가졌다. 네 줄짜리 별첨1·2 는 남고 다섯 줄짜리 별첨3 «규격별 F/E» 가 `overflow:hidden` 에 잘렸다. 제 줄수만큼만 갖게 고쳤다   // 3.7-01 **목적지 글자를 구석 표식으로 되돌렸다** — 검수사 실물 지적 «삼각형 있던 부분에 포트가 붙어야 하는데 화물 표기랑 붙어있어 헷갈립니다» · «포트 표기가 화물표기 보다 작아야 합니다». 3.7 이 «짧을수록 크게»라며 한 자를 0.95배로 키운 것이 원인 — ① 화물 표기(1em)와 크기가 거의 같아 «DGF»+«D» 가 «DGFD» 한 덩어리로 읽히고 ② 코드 글자 상자가 가운데 표기자의 세로 띠를 **2.5px 파고들었다**(실측 ATPR 칸 28.4×21.1px · 표기자 대문자 높이 6.9px · 띠 7.1~14.0px — 0.95 ⛔2.5 · 0.85 ⛔1.5 · 0.75 ⛔0.5 · 0.62 여유 0.8). ⇒ 검수사가 처음 보고 «세자표기가 확실하니»라고 한 그 크기(0.62 · 인쇄 4.5pt)로 자릿수와 무관하게 통일하고, 흰 후광을 넣어 바탕색·표기자에서 떨어뜨렸다. 연막검사가 «표기자보다 작은가»·«세로 띠를 침범하는가»·«후광이 있는가»를 치수로 잰다   // 3.7 **플랜의 목적지 표시를 무늬에서 «고정 바탕색 + 세 글자»로 바꿨다** — 검수사 지시 2026-09-03 «플랜에서 포트별 격자를 없앱니다. 눈이 아프고 구분이 잘 안됩니다. 표기보다 엷은 바탕색으로 대체합니다. 목적지별 색으로» · «특수화물도 목적지별 바탕색을 갖고 표기문자만 기록합니다» · «포트색 기본은 가장 포트가 가장많은곳을 선택해서 정해놓고 지정색을 사용 했으면 좋겠습니다. 언제나 그색의 목적지는 어디다 할수있게». 3.2 의 무늬(빗금·줄·점)와 대수 순 배정을 폐기하고 utils.POD_BG 고정 사전을 만들었다 — 선적 EDI 230항차 55,453대를 세어 **실재하는 목적지 31개 전부**에 색을 못 박았다(리스트로 세면 헤더 없는 엑셀 fallback 이 섞여 없는 목적지 16개가 딸려 온다 — 항구의 진실은 EDI). 실제 «한 항차에 같이 실린 조합»으로 그래프 색칠해 **같은 플랜에 나오는 목적지끼리 명도가 겹치지 않게**(실측 0쌍·같은 색 0건) 명도 240~190 여섯 단 × 여섯 색상으로 배정했다 — 명도를 먼저 가른 것은 흑백 레이저 때문이다. 칸 오른쪽 위에 목적지 세 글자(검수사 «세자표기가 확실하니» · «폰트를 굵게 하지 말아 주세요 겹쳐보입니다» — 가는 글씨, 가운데 표기자는 그대로 가운데). 특수화물도 제 색 대신 목적지색을 깔고 종류는 가운데 글자만. 별첨1은 견본 칸을 없애고 포트명 칸에 색을 입힌다(«원래 별첨 포트명에 입혀 주시고»). 600dpi(HL-L2360D 일반 모드) 106lpi 33계조 기준으로 명도 간격을 잡았다. 베이플랜·카고플랜·베이상세 한 벌(utils.podBgOf)   // 3.6-02 **PDF 표 머리글을 항구로 삼지 않는다** — 검수사 실측 2026-09-03 SWMM 2609S 선적 리스트에서 7대의 목적항이 «CLASS» 로 들어가 있었다. `SWMM 2609S Summary.pdf` 의 특수화물 표 머리글이 «POD CLASS / UNNo. 10' 20' …» 인데 `mixerUpload.parsePdfContainers` 의 정규식 /POD\\s*[:-]?\\s*([A-Z]{5})/ 이 그것을 물어 CLASS 를 문서 전체 목적항으로 삼았다(POL 도 같은 식으로 KRPTK 를 물어 엑셀에서 온 나머지 698대의 PTK02 와 어긋났다). 원본 엑셀에는 그 7대도 DISCHARGE=VNSGN 으로 멀쩡했다. ⇒ 머리글 낱말(CLASS·TOTAL·GROSS…)은 항구로 안 보고, 뒤에 «/» 가 붙은 합성 머리글도 값으로 안 읽는다. RTDB 7대 pod 정정 별도   // 3.6-01 **작업 속도의 분모를 «배가 일한 시간»으로 바로잡았다** — 검수사 정정 «이건 회피입니다. 1초에 몰아 찍건 정확히 실시간으로 입력하든 총 걸린 작업시간은 같습니다. 1시간에 800개를 해도 10시간을 일했으면 시간당 80개입니다». 3.6 은 «첫 완료~마지막 완료»로 나눠 몰아 찍으면 값이 튀었고 그래서 «몰아 입력이라 못 잼»이라는 회피를 붙였다 — 그 회피를 없앴다. 이제 접안(atbActual)~이안(atdActual)의 실작업 시간으로 나눈다. **1순위는 터미널 실적**(검수사 «그이유로 터미널 실시간 조회를 하는것입니다» · «저혼자만 앱을 사용해도 나머지는 다른검수 기록이기 때문에 가능합니다») — 앱 기록은 내가 찍은 것뿐이라 폴백이고, 그때는 그 사실을 화면에 밝힌다. 분자도 양하+선적 합계로 바꿨다(분모가 기항 전체이므로). 실측 NSDC 2608N 238대÷310분 = 시간당 46.1대(갱당 23.0) · 몰아 찍은 이종부 90대도 17.4대로 잰다   // 3.6 **컨번호 검산(ISO 6346)** — 손으로 친 번호의 마지막 자리를 검산해 오타를 그 자리에서 짚는다. 「➕ 초과 컨 추가」·조회창·미르가 같은 말을 한다(utils.isoCheckDigit / isoFixLastDigit 한 벌). 막지 않고 «마지막 자리가 N 이면 맞습니다»로 알리기만 한다 — 규칙 안 지킨 번호도 드물게 오므로 현장이 정답이다. 실측 활성 8척 EDI 실번호 120개 전부 통과·마지막자리 오타 120/120 검출. 검수사 확정 2026-09-03(외부 MirAI.py 중 이 부분만 채택) · **같은 판에 작업 속도 수리도 들었다** — 아래   // 3.6 작업 속도를 **전체 실작업 시간**으로 잰다 — 종전엔 «최근 20대 완료 간격»만 봐서 몰아 입력이 섞이면 시간당 수천 대가 나왔다(검수사 실측 2026-09-03 NSDC 2608N 선적 마지막·3호기 이종부 — 20대가 28초 안에 찍혀 **시간당 2,410대**). 판정 한 벌 nlSearch.paceFromRecords(첫 완료~마지막 완료, 쉬는 시간 뺀 실작업분 · 30분 미만이면 안 잼 · 크레인당 45무브 상한 넘으면 «몰아 입력이라 못 잰다»고 밝힘)를 미르 답·통계탭·홈 「오늘의 나」 세 곳이 같이 쓴다. 같은 자료 재계산 2,410 → 35.6대(2갱이면 갱당 17.8)      // 3.5 베이매트릭스 관리 화면 정리 도구 · 보조 보관함(spare 표만 달고 목록에서 가림 — 입항하면 자동 복귀) · RZOR 문구 정정(«LOLO선» → «RORO/LOLO 혼용선, 갠트리 적재분만 셀 좌표») — 상태 칩(지금 기항·기록 있음·기록 못 찾음)·여러 척 고르기·[휴지통으로](되돌릴 수 있는 이동, ship_bay_dict_trash)·배마다 비고(300자, 얕은 저장 fbSetShipBayDictNote). 검수사 지시 2026-09-03 «19개를 선택할수있는 칸을 만들어 화면에 띄우고 선박마다 비고란을 만들어 주세요» — 사전 107키 중 실제로 불리는 것은 36키(08-11 전체 동기화 92건 잔재)   // 3.4 고려해운(KMTC) 선박에서 **클래스 8(부식성) 위험물이 홀드 자리로 선적 계획**되면 빨간 경고(dg8_hold) — 선사 공문 2026-09-03 09:44 «클래스 8은 갑판 적재, 홀드 금지». 게이트는 utils.isKmtcShip 한 벌(선사 KMT·KMD·KMTC 또는 약자 KK__·KSKM), 선적 모드·평택 선적분만. 실측 KSKM 2613N 5-01-06 · 2615N 13-01-08   // 3.3-01 끝4자리가 겹치는 컨이 **둘 다 평택 작업분**일 때도 큰 확인 카드를 안 세운다(자리를 보고 고름) — 3.2-01 은 통과화물만 막았고, 하나를 완료하면 남은 하나가 큰 카드로 승격되던 것이 남아 있었다(검수사 지적 2026-09-03, 실측 KSKM 2616N 7075·1992 · ATPR 2640W 선적 9쌍). 후보·트윈 칩에 자리(베이-로우-단) 표시   // 3.3 양하 순서 «해상부터» — 자동 가이드 접안 칩 옆 [⇄ 육상부터/해상부터] 토글(항차 info.seqRowFrom, 확인 후 저장). 같은 단 안 로우 순서만 뒤집힘, 미르 «순서대로 양하하자»도 한 벌(guidedQueue rowFrom). 케빈이 해상부터 내리는 날 수동으로만 쓰던 것(김성일 메모 2026-09-03)   // 3.2-01 끝4자리 중복(통과화물이 완료 카드로 승격되던 것·NSDC 2608N 0320)·해치커버 장수는 평택 홀드분이 있는 장만·미르 «MCSC 카고플랜»(동사 없음) 플랜 열기·항차번호(633N)≠끝자리·«미르 점심은?»   // 3.2 선적 플랜에서 목적지(POD)별 무늬 — 최다 POD 무늬 없음, 나머지 대수 순 빗금·줄·점(검정 가는 선이라 흑백 인쇄·컬러 화면 동일). 평택 선적분만, 베이플랜·카고플랜·베이상세 한 벌(buildPodPatternMap). 3.1 ATPR 전용 노란 빗금을 흡수(검수사 «선적시 포트 구분이 안돼서 선적위치를 못잡는 경우»)   // 3.1 ATPR 선적 플랜에서 위해(WEI)행 컨을 노란 빗금으로 — 같은 엠티인데 씰을 따로 달아야 해서(검수사 긴급 지시 2026-09-02). 베이플랜·카고플랜·베이상세 한 벌(podHighlightKeys)   // 3.0-01 SMC YANTAI 선박코드 RZSY→SMYA(터미널 배정목록 정본: 선사 RZF·항로 RZPT·항차 635W) — 내장 .def 사전 키·주야 구분 없음 목록·연막검사   // 3.0 미르 자체 학습 — 못 알아들은 말을 이어진 말에서 그 자리에서 배우고(별칭·일반화), 못 배운 말은 mir_misses 로 모아 매일 클로드가 결산해 가르친다(TASK-2026-012)   // 2.99-03 OBWH·RZOR 주야 구분 없음(검수사) — 브리핑 «○○조 앱 미사용» 조 이름 생략 · 갱 배분 조 창 = 계획 작업 시간 전체   // 2.99-02 X-RAY 엑셀 첫 장 기본 양식 = 굴림체 10·가운데 정렬·네 변 실선(검수사 샘플 그대로 — 받을 때마다 손으로 다시 그리던 것)   // 2.99-01 터미널 실적은 작업 시작 시점부터만 — 지난 기항 트레드링스 피드(OBWH 2727E 278/278)가 2729E 브리핑에 «278대 앱 미입력»으로 붙던 것(BUG-2026-008)   // 2.99 베이매트릭스 해치 줄에 커버 «폭» 입력(홀드10 3 4 3 · 3.5 3.5=00열 두 장) + 빌더 저장이 경계를 지우던 구멍(BUG-2026-006) + 00열 두 장 판정   // 2.98-14 커버 막대를 실측 경계(hatchRows) 열수 비례로 — 등분 그림이 «두 장 다 열어야» 오판을 만들던 것(검수사 «판단은 눈으로»)
...
export const APP_NOTE = '캔슬 리스트를 업로드 창에 올리면 그 컨을 리스트에서 뺍니다(더하지 않습니다). 수집기가 읽은 취소 요청분은 경고가 아니라 안내로 뜹니다. 로그인 뒤 작업 선박 선택과 조회만은 3.50 그대로입니다'
