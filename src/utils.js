...
export const APP_NOTE = '수석 보드 호기 카드의 대수와 그림이 서로 바뀌어 보이던 것을 고쳤습니다 장 전체 대수로 호기를 맞춥니다'
...
//    선수(작은 베이)부터 호기 번호 오름차순으로 붙인다. 호기 번호는 QC 목록에서 가져오므로 3·4호기 배도 된다.
...
export function craneBaysByTime(voyage, craneNos = null) {
  ...
  const assigned = pairCranesByCount(nos, counts, craneDoneByNo(voyage));
...
//    동방(컨별 호기 없음) — 호기 표(qcWork)와 «같은 분·다른 장 = 다른 호기(선수부터 오름차순)» 바구니(3.47-02)로 호기를 붙이고, 그 뒤 같은 장은 그 호기가 잇는다.
...
export function hatchEventsOf(voyage, pages = null, now = Date.now()) {
  ...
  if (!rows.some((r) => r.crane)) {
    //  동방 — 바구니로 호기 붙이기(3.47-02 와 같은 규칙: 같은 분에 서로 다른 장 = 호기 수만큼이면 선수부터 오름차순)
    ...
    if (nos.length >= 2) {
      ...
      const doneByNo = craneDoneByNo(voyage);
      for (const m of [...byMin.keys()].sort((a, b) => a - b)) {
        ...
        if (hs.length === nos.length) {
          const assigned = pairCranesByCount(nos, hs.map((h) => hatchCount[h] || 0), doneByNo);
