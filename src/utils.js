...
export const APP_VERSION = 'TallyOne 2.16';   // 검수사 시안 V2(Tallyone-System-V2-Premium)는 앞의 둘과 달리 **디자인 시스템 명세**였다 — 값이 다 적혀 있어 그대로 받았다. ①**아이콘 stroke 2px→1.75 + 둥근 끝**(전역 svg.lucide): lucide 기본 2px 이 굵어 투박했다. 같은 아이콘이 확연히 정갈해진다 — 「고급스럽게」의 실체가 이것이다. ②**페이지 바닥 #080C1A→#060A14**, 카드면 #151F32, 깊은 면 #0F172A 로 3단 분리(종전엔 카드와 우측 패널이 둘 다 #0F172A 라 패널이 안 떨어져 보였다). ③**카드 radius 16→20px · 보더 white/6% · shadow 0 8px 32px · hover 2px 들림+초록 보더(160ms)** — PC 마우스에서만 뜬다(@media hover). ④**카드 간격 8px→폰 12·PC 16px**(시안 «Cards 16px desktop / 12px mobile»). ⑤muted 글자 #8B9BB4 · 비활성 #475569 · 밝은 글자 #F1F5F9 로 램프 교체, 상태색(양하 #3B82F6 / 선적 #F59E0B / 위험 #EF4444 / 수석 #8B5CF6) 토큰화. ⛔ 빌드 교훈 둘: `npx vite build` 직접 호출 금지(루트 index.html 이 빌드본이라 소스를 안 읽고 9모듈만 도는 빈 빌드가 나온다 — 반드시 bash build.sh). 그리고 @layer 안의 클래스 선택자는 Tailwind content 스캔에 잘린다 — svg.lucide 는 소스에 그 글자가 없어 통째로 사라졌다(레이어 밖 평문 CSS 로 이동).
