...
export const APP_VERSION = 'TallyOne 2.03-01';   // 검수사 지시 2건 — ①«자연어라는 용어를 삭제 했는데 검색 예제에 남아 있습니다»: 표시 문구 잔존 3곳 정리(통합검색 안내문 «자연어 탭»→«[▶ 작업 시작] 탭의 미르», 홈 검색창 placeholder «자연어 질문»→«미르에게 질문», 수석 활동 로그 탭 라벨 '자연어'→'작업 시작') ②«수석대쉬보드엔… 데이터 새로 고침 버튼옆에 통합검색창을 넣어 주세여»: 대시보드 전체 카운터 아래에 통합검색 입력창+🔍 — 엔터 치면 그 질문을 들고 통합검색으로 이동(App onOpenGlobalSearch 가 질문 전달, 홈 검색창과 같은 경로)   // 기능(검수사 실사례 2건 — ①터미널 카톡 «명일 양하 DSYU0070126 데미지사진, 데미지리포트에 추가 요청» + «아무 자료도 도착하지 않았습니다. 예약기능이 있어야» ②«SPSU2041959 혹시 17일날 데미지 잡힌게 있었을까요? 가끔 이런 메시지가 옵니다» «다음부터는 찾을수 있어야» «미르에게 8월 17일에 발생한 데미지건 알려줘 하면 보여줄수 있게») — **데미지 예약 + 이력 조회**: ①헤더 ⋯ [📷 데미지 예약] — 자료 도착 전 컨번호+사진(갤러리 첨부)으로 pendingDamage/{CN} 예약, 자료가 도착해 그 컨이 항차에 나타나면 VoyagePage 가 photos 로 자동 승격(멱등) → CARGO DAMAGE REPORT(buildDamage)·조회에 그대로 반영, 예약 목록 ⏳대기/✅반영 표시 ②컨번호 조회 큰 카드(BigResultCard) 맨 위에 📷 데미지 기록 띠+썸네일(예약분+작업 중 보고분, 탭하면 확대) — voyagePhotos prop 4개 호출부 전수 전달 ③경량 색인 damageIndex/{CN}/{ts}(메타만, 사진 제외) — fbAddPhotoReport(damage)·승격이 기록, 통합검색(미르) «SPSU2041959 데미지»·«8월 17일 데미지»·«데미지 기록» 질의에 날짜·항차·내용 나열 + [📷 사진] 단건 로드(voyages→archive 폴백 — 보관 항차도 찾는다) ④소급 색인 배치는 별도 실행   // 기능(검수사 확정 «별도의 미르에게 원함 칸» + «메모함은 하나일지라도 수집함은 이름은 달라도 됩니다. …기존 클로드 메모함으로 들어 갑니다. 다만 입력상자 이름만 틀려질뿐» + «클로드라는 이름은 앱에서 사용안할것») — ①메모 모달 개명 «개발 요청 · 미르에게 원함» + 입력상자 구분 칩 [🔧 개발 요청]/[🐱 미르에게 원함](kind 'dev'/'mir', 저장소는 claude_inbox 하나 그대로 — 세션이 읽어 미르에게 능력 추가) ②화면의 «클로드» 표기 전수 «개발/개발자»로(메모 모달·헤더·보조기능 카드·오답노트·오답 신고 모달·수석 대시보드·기능 색인 — 코드 주석·검색 별칭은 유지) ③매뉴얼 미르 섹션에 원함 칸 안내 한 줄   // 매뉴얼(검수사 지시 «메뉴얼에 보니 자연어 표현이 아직 남아 있습니다. 도우미 미르 라고 바꿔 주시고 미르 사용법을 구체적으로 설명 바랍니다») — ①helpData·featureIndex 의 «자연어» 표시 문구 전수 교체: 탭 이름은 실제 표기 [▶ 작업 시작](1.84 개명 — 검수사 «자연어는 이름이 검수용어가 아닙니다»), 도우미 개념은 «도우미 미르»(«통합검색이든 도우미 미르든», 🐱 미르 조회창) ②찾기·질문 맨 앞에 「도우미 미르 — 부르는 법과 쓰는 법」 섹션 신설: 부르기(미르야)·어디서든(작업 시작 탭·양하/LOLO 검색줄·통합검색)·무엇을 묻나(위치·개수·브리핑·계획·소요시간·EDI·담당자·기능 위치)·되묻기 8초·후속 버튼·⌨/🔊·모르면 AI/질문 기록. 검색 별칭(a:)의 '자연어'는 유지 — 옛 습관으로 검색해도 걸리게. 코드 무변경   // 문구(검수사 확정 «인건비 내용은 다 중요한 뜻으로 대체») — 화면에 보이는 인건비 문구 전수 대체 5곳: 갱 미선택 완료 차단 alert(SearchPanel) «그 갱의 작업 기록이 남지 않습니다 — 아주 중요한 값», 마감 점검 라벨 2곳 «갱별 대수 근거», 마감 점검 설명 «작업 기록이 통째로 빠집니다(아주 중요한 값)», featureIndex 장비 번호 설명. 잔존 확인: 화면 문자열 0 — 남은 것은 코드 주석뿐(클로드용 근거라 유지)   // 매뉴얼 문구(검수사 지시 «여기서 인건비내용은 삭제 해주세요 그만큼 중요하다는 뜻입니다») — helpData 의 인건비 언급 4곳 삭제, 중요성 문구(갱별 작업 대수의 근거·마감 점검 확인)는 유지. ⚠ 범위는 검수원용 매뉴얼만 — 마감 점검 화면(WorkClosingChecklist)·완료 차단 alert·코드 주석의 인건비 근거 설명은 그대로(동작·클로드용 근거) — 바꾸라는 지시가 오면 그때   // 매뉴얼 보충(검수사 지시 «사용메뉴얼에 빠진 기능이 있으면 추가도 부탁» — 0-B 조항) — helpData 에 1.91~2.01 누락분 6줄: 모드 미명시 되묻기([⬇양하][⬆선적][둘 다]+8초), «계획 어떻게 돼»=양하·선적 둘 다, 자료 없는 배 개요 브리핑+EDI 도착 패턴, 자리 뺏김·맞교환(🔁), 수동 «빈 칸 N·남은 컨 M대» 병기, 양하 없는 배 선적 직행(1.94), 규격 40RH=선사 제출 형식(1.99), 검수 리스트 비고 위험물 9/3480 형식(2.00-03 반영해 기존 줄 갱신). 코드 무변경 — 문구만   // 기능(검수사 확정 «양하 탭이든 선적 탭이든 어디든 브리핑 해달라고 하면 그자리에서 해줘야 합니다. 굳이 작업시작을 누르는 불편함을 주어서는 안됩니다» — 스크린샷: 인라인 카드가 «▶ 작업 시작 탭에서 답 보기» 폴백) — 인라인 즉답 카드(양하·LOLO 탭 공용)가 **브리핑·실번호 점검을 그 자리에서** 직접 낸다. 재료는 briefCtx 한 벌(pairs=getBayPairs·rfSkip·esealBrief — SearchPanel :1043 과 동일)로 VoyagePage→ListTab/LoloTab→InlineAnswerCard prop 체인(1.98 교훈 — 시그니처·호출부 전수 grep 대조). containers 는 이미 현재 모드 병합본(:704)이라 SearchPanel 의 modeCs 와 같은 재료. 브리핑 속 유도 버튼(리퍼 보기·실번호 점검 등)도 인라인에서 그대로 이어진다. ②검수사 확정 «항차목록에서 브리핑은 선박명을 특정 안하면 그날 작업할 선박들 전부를 브리핑» — 통합검색 배 미지정 «브리핑» = planDate 가 오늘과 겹치는 항차 전부, 배별 개요 브리핑(1.97) + 컨 자료 있으면 특수화물 한 줄(리퍼F·위험물·FR·OT·탱크·X-RAY). 오늘 배 없으면 종전 «배 이름을 붙여 주세요» 폴백   // 표기 형식(검수사 지시 «DG 표기는 */**** 형식으로») — 검수 리스트 비고·CSV 의 DG 를 «클래스/UN»만(예: 9/3480, 빨강)으로. 번호 없으면 DG 폴백, PG 는 뒤에 유지   // 표기 형식(검수사 지시 «DG cl.9 UN3480을 DG 9/3480 로 표기») — 검수 리스트 비고·CSV 의 DG 표기를 «DG 클래스/UN»(예: DG 9/3480)으로. PG 는 뒤에 유지   // 🔴 버그픽스(검수사 스크린샷 — 2.00 을 넣었는데도 검수 리스트·카고플랜에 DG 가 하나도 없음) — 병합이 지우고 있었다: EDI 파서는 모든 컨을 dg:false 로 초기화하므로 **DGS 세그가 없는 EDI(연운항형)의 false 는 «위험물 아님»이 아니라 «정보 없음»**인데, PrintHub PROTECTED_EDI_FIELDS 와 VoyagePage CORE_FILL(undefined/'' 만 빈 값 취급) 둘 다 리스트의 dg:true 를 버렸다(TNJP 26360E — records 엔 dg 23·un 23 전수 존재 실검증). 수리: 두 병합 모두 특수화물 플래그(rf/fr/ot/tk/dg/oog)는 false→true 승격만 허용 + CORE_FILL 에 pg 추가. tmp_missing 은 대상 제외(EDI 온도 있는데 «미기재»로 뒤집힘 방지). 카고플랜은 dg 만 흐르면 셀 'D'·별첨2 DG 행이 기존 코드로 자동   // 기능(검수사 지시 2026-08-20 «검수용 리스트에 DG (클래스 및 유엔넘버)표기 리퍼온도 표기 OOG 표기(높이 폭) 다 기록 되어야 함» «특수화물 표기도») — 검수 리스트(출력·별첨·CSV) 비고 완전화: ①DG = cl.클래스 UN번호 PG등급(빨강, TNJP 26360E 실측 cl.9 UN3480) ②OOG = EDI DIM 초과 H+/W+/L+cm(1.84-04 ovh/ovw/ovl), 없으면 치수엑셀 폭×높이mm ③FR/OT/TK 플래그 폴백(ISO 일반인데 EDI 플래그만 있는 컨) ④별첨(시트2) 대상에 DG·OOG 플래그 컨 포함(일반 ISO 위험물이 별첨에서 빠졌었다) ⑤CSV 동반 수리 — 실번호가 sl 필드인데 seal 만 봐서 늘 비었고, 온도도 temp 만 봐서 늘 비었다   // 버그픽스(TNJP 26360E 실측 — 규격은 채워졌는데 온도만 실종) — 수집기 합본(LOADLIST) 왕복에 **Temp 열이 없었다**: 합본 생성(mergeApi)에 Temp 추가 + 읽기(autoRegApi·mergeApi) 시 tmp·rf 복원   // 기능(검수사 확정 «규격 표시는 선사에 제출하는 형식으로 — 모든 선사 방식으로, 카고플랜 별첨·검수용 양하 선적 리스트 포함») — 규격 라벨을 제출(텔리 리포트·ASC) 형식과 통일: **하이큐브 리퍼(45R1·4530·43RF·44R·40HR 등) = 40RH**(종전 40RF), 표준높이(42R·4282) = 40RF 유지. 파급 전수 보정: 리퍼 판정 /RF$/→/R[FH]$/(nlSearch·cargoPlanCore), known 라벨 3곳 +40RH, 필터 칩 rf40 라벨 40RH(40RF 합산), 별첨 isoToPdfLabel 40RH→RFHC·40RF→RF40. 시뮬 10케이스+리퍼 필터 회귀 PASS   // 🔴 긴급 버그픽스(검수사 실측 — 항차 검색창 브리핑 시 전체 크래시 «rfSkip is not defined») — 1.86·1.87 이 SearchPanel prop(rfSkip·esealBrief)을 **별도 컴포넌트 SingleSearch 안에서** 참조(1043행 generateBriefing 호출). SingleSearch 시그니처+호출부 2곳에 prop 전달. 하루 잠복 이유: 그 경로(항차 검색창 «브리핑») 렌더를 배포 전에 안 눌러봤다 — 1.86-01·1.92-01 에 이은 스코프 계열 3번째: **컴포넌트 경계 넘는 참조는 반드시 시그니처를 grep 으로 대조**   // 기능(검수사 확정 — TNJP 실측 «tnjp 브리핑»이 컨 100개 나열) — ①컨 자료 없는 배의 브리핑 = **개요 브리핑**(홈 카드 수준: 부두·항차·작업 예정·PORT-MIS·양하/선적 자료 상태·배정 대조·완료) ②«보통 언제쯤 EDI가 도착하는지» — 수집기록 배치 분석(ediPattern, 23척: 마지막 EDI/ASC vs 작업시작, 작업 전 도착분 중앙값·범위)을 «이 배는 보통 작업 시작 약 N시간 전에 옵니다 (과거 M항차)» 로 표기, EDI 대기 중이면 «그 범위 안이면 기다려도 정상» ③브리핑 질의 시 통합검색 컨 나열 억제   // 검수사 정정 «자리를 빼앗은컨이 갖고있던자리로 가면됩니다» — 자리 뺏긴 컨의 재배정은 아무 빈자리가 아니라 **맞교환**: 뺏은 컨의 원계획 자리(비어 있을 때만)를 «🔁 끝4 → XX-XX-XX (뺏은 컨 YYYY의 원자리)» 로 카드에 컨별 추천 표시. 실제 지정은 기존 위치 지정 모달(검증 경로) 그대로 — 일괄 자동 쓰기는 물리 검증 우회라 안 함   // 기능(검수사 확정 «남은컨은 다른방법으로 빈자리에서 배정되어야 합니다» — SWSP 34번: 창고 보관분 자리에 다른 컨이 실림) — **자리 뺏긴 미완 컨 = 재배정 대상**: 계획 칸을 다른 컨이 실체로 점유(occ done·cn 상이)한 컨을 ①그 베이 «남은 컨»에서 빼고 ②「⚠ 자리 미지정·자리 뺏김」 그룹에 편입 — 기존 🅿 빈자리 배치 흐름 그대로   // 검수사 확정(SWSP 실측 «자동가이드와 수동과 왜 틀린지» — 9대가 계획과 다른 자리에 실려 자동 94 vs 수동 103) — 정의가 달랐다: 자동=남은 컨 머릿수, 수동=빈 칸 수(1.56 확정). 검수사 선택 «빈 칸 수 유지 + 병기» + «자동보다 수동이 우선함»: 수동 버튼 «빈 칸 N · 남은 컨 M대», 자동 가이드 버튼에도 빈 칸을 앞에 병기 — 빈 칸 계산은 SearchPanel manualGroups 한 벌을 prop 으로 내림(두 벌 금지)   // 버그픽스(검수사 실측 — 1.94 후에도 SWSP 진입이 양하) — 한쪽만 자료 있는 배의 초기 모드에서 info.mode('discharge' 등록값) 폴백이 실자료 판정보다 앞서 있었다. **자료 있는 쪽이 info.mode 를 이긴다** — info.mode 는 자료가 아예 없을 때만 폴백   // 검수사 실측(SWSP — 양하 없음·선적 243 매칭) 2건 — ①카드 «⏳ 양하자료 대기중» 오표시: 배정에도 0(정상)인 모드는 예정 섹션에서 제외(HomePage _sr 게이트, MissingSideNote 1.78 과 같은 판정) ②항차에 들어가면 빈 양하부터 열리던 것: hasDis/hasLoa 를 노드 존재가 아니라 **실자료(ediContainers·records) 유무**로 — 양하 없는 배는 선적 화면이 바로 열린다(양하 자료가 오면 구독 갱신으로 토글 복귀)   // 검수사 확인 «갱당 7:27? /2?» — 예측 시간은 크레인 N대 병행 기준(배 전체)인데 문구가 헷갈렸다. «크레인 2대가 같이 하면 약 7시간 27분 (1대면 약 14시간 54분)» 으로 명시(formatEta·answerShipSpeed 둘 다)   // 검수사 실측 «미르야 얼마나 걸릴까?» 가 «몇 대 진행되면 페이스를…»으로 끝남(인라인 카드 — 시작 전이라 페이스 없음) — 완료 예상(formatEta)이 **시작 전·기록 부족이면 실측 평균(shipSpeed)으로 먼저 예측**: «아직 시작 전인데, 실측 평균으로 보면 약 7시간 27분 예상이에요». 이 배 기록 없으면 부두 전체 평균 폴백(출처 명시). ctx(shipSpeed·vsl·pier)를 세 경로(작업 검색창·인라인 카드·통합검색… SearchPanel/InlineAnswerCard prop 체인)로 전달 — ⚠ ListTab/LoloTab 스코프에 voyage 없음: vsl·pier 를 prop 으로 내려 ReferenceError 방지(1.92-01 교훈 적용)   // 기능(검수사 확정 — 애매한 질문 처리 원칙) — «얼마나 남았어» 같은 애매한 질문엔 **기본 답(이해 차원)만 주고 질문 방식을 바꿔 달라고 어필**: «더 자세히 물으실 수 있어요 — \"지금 홀드 몇 개 남았어\"·\"몇 시에 끝나\"·\"몇 시간 걸릴까\"» 유도 문구(1.84-03 패턴이라 자동 버튼, 전부 기존 인텐트로 연결). 동봉 1.92-04: «얼마나 걸릴까(걸릴)» 트리거 포함 + 통합검색에서 속도 분기를 물량(isStat)보다 앞으로(«얼마나»가 isStat 도 켜서 물량 답으로 빠지던 것 — 검수사 실측)   // 검수사 실측 «미르야 SWSP 작업 얼마나 걸릴까?» 가 물량 답(개수 질의)으로 빠짐 — ①트리거에 «걸릴(까)» 포함(걸리/걸려/걸릴 전부) ②통합검색에서 속도 분기를 물량(isStat 1.68) 분기보다 앞으로(«얼마나»가 isStat 도 켜는 탓)   // 검수사 실측 «미르야SWSP 몇시간 작업이야?» 가 무응답 — isSpeedQuery 트리거가 «걸릴» 화법만 알았다. «몇 시간» 들어간 질문 전반을 소요 시간 질문으로(«남았» = 진행 질문은 제외)   // 검수사 확정 «미르야 하면 답변이 이상하게 들립니다» — 낭독 자연화: ①«미르야» 인사는 짧게 «네, 말씀하세요»만 읽는다(화면 문구는 그대로) ②모든 즉답 낭독에서 이모지(🐱❄🔖 등 전부)·특수기호를 벗겨 읽는다(\p{Extended_Pictographic}) — 세 경로(작업 화면·인라인·통합검색) 공통   // 🔴 긴급 버그픽스(검수사 실측 «작업시작 누르면 오류뜸») — 1.91-02 의 되묻기 무응답 타이머 useEffect deps 가 아래 선언되는 parsed·results 를 참조해 TDZ 크래시(1.86-01 과 같은 유형 재발). 타이머를 선언 뒤로 이동. ⚠ 재발 방지: 훅 deps 는 렌더 중 즉시 평가 — 새 훅을 넣을 땐 deps 변수의 선언 라인을 grep 으로 대조할 것   // 기능(검수사 확정 «그걸 앱에 반영을 하고 PCTC도 적용을 해야합니다 PCTC자료를 분석해서») — **선박별 실측 작업 속도 + 소요 시간 예측**. 텔리 리포트 51개(20척, PNCT+PCTC) 전수 분석 → 정상범위(8~45무브/크레인h) 23항차 채택 → RTDB shipSpeed/{VSL}_{PIER} 10종(PNCT 3·PCTC 7) 배치 업로드. 미르 «평균 속도»·«몇 시간 걸릴까» — 그 배(부두별) 평균으로 «물량 N무브 ÷ (rate × 크레인) ≈ 약 H시간» 예측, 기록 없는 배는 같은 부두 평균 폴백, 표본<3이면 참고치 경고. useShipSpeed 훅(1회 로드). ⚠ 오파싱 이상치는 배제 — 파서 정밀화·자동 축적은 수집기 후속 판(인계함)   // 버그픽스(검수사 스크린샷) — 통합검색에서 «미르야»가 인사 대신 컨테이너 100개+를 나열. 통합검색 즉답 최상단에 미르 인사 추가 + 그때 컨 결과 나열 억제   // 검수사 확정 2건 — ①«리퍼 어디에 있어?»류 모드 미명시 조회: **되묻고 기다린다**(검수사 확정 «양하나 선적을 선택할 시간을 주고 답이 없으시에 두가지를») — «양하인가요, 선적인가요? 🐱» + [⬇양하][⬆선적][둘 다] 버튼, 8초 무응답이면 【양하 — 여기 있어요】【선적 — 여기에 실릴 거예요】로 갈라 둘 다(splitByModeAnswer). 모드 명시하면 종전대로 바로 ②«계획 하나지만 답은 양하일지 선적일지 모릅니다. 두가지 다를 알고 있어야» — «미르야 계획 어떻게 돼» 처럼 모드 없이 물으면 **양하·선적 전망을 둘 다** 답한다(자료·배정 없는 쪽은 생략). 양하/선적 명시 시엔 그쪽만(1.91-01)   // 검수사 확정 «선적 계획을 알면 양하 계획도 알겠죠?» — 전망 답을 양하·선적 공용으로(answerPlanOutlook): «미르야 양하 계획 어떻게 진행 될것 같아»·«양하 전망» — 배정(planDis)/리스트/EDI 대조·수신 시각·양하확인 진행률. 모드 미명시면 현재 작업 모드 기준   // 기능(검수사 확정) — 즉답 비서 이름 **«미르»**(검수사 고양이): ①«미르야» 단독 = «네, 미르예요 🐱» ②«미르야 …» 호출어를 벗기고 나머지를 질문으로(음성 포함, 전 검색 경로) ③답 카드 이름 «🐱 미르 즉답» ④검수사 예시 «미르야 이번 선적 계획 어떻게 진행 될것 같아» — 선적 계획 전망 답(answerLoadOutlook): 작업 예정·물량(배정/리스트/EDI 대조)·EDI 수신 시각·PRE 여부·선적확인 진행률 조합, 항차 검색창+통합검색(배 지정) 둘 다. 시뮬 PASS   // 검수사 확정 «수집기가 기록한 시간이 있을것입니다. 그걸 말하면 될것» — 수집기 1.8-04 가 _수집기록.txt 의 **메일 수신 시각**을 raw/edi.recvAt 으로 실어 오고, 자료 도착 답이 «메일 수신 HH:MM (앱 반영 HH:MM)» 로 정답을 먼저 말한다(recvAt 없으면 종전 반영 시각)   // 기능(검수사 테스트 질문 «SWSP 선적 EDI 자료 몇시쯤에 받은거야? 최종본 맞아?» — «앱에 질문을 이런식으로 하면 답을 할까») — 자료 도착 답(answerDataArrival) 확장: ①EDI 파일 단위 — 반영 시각·파일명 표시 ②**최종본 판단** — 파일명 PRE(예비)/최종형(FINAL·LOAD EDI) + EDI 대수와 선사 리스트 대수 대조로 근거를 말한다 ③트리거에 «받은거야»·«최종본 맞아» 추가(isDataArrivalQuery 공용) ④항차 화면 검색창(SingleSearch)에서도 즉답(종전 통합검색만). 실구조(SWSP 2608S) 시뮬 — 최종본·PRE 판정 PASS   // 기능(검수사 확정 «두 앱의 기능을 통합검색 답변 담당이 다알고 응용해서 답변» 1탄 + 예시 질문 «이번 SWSP 관련선사는 몇군데이고 각각 몇대씩이고 담당자가 누구지?») — «관련 선사·담당자» 인텐트 신설: 선사(op)별 대수(양하/선적 구분)+한글 선사명+담당자 이름·메일. 담당자는 수집기 1.8-03 이 sender_names(메일 발신자 학습)를 carrierContacts 로 올린 명부 — useCarrierContacts 훅(1회 로드·모듈 캐시)으로 SearchPanel·인라인 카드·통합검색 세 경로 모두. 자료 미도착이면 «셀 수 없습니다», 명부 미도착이면 «수집기 업로드 후 이름이 붙습니다». 시뮬 PASS   // 기능 마무리(검수사 확정 «메일 수집기에서 그것도 수집하게») — 수집기 1.8-02 가 수신 ASC 머리($600/$609)를 info.ascHdr 로 실어 오고, buildActualAsc 가 그대로 따라 만든다(없으면 종전 $609). EDI(1.88 SMDG 감지)와 짝 — 이제 EDI·ASC 둘 다 «받은 판대로»   // 기능(검수사 확정 «선박별로 edi asc 받을때 그버전으로 만들게 해주시면 됩니다») — 실선적 EDI 내보내기가 **그 배가 받은 판대로** 나간다. detectEdiVer(수신 raw EDI 원문 UNH·UNB·DTM 감지: SMDG15=UNOA:1·10자리:201 / SMDG20=UNOA:2·12자리:203 / SMDG22=현행) → buildActualBaplie 헤더 분기(UNB 문법·UNH·DTM 포맷). 선적 수신 EDI 우선, 없으면 양하 수신 EDI, 둘 다 없으면 현행 SMDG22(회귀 0). 실물 시뮬: XTPG SMDG20·SMDG15·STMJ SMDG22 3판 감지·헤더 재현 PASS. ⚠ ASC $600/$609 헤더 차이는 수신 ASC 원문이 RTDB에 없어 자동 감지 불가 — $609(카스피 검증 형식) 유지, 필요 시 선박별 표에 수동 지정(스펙 문서)   // 기능(검수사 확정 — ATPR·WEIHAI 엠티실 관리) — ①선적 목록에 🔖 엠티실 범위 카드: «이번 항차 엠티실은 몇 번 실부터 ~ 몇 번 실까지입니까?» + 복수 구간(«100개가 넘어가면 앞자리 세자리가 바뀝니다 … 1. 521001~522000 2. 523001~524000») → voyages/{키}/loading/esealRanges 저장 ②브리핑에 부착 대상 베이별·규격별 분포+실 배정/부착/잔여(«베이별 규격별 설명도 첨부 … 엠티실 갯수도») ③[엠티실 정리 리스트] — 부착 실(실번호+컨) + **잔여 실 목록** 복사 제출(«200개를 입력했고 컨테이너가 168개라면 32개의 잔여 실번호 리스트도 제출 … 그래야 오기입 문제를 찾습니다»). 시뮬: 브리핑 2구간·미입력 안내 PASS   // 검수사 확정(스크린샷 — 필터 칩, «리퍼만») — 리퍼 규격 칩(20RF/40RF/45RF)의 눈꽃을 아이콘으로: 그 칩에 **풀이 하나라도 있으면 파란 눈꽃, 전부 엠티면 회색 눈꽃**. 다른 칩은 그대로   // 🔴 긴급 버그픽스(검수사 실측 — 앱 전체 크래시 «Cannot access '$' before initialization») — 1.85-08 에서 LoloTab 의 base(useMemo)를 filtered **뒤**에 선언해, filtered 의 deps 배열이 렌더 중 TDZ 로 죽었다. base 선언을 filtered 앞으로 이동. 교훈: useMemo deps 는 렌더 중 즉시 평가 — 참조하는 메모는 반드시 위에 선언   // 기능(검수사 확정 — 리퍼 표시 정책) — ①«리퍼 엠티를 전면에 보이게 하는것은 별로 좋지 않습니다. 검수사가 헷갈려 합니다»: «리퍼» 조회·브리핑 리퍼 카운트 = **풀만**(엠티는 «리퍼 엠티» 명시 질의로만), 리스트 눈꽃 뱃지 = 풀만 색·엠티 회색 ②«머스크는 리퍼가 다수입니다. 그래서 리퍼 체크를 하지 않습니다»: 실 정책에 「리퍼 체크 안 함」(rfSkip) 신설 — 리퍼 확인 모달 자동 띄움·미확인 배지·브리핑 리퍼/온도 경고를 그 배에서 끔(수동 조회는 그대로). 시뮬: 리퍼 조회 F만/엠티 명시/브리핑 rfSkip 3/3 PASS   // 업무규칙(검수사 확정) — 양하 LOLO 탭 기준을 **덱플랜 갠트리 지정분**으로(RZOR 49): 헤더 누적/전체·규격 집계·칩·목록 전부 지정분 기준. «LOLO 49개 리스트에서 조회가 안되면 이번항차 203개 안에서 조회가 되어야» — 검색만 항차 전체 폴백 + 경고 배너: «배에는 실었는데 그 자리에 없었다» = 상대 항구 선적 과실 확인. 선적 모드·덱플랜 미도착은 종전대로 전체 기준   // 업무규칙(검수사 확정 «양하는 볼수 있습니다. 선적은 작업시 볼수 없습니다. 그래서 전체 리스트를 보여 줘야 합니다») — LOLO 질의를 모드로 가른다: 양하 = 덱플랜 갠트리 지정만(1.85-06) · 선적 = 자리 지정이 없으므로 **전체 리스트**가 곧 작업 대상   // 버그픽스(검수사 지적 «덱이 보이면 컨이 지정되어 있는거 아닙니까» — 맞았다) — 1.85-05 의 «갠트리 지정 자료가 아직» 안내가 오진. 덱플랜(stowagePlan)은 와 있었고(RZOR R089E 갠트리 49) 화면(DeckPlanView)은 그것을 직접 그려 보여줬는데, 조회(nlSearch)가 보는 ediContainers 엔 lolo 주입 전이라 0건이었다. containers 병합 단계에서 stowagePlan 슬롯의 lolo·pos·dbl 을 컨 속성에 얹어 화면과 조회의 불일치 제거 — «LOLO 리스트» 실데이터 49대·«D덱 3줄 12칸» 자리 표기 PASS   // 버그픽스(검수사 실측 2건) — ①**질문한 화면에서 바로 답** «양하화면에서 조회했는데 답은 작업시작 화면에서 나옴» — 양하·LOLO 탭의 문장 질문(Enter·음성)이 탭을 옮기지 않고 검색줄 아래 인라인 즉답 카드(InlineAnswerCard, 두 탭 공용)로 뜬다. 후속 버튼·«← 이전 답으로»·첫 줄 낭독 동일, 로컬로 답 못 내는 질문만 «▶ 작업 시작 탭에서 답 보기» 폴백 ②**«LOLO 리스트는?»이 전체 203대를 나열** — LOLO 단어가 lolo(갠트리) 인텐트 트리거에 없었고, '리스트'가 listQuery 로 잡혀 전체 나열. lolo 트리거 추가 + 갠트리 지정(덱플랜) 미도착이면 «지정 자료가 아직 없습니다» 안내, 지정 오면 그 대상만. 특수 타입 0건 즉답(«📭 없음») 추가   // 버그픽스(검수사 실측 — «주의 9건» 전부 오탐) — SDYT079831·YTZL2458967·BHGJ3019428 등 **실 브랜드**가 4문자+숫자라는 이유로 «컨테이너 번호 형식» 판정. 형식만으론 판정 안 함: ①같은 접두 실이 3개 이상이면 그 배의 실 형식(통과) ②접두가 이 항차 컨 소유코드(BMOU·CAAU…)일 때만 혼입 의심 ③항차 컨번호와 정확 일치는 무조건 의심. 시뮬: 실측 9건 통과·진짜 혼입 2건 검출   // 버그픽스(검수사 실측 2건) — ①**LOLO 리스트도 기본 숨김** «여기는 그대로 입니다» — 1.84 게이트(칩·검색 시에만 목록)가 LoloTab 에 빠져 203대가 늘 펼쳐졌다. 칩 토글(같은 칩 다시 누르면 닫힘)도 동일 적용 ②**실번호 의심 판정에서 자릿수 기준 전면 제거** «실번호4자리 인 경우도 있습니다. 알림은 자릿수 기준이 아니라 일반적인것에서 벗어난것을 찾아 주거나 중복 실번호, 풀인데 실번호가 없는거 같은것을» — 절대 자릿수(<4)·그룹 최빈 길이 비교 삭제(4자리 실 오탐), 판정은 중복·컨번호 혼입·특수문자 + 신설 «풀(F)인데 실번호 없음»(실을 주는 배에서만, 절반 넘게 무실이면 자료 특성으로 보고 안 올림). 시뮬 오탐 0   // 매뉴얼 동승(0-B 조항) — 1.79~1.85 신기능의 사용법 줄을 helpData 에: 브리핑 점검 버튼+«← 이전 답으로», FR 화물 실치수(치수 엑셀 자동 수집), 통합검색 배 이름 오타 교정+배 지정 브리핑 즉답, [개발부여]→[테스터부여]/📐 매트릭스(1.80~81) 문구 교정   // 버그픽스(검수사 실측 스크린샷) — 1.85 가 «리퍼 보기»·«엑스레이 보기»를 빠뜨렸다. 검수사 확정 «브리핑 자료 답변 버튼은 컨번호나 실번호를 볼려고 하는것 보다 그것들의 정보를 보고자 함» — 리퍼 조회가 null 로 떨어져 작업 카드(실번호·양하확인)가 떴다. 특수화물 7타입(FR·OT·OOG·DG·TK·리퍼·X-RAY)은 대수 무관 정보 답: >5 베이 분포(온도·클래스 집계) · ≤5 나열+컨별 상세(리퍼 설정온도, X-RAY 대상 표시)   // 기능(검수사 확정 3건 2026-08-19) — ①**«브리핑에서 누른 버튼은 반드시 되돌아 가기 버튼이 있어야 합니다»** — 후속 버튼이 연 화면이 텍스트 답이 아니어도(컨번호 조회·특수화물 결과가 작업 카드로 그려질 때) 「← 이전 답으로」 를 결과 위에 상시 노출 ②**특수화물 답이 실전에서 설명 없이 나오던 구멍** — 1.84-04 상세 줄은 베이 분포 답에만 있어 나열 답·1대 답에서 안 보였다(검수사 실측 «리퍼 조회나 oog 조회등 이전답화면으로 가기 버튼 안보임»·«FR 설명이 없음»). 상세 줄을 specialDetailLines 헬퍼로 빼 나열 답(formatLocationList)에도 붙이고, FR·OT·OOG·DG·TK 는 **1대여도** 텍스트 답을 낸다(종전 1대 = null → 작업 카드 직행) ③**화물 실치수 표기** — 검수사 실물(연태훼리 2717E 입항서류 메일 BH2717YP063货物尺寸.xlsx · HDMU7202406 22P3 플랫랙): 수집기 1.8 이 선사 치수 엑셀(长/宽/高/毛重/件数)을 ediContainers 에 cgL/cgW/cgH/cgWt/cgPcs 로 patch → 답변 컨별 줄에 «화물 5500×2030×2400mm 5.3t ×2건». EDI DIM(초과분)과 나란히, 없는 값은 «초과 치수 기재 없음»(OT 포함 — 검수사 «OOG는 FR을 의미하기도… OT도 높이는 존재»; OOG 필터는 이미 FR·OT 포함)      // 검수사 확정 3건 — ①«← 이전 답으로»가 브리핑으로 안 돌아가던 것: 프로그램적 질문(버튼·릴레이·뒤로)이 음성과 같은 제출 경로(askedAt 등)를 타게 통일 ②FR·OOG 상세: DIM 세그 치수(높이+/폭+/길이+cm)를 파서가 버리지 않고 담아 컨별 표기 ③DG 상세: UN 번호·클래스·포장등급 컨별 표기(파서에 이미 있던 값) — ≤12대일 때 컨별 줄      // 검수사 확정 — 브리핑 답변 속 «\"실번호 점검\"으로 상세 확인» 류 안내를 **버튼**으로(🔍 실번호 점검 보기). 누르면 상세 답, «← 이전 답으로» 누르면 브리핑으로 복귀(질문 스택). 패턴 매칭이라 같은 형식의 유도 문구가 늘어도 자동      // 버그픽스(검수사 스크린샷) — 양하 탭에서 질문+Enter 시 답이 안 보였다. 답을 그리는 SingleSearch 가 수동 모드+베이 선택 뒤에 있던 구조 → 질문이 넘어오면 「작업 시작」 탭 **맨 위에 💬 질문 답변 카드**를 바로 띄운다(✕ 닫고 작업으로). 기존 수동 흐름 인스턴스는 그대로      // 검수사 확정 2026-08-19 «검색창은 통합검색창 하나로 통합… 마이크 스피커 키보드 csv 네개가 우측에» — 양하 탭 검색줄을 통합검색으로 확장. 숫자·컨번호·베이=종전 라이브 필터 그대로, 문장(Enter·음성)=「작업 시작」 탭으로 릴레이해 즉답. 폰 자판 숫자판 기본(작업용), ⌨로 문자(질문용) 전환. 🔊 자동 읽기 — 끝4 조회 결과 1건이면 위치 낭독(speakContainer). 우측 [🎤][🔊][⌨][CSV]      // UI 1차 판2 — 작업 화면(검수사 시안 확정 2026-08-19). ①리퍼 확인 모달: 열면 방식 선택 3버튼(촬영/전부 리스트대로/수기)이 먼저 — 목록은 고른 뒤(«1개든 100개든 한줄로») ②컨 목록 기본 숨김 — 필터 칩을 눌러야 그 항목만 열림, 같은 칩 다시 누르면 닫힘(«리스트는 잘 안봅니다, 필요한 컨만») ③탭 표시 정리 — «자연어»→«▶ 작업 시작», 통계·결과·업로드는 ⋯ 접이(점프 8곳·tab state 불변) ④작업 시작 화면: 지금 모드 카드 하나만 크게(«선적인데 양하를 누르고») — 전환은 상단 토글, 완료는 작은 칸. 기본 구조 불변 — 표시 계층만      // UI 1차 판1 — 실 정책(검수사 확정 2026-08-19). ①«일반»도 저장 → 신규 선박만 묻는다(종전엔 저장 안 해 매일 재질문) ②applyPolicyToContainer 가 none 이면 null — 일반 배에서 엠티 실 입력칸·경고·수석 실 카드 전부 안 뜸 ③홈 하단 「선박 실 정책」 접이판 — 조합 라벨(RZOR=LOLO+엠티실 확인)·탭하면 수정 모달(initial 프리필) ④선적 모드에서만 묻기(M8.08) ⑤기기 간 1일 1회 가드의 default import 버그 수정. 기본 구조 불변 — 저장·표시 계층만      // 콘앱 표기 정정(ConeOne 1.7-01) — 검수사 «콘앱 양하/선적 터미널 양하/선적 이렇게». 출처별로 묶어 «콘앱 211/168 · 터미널 211/168 ✓» 한 줄. 어긋나면 (+3/-2) 로 차이      // 검수사 메모 2건(2026-08-18) — ①**«홀드에 선적될 화물이 있는데 커버 클로징 멘트 나옴»** 수리: shouldAskHatchClose 구멍 4개 (holdDone 이 «1대 완료»였다 → 잔여 0 · loadingHere 가 _ptk 요구 → 자리만 있는 선적을 놓침 · 베이 짝을 사전에서만 봐 홀수 베이 선적 누락 → 인접 ±1 인정 · 자리 모르는 선적 잔여면 묻지 않음). 시뮬 7/7 ②콘앱(ConeOne 1.7)에 작업예정시간·양적하 갯수·터미널 배정 수량 표시      // 검수사 요청 2026-08-17 «항차목록에 터미널과 수집량을 비교할 수 있도록» — 홈 카드 양하/선적 줄에 «터미널 N ✓» 또는 «터미널 N (+3)» 을 나란히. 수집기 1.6 이 PCTC·PNCT 배정목록을 HTTP 직접 수집하며 양쪽 수량을 info.planDis/planLod 로 실어 온다(1.6 전엔 PCTC 만). 일치하면 회색 ✓, 어긋나면 호박색으로 차이. ⚠ 배정 수량은 예정치가 섞이는 참고치 — 카운트 기준 아님      // 검수사 요청 2026-08-17 «수석대쉬보드 화면에서 TOP화면버튼과 통합검색후 나가는 버튼» — ①수석 대시보드 우하단 ▲TOP 플로팅(300px 스크롤 시 표시, 부드럽게 맨 위로) ②통합검색 상단 «← 나가기»(history.back — 들어온 화면으로, 이력 없으면 홈)      // 기능(검수사 확정 2026-08-17) — 인원관리 행별 **테스터부여/테스터회수** 버튼. 왜: «임원들에게 테스터 자격을 부여하고 앱 설명을 하기 위해» «임원 전체가 아니고 한두명». 직급은 살리고 직책만 얹음(회장→회장(테스터), 회수 시 원상복구 — 관문4 시뮬 7/7 PASS). fbSetStaffRole 신설(addedAt 보존). 이미 수석인 사람·소유자는 버튼 숨김      // 기능(검수사 확정 2026-08-17) — ①**매트릭스 편집 권한을 인원관리로**. 종전엔 빌더(배를 하나 열어야 접근) 안 「권한자 관리」에만 있었다. StaffManagerModal 에 📐 부여/회수 토글+배지, fbSetMatrixEditors 자격을 «명단 포함 ∨ 관리자»로 확장(관문4 시뮬 5시나리오 PASS). 노드·게이트·빌더 패널 무변경 ②홈 상단 버튼 줄 폰 반응형 — 수석 대시보드 버튼이 쭈그러들고 데이터 새로고침이 화면 밖으로 밀리던 것. 폰(<640px)은 두 줄, 수석 버튼 글씨·아이콘 키움. PC 종전 그대로      // 기능(검수사 확정 2026-08-17) — **테스터 권한** 신설. «모든 기능을 사용하되 제(소유자) 고유 기능만 빼고». isChief 정규식에 '테스터' 편입 한 곳으로 수석 게이트 전부(대시보드·마감텔리 생성·완료 저장·복원·정리)+비번 잠금이 함께 걸린다(관문 4 시뮬 — 실직원 35명 회귀 변화 0). 인원관리 직책 선택지에 테스터 추가, 호칭만 isTester 로 '테스터' 표시(Header·AuxPage). 검수 작업 화면(양하·선적 입력·수정)은 원래 전원 공개라 자동 포함      // 인계함(2026-08-16 «시프팅 모브 수를 ×2 고정이 아니라 실제 완료로») — 홈 배지 모브를 완료 도장 합산으로. 내림(양하 completed) 1 + 실음(선적 completed) 1 = 2모브, 내림만 = 1모브, 작업 전 = «예정 N×2모브». 마감텔리 1 TIME/2 TIME 셈법과 일치      // 인계함 처리(2026-08-17) — ①홈 카드: 한쪽 섹션이 없으면 «없음»의 근거를 말한다(자료 미도착=재촉 / 배정에도 0=정상 / PNCT=수량 미상). 근거는 수집기 1.5-06 의 info.planDis/planLod(배정목록 수량, 근거 표시 전용) ②helpData: 배 순서(작업시작→먼저 끝나는 배→자료 있는 배) 한 줄 ③cone.html: 카드 렌더 안 _planMs 복제본 2벌을 공용 헬퍼로 통합(ConeOne 1.6-02)      // 검수사 지적 2026-08-17 «089항차랑 088항차랑 같이 들어 오나요?» — RZOR 한 배의 두 항차가 둘 다 오늘 일정으로 떴다. `pilot_forecast[선박코드]`·PORT-MIS 는 **선박 단위**라 항차 구분이 없는데 앱이 planDate 없는 항차에 그대로 붙였다(수집기는 배정 대조로 이미 거부한 값이다). → 같은 선박 항차가 둘 이상이면 planDate 없는 쪽엔 선박 단위 값을 안 쓴다. 덤으로 도선 예보를 직접 쓸 때 **작업시작 환산(PNCT +120·PCTC +90)** 을 안 하던 것도 고쳤다 — 카드가 도선 시각을 작업시작인 양 보여주던 자리      // 검수사 확정 2026-08-17 «저라면 같은 시작시간이라고 끝나는 시간이 먼저인걸 앞에 놓겠습니다. 이유는 먼저 사라지기 때문입니다» «같이 08시 시작이지만 하나는 16:00 하나는 중간 하나는 그 다음날까지 갑니다» — 작업시작이 같으면 **작업종료(출항) 빠른 순**을 2순위로. 자료 유무·완성율은 그보다 뒤. 콘앱(ConeOne 1.6-01)도 같이      // 검수사 확정 2026-08-17 «검수앱 선박 순서도 작업 일시순으로 해주세요» — 홈 항차 목록 정렬의 1순위를 **작업일시**로. 종전 1·2순위였던 자료 완성율·자료 유무는 «작업일시가 같을 때만» 가르는 동률 판정으로 내려왔다(검수사 확답 «작업일시 먼저, 같은 시간대면 자료 있는 배 위»). '작업 중'과 '입항 예정'을 한 등급으로 합쳐 두 덩어리로 갈리던 것도 없앴다. 콘앱(ConeOne 1.6)도 같은 등급으로 맞춤      // 검수사 메모 4건 (MAMP 631N 작업 중 신고) — ①시프팅을 «보여주기»에서 «양하처리되는 작업 항목»으로 승격(리스트+가이드 큐, 카드 둘: 내림·실음, 카운트는 별도 칸) ②해치커버 오픈 배너를 막던 게이트 3개 제거(통과화물이 얹힌 커버·평택 데크분 0·카드 null) + 치울 컨을 패널 단위로 지목 ③실번호 중복(두 컨에 한 실)을 목록·가이드 카드에서 상시 검사 — 판정을 utils.dupSealMap 한 벌로 통합 ④양하 40ft 우선을 «스프레더 전환 최소화»로 다시 씀 — 1.76-03 의 rowsBlock 은 베이 비교가 빠져 20개 로우 전부를 막아 재정렬이 한 건도 일어나지 않았다   // 구 APP_VERSION 주석(1.76-04 되돌림 경위)은 작업 기록부로 이관   // 되돌림 — 1.76-03(40ft 먼저 복원·해치 배너·실번호 중복 알림) 전량 철회, 코드는 1.76-02 와 동일하다. 검수사 지시 2026-08-16 «오늘 수정한거 복구해놔요 원래대로». 버전만 올린 이유는 폰 서비스워커 캐시가 1.76-03 을 물고 있어서다(1.74-01→1.74-02 되돌림과 같은 방식).
