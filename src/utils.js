...
export const APP_VERSION = 'TallyOne 2.26-10';   // **적부도가 늦는 것은 경보 거리가 아니다 — 대신 물으면 답한다** (검수사 확정 2026-08-24). 종전엔 «X-RAY N대 중 M대 EDI 매칭 안됨» 을 정보 경보로 띄웠다. 검수사 — 그건 경보거리가 안 되고 EDI 보충되면 자동으로 보인다. 실측 DXQD 2633E — `records` 237대만 있고 `ediContainers`·`raw/edi` 는 **아예 없다**(적부도 미도착). 화면엔 컨이 다 보이는데 «매칭 안됨» 이라 하니 자료가 어긋난 것처럼 읽혔다. ⇒ ①**양하 EDI 가 아직 없는 배는 경보를 안 띄운다**(있는데 그 컨만 없으면 종전대로 알린다 — 그건 어긋난 것이다). ②**미르가 답한다** — 위치를 물었는데 화물 중 **한 대도** 자리를 모르면 «양하 EDI 가 아직 안 와서 선내 위치를 모릅니다» 를 붙인다. 출구가 스무 곳이 넘어 각 자리에 붙이지 않고 **내보내는 문 하나를 감쌌다**. 대수·온도 질문엔 안 붙는다(잔소리 방지). ⚠ 같이 고침 — 이 대조가 **한쪽만 정규화**하고 있었다(EDI 쪽 raw · X-RAY 쪽 toUpperCase). 같은 파일 202행 벌로 맞췄다. 컨번호에 공백이 섞이면 조용히 어긋난다. ✅ 검증 4케이스(EDI 없음·일부 없음·정상·공백 낀 번호) + build.sh 에 미르 답변 검사 추가. · 앞선 내력은 git log 참조(2.26 X-RAY 양식 신설 → -01~-09).
