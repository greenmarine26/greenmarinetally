...
export const APP_VERSION = 'TallyOne 2.38-02';   // **카고플랜 글자 크기 재조정 — 검수사가 좁은 배 잘림을 걱정해 한 단계 낮춤.** ①마크 글자 1.25배 → **1.2배**(원래 8px 기준 9.6px). ②소문자 e 는 **1.1배 + 굵기 600**(다른 글자 700) + 가운데 보정 — «1.1로 볼드를 조금만 주고». 화면만 보면 배율 없는 쪽이 차분한데 «인쇄하면 어떨런지» 라는 판단으로 이쪽을 골랐다 — 레이저에서 9.6px 얇은 획은 날아가기 쉽다. ③선박 사전 23척 실측으로 잘림을 확인했다: 최악 MCSC 가로 12칸·세로 13단 → 칸 17.0×15.7px, 가장 넓은 두 글자 DG 가 14.4px·글자높이 11.0px 라 여유 있게 들어간다. 종전 1.25배도 15.0px 로 안 잘렸지만 여유를 더 뒀다. ⚠ 폭만 보면 안 된다 — 칸이 많은 배는 세로도 눌린다(검수사 지적으로 높이를 처음 쟀다). 2.38-01 의 칠·색·RE/RF 규칙은 그대로. ⚠ 이 주석 줄에 따옴표 금지 — build.sh 버전 추출이 깨진다.
