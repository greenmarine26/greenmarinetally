...
export const APP_NOTE = '베이매트릭스 휴지통에 넣은 배는 다시 올라오지 않습니다 — 휴지통이 곧 영구 삭제입니다';
...
export function crewWorkStats(voyage) {
  ...
  //  다음 조 등록이 없다고 그 뒤 작업까지 앞 조 사람 몫으로 세면 안 된다(감사 실측 SWTD 9012E — 589무브가 27시간 뒤까지 야간조 이름으로 붙었다).
  //  카페리(주야 구분 없음 — OBWH·RZOR·SMYA)는 한 조가 끝까지 하므로 다음 등록까지다.
  const noShift = !shipHasShifts(info.vsl);
  const natEnd = (sg) => (/야간$/.test(sg.key) ? sg.startMs + 13 * 3600000 : sg.startMs + 11 * 3600000);
  const segs = shifts.map((s, i) => ({ ...s, endMs: Math.min(shifts[i + 1] ? shifts[i + 1].startMs : Infinity, noShift ? Infinity : natEnd(s)) }));
  //  호기 번호 — 앱 «4호기»·카토스 «GC104»·동방 «QC103» 전부 **끝자리 한 숫자**다(computeTermApply 의 GC10x→x호기 와 같은 뜻).
  const craneNo = (e) => { const m = /(\d)\s*(?:호기)?$/.exec(String(e || '').trim()); return m ? +m[1] : 0; };
  //  첫 등록 조보다 이른 작업(등록 전에 시작한 것)은 첫 조에 — 그 뒤 창 밖은 «조 미상»으로 남긴다
  const segOf = (at) => segs.find((s) => at >= s.startMs && at < s.endMs) || (segs.length && at < segs[0].startMs ? segs[0] : null);
  let outside = 0;
