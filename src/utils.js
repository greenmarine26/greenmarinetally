...
export const APP_VERSION = 'TallyOne 2.17';   // 항목 5 첫 판 — **양하·선적 리스트 탭**(같은 ListTab 을 mode 로 나눠 쓰니 한 번에 걸린다. 검수사 «양하와 선적 리스트 구조는 같으니 같이 적용»). 시안 3장에서 **필요한 것만** 취했다. ①**뱃지 9px → 폰 11px·PC 10px**(높이 22/20 통일): 리스트 한 줄에 9px 뱃지가 16종이라 «뱃지가 너무 작아서 현장에서 안 보였다». 크기·여백·모서리를 .bdg 한 곳으로 모으고 **색·굵기는 호출부에 그대로 뒀다**(이 CSS 가 @tailwind utilities 뒤라 font-weight 를 주면 font-black 을 이겨 버린다). ②**완료 버튼 36 → 폰 44px**, 로우 최소 높이 **폰 72 / PC 64px**, 끝4자리 14 → 폰 16px. ③**검색창 48px**·보조 버튼 36 → 폰 44px, ④**필터 칩 24 → 폰 36 / PC 32px**. ⚠ **검색줄·칩·닫힘 안내가 리스트 탭과 LOLO 탭에 복제본 두 벌**이었다(색만 amber/cyan 으로 다르다). 한쪽만 고치면 규격이 또 갈린다 — 치환 횟수 assert 가 네 번 걸러 냈다. 3금지 1번(호출부·복제본 전부 나열)이 이럴 때 값을 한다. ⛔ 시안에서 **버린 것** — 「장기 보관 컨테이너」(앱에 없는 개념) · 「설정·실제 온도 차 3°C 이상 즉시 보고」(그런 임계값이 앱에 없다. 안전 문구라 더 위험) · 체크박스 20px(**장갑 낀 손에는 지금 44px 버튼이 낫다** — 시안이 늘 옳은 게 아니다). ⛔ **기본 닫힘(1.84) 유지** — 시안이 목록을 펼쳐 그린 것은 «열렸을 때의 모습»이지 기본 동작이 아니다(검수사 확정). 안내 문구도 원문 그대로. ★ 판단 근거는 CLAUDE_SHARED\★시안용_사실목록_리스트·상세·장비.md — 시안 쪽은 앱 동작을 모르고 화면만 만드니 **사실은 이 문서가, 배치는 시안이** 댄다(검수사 «클로드가 검토해서 필요한것만 취하면 되는것»).
