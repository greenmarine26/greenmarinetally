...
export const APP_VERSION = 'TallyOne 2.26-01';   // **출력물이 기존 양식에서 넷을 빠뜨리고 나갔다.** 검수사 실물 대조(`OWBH_2721_XRAY2.pdf`) — *«출력물이 기존자료에서 빠진게 없어야 합니다»*. 종전 양식 머리는 **여섯 칸**이다 — 항차/항공편명 · 운항선사 · 입항일자 · 양륙항 · 선박명 · 선박 호출부호(+MRN). 2.26 은 제목 한 줄과 MRN 만 찍어 **운항선사·입항일자·양륙항·호출부호 넷을 빠뜨렸다.** 게다가 선박명을 `info.vsl`(코드 `OBWH`)로 써서 기존 풀네임(`OCEAN BLUE WHALE`)과 달랐고, 열 이름도 「선사SEAL NO」가 아니라 「SEAL NO」였다. ⇒ ①머리 여섯 칸 복원 — 호출부호는 `info.callsign` 이 빈 항차가 많아 **베이사전에서 코드로** 찾는다(실측 OBWH_2721E). 운항선사는 `carrier ∨ lane ∨ MRN 2~5자`(MRN 26**YTFF**2721I 가 선사 코드를 품는다). 입항일자는 PORT-MIS 입항일시 ∨ 항차 작업창. 양륙항은 KRPTK 고정. ②선박명은 `resolveShipDisplayName` 정본으로(EDI→PORT-MIS→베이사전) ③열 이름을 기존 그대로. ★ **인쇄 블록을 상시 렌더로 바꿨다** — 누를 때 만들면 연막검사가 머리를 볼 길이 없다. 2.26 이 넷을 빠뜨린 채 나간 진짜 이유가 그것이다. 이제 `tools/smoke_xray.cjs` 가 **머리 여섯 칸과 밑줄칸을 매 빌드마다 확인**한다 — 다시 빠지면 배포가 막힌다. ⚠ 교훈 — 기존 서식을 대체하는 화면은 **원본을 옆에 놓고 칸을 세어야** 한다. 시안만 보면 시안이 뺀 것을 같이 뺀다.  **RZOR PORT-MIS 매칭 수리** (검수사 «이 문제도 해결한 거 같은데 또 나오고»). 여러 번 고쳤는데 또 나온 이유가 있었다 — 매칭이 `getShipBayDictData` 에 얹혀 있는데 그 함수는 «베이 **구조**»를 주는 함수라 **구조 없는 항목(껍데기)을 일부러 버린다**(`_fb[_key].bayDef` 없으면 제외). RZOR(RIZHAO ORIENT)는 **LOLO 라 베이 매트릭스가 애초에 없다** — 카고플랜을 덱플랜에서 받는 배다. 그래서 사전에 콜사인 HOAG 가 멀쩡히 있는데도 조회가 **언제나 null**, 카드는 매번 «콜사인: 없음». 실측 2026-08-23: 사전 39척 중 껍데기는 **RZOR 한 척**, 그 한 척이 계속 안 잡히던 바로 그 배다. PORT-MIS 에는 키 `HOAG` 로 들어와 있었다 — **콜사인만 있으면 붙는 자리였다.** ⇒ `getShipIdentity(imo, code)` 신설 — 구조 없이 **신원(선박명·콜사인·IMO)만** 돌려준다. 베이 그림·카고플랜은 종전 함수를 그대로 쓴다. ⚠ 계열 대체(series-substitute)는 **하지 않는다** — 남의 배 콜사인을 빌려 오면 PORT-MIS 가 엉뚱한 배에 붙는다(V7.26 에서 겪었다). ✅ 파급 검증 — 전 항차 17개 전후 대조: RZOR 만 «매칭 X → O (RIZHAO ORIENT · ETA 08-24 09:00 · 동부두 14번선석)», **나머지 16항차 변화 0**.
