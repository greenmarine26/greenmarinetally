...
export const APP_VERSION = 'TallyOne 2.26-09';   // **출력물 쪽마다 회사명 바닥글** (검수사 지시 2026-08-24 — 좌측 하단 about:blank 자리를 그린마린(주) 영문으로). ⚠ about:blank 은 **브라우저가 찍는 그 탭의 주소**다 — 내용이 아니라 주소다. 새 창을 빈 주소로 열어 문서를 써 넣는 방식이라 주소가 없고 브라우저가 그렇게 적는다. **그 자리는 페이지에서 못 바꾼다**(URL 자리라 CSS·JS 로 손댈 수 없다). ⇒ 우리 바닥글을 쪽마다 직접 찍는다 — «GREEN MARINE CO., LTD.» 중앙, 7pt. 브라우저가 찍는 날짜·제목·주소·쪽번호는 인쇄 설정 «머리글 및 바닥글»을 끄면 사라진다(검수사 안내). ⚠ 영문 상호는 표준형으로 넣었다 — 등기 상호가 다르면 한 줄만 고치면 된다(inspectionList.js 의 .ft). · 앞선 내력은 git log 참조(2.26 X-RAY 양식 신설 → -01 머리 6칸 → -02 인쇄 별도문서 → -03 여백·폰트 → -04 중앙정렬 → -05 운항선사 → -06 머리표 폭 → -07 X-RAY 대상 표기 → -08 출력 셋(PDF·인쇄·엑셀)).
