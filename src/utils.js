...
export const APP_VERSION = 'TallyOne 2.14';   // 검수사 «폰화면에서 **진행중인 항차·수석대쉬보드·양하·선적·예보·데이터 새로고침 이것이 세줄**을 차지 합니다. 분명 **2줄**로 줄이고도 남을수 있씁니다» — 홈 상단 줄 2줄화. 원인 셋: ①「진행 중인 항차/17건」이 세로 2줄 ②수석 대시보드 부제 «전체 검수원 진행률·통계» 한 줄 ③「데이터 새로고침 (5분 전)」이 길어 4버튼이 flex-wrap 으로 넘침. → ①폰은 «진행 17건» 가로 한 줄 ②폰은 부제 숨김 ③RefreshDataButton 폰은 아이콘만(sm 이상만 글자) + 4버튼 줄에서 flex-wrap 제거하고 앞 3개 flex-1 균등. 아이콘은 2.13 규격(ico/ico-s)으로.
