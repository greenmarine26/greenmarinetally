...
import { allStaffNames } from './staffList.js';   // ★ 3.8: 호기–검수원 등록 — 이름은 명단(코드+서버)에 있는 것만 받는다
...
export const APP_NOTE = '실시간 작업 보드가 화면 바닥까지 쓰고, 베이 그림이 잘리지 않습니다';
...
//   완료자 표기는 TERM_APPLY_BY 고정 — 검수원 이름과 섞이면 인건비 근거가 흐려진다(검수사 확정).
export const TERM_APPLY_BY = '터미널(CATOS)';
export function computeTermApply(termWork, completed) {
  ...
  for (const [cn, rec] of Object.entries(termWork || {})) {
    ...
    out.push([cn, equip ? { by: TERM_APPLY_BY, at: rec.at, equip } : { by: TERM_APPLY_BY, at: rec.at }]);
  }
  return out;
}
...
export function craneBoardOf(voyage, workers = [], recordsByMode = null) {
  ...
  for (const mode of ['discharge', 'loading']) {
    ...
    for (const [cn, r] of Object.entries(sec.completed || {})) {
      ...
      if (by && by !== TERM_APPLY_BY && r.at >= c.nameAt) { c.name = by; c.nameAt = r.at; }   // 가장 늦게 찍은 사람
      if (seen.has(cn)) continue;
...
export function legendLiveOf(rows, mode, compMap) {
  ...
  const isPtk = (c) => mode === 'discharge' ? isPyeongtaekPort(c.pod) : (!!c._inList || isPyeongtaekPort(c.pol));
...
export function crewWorkStats(voyage) {
  ...
  for (const mode of ['discharge', 'loading']) {
    ...
    for (const [cn, r] of Object.entries(sec.completed || {})) {
      if (!r || !r.at) continue;
      const by = String(r.by || '').trim(); const who = by && by !== TERM_APPLY_BY ? by : '';
  ...
  for (const mode of ['discharge', 'loading']) {
    ...
    for (const [cn, t] of Object.entries(tw)) {
      ...
      recs[cn] = { ...r, bay_actual: pos.bay, row_actual: pos.row, tier_actual: pos.tier,
                   _pos_src: 'catos', actual_at: t.at, actual_by: r.actual_by || TERM_APPLY_BY };
      hit++;
    }
 ...
 *  ⚠ 벗기는 것은 `_pos_src === 'catos'` 인 것뿐이다 — 검수원이 찍은 자리는 손대지 않는다.
...
export function xraySealerOf(xs, comp, info = null) {
  ...
  //  ★ 3.9 (검수사 2026-09-05): **터미널 표기(TERM_APPLY_BY)는 봉인자가 아니다 — 그 글자가 서류에 들어가면 안 된다.**
  ...
  const person = (by) => { const b = String(by || '').trim(); return b && b !== TERM_APPLY_BY ? b : ''; };
  const human = person(comp && comp.by);                                                        // 앱으로 직접 양하완료를 누른 사람
  const inferred = () => (comp && comp.by === TERM_APPLY_BY ? crewNameAt(info, compAt, comp.equip) : '');   // 터미널 완료 → 조 등록 근무자
  if (sealerAt > 0) {
