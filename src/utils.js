...
export const APP_VERSION = 'TallyOne 2.18';   // 항목 5 둘째 판 — **배치를 실제로 바꿨다.** 검수사 «뭐가 바뀐지 모르겠네요» — 2.13~2.17 다섯 판이 전부 크기·색 다듬기였고, 요청은 «항목이 다른게 아니고 **디자인 구성 배치**가 달라야»였다. 그래서 이 판은 눈에 보이는 것부터. ★**PC 항차 리스트가 2단이 됐다** — 좌: 목록 / 우: **340px 고정(sticky) 상세 칼럼**. 컨을 고르면 오른쪽에 상세가 **붙어 있는다.** 종전엔 모달이 떴다 닫혔다 해서 다음 컨으로 갈 때마다 목록을 다시 찾아야 했다. 아무것도 안 골랐을 때도 칼럼을 비우지 않고 안내를 넣는다 (검수사 «어쩔수 없이 여백이 남으면 관련그림이나 부가 설명을 넣고» · 고를 때마다 목록 폭이 출렁이면 그게 더 불편하다). ★**폰은 바텀시트** — 상세 높이를 92vh→70vh 로 낮추고 배경 흐림을 걷었다. 시안 «바텀시트 60%» 의 요지는 높이가 아니라 **뒤에 목록이 남아 다음 컨으로 바로 넘어가는 것**이다. 60%로 자르면 항목이 너무 잘려 70% 로 뒀다. ⚠ **인스턴스는 하나다.** CSS 로만 가르면 같은 상세를 두 번 그려야 하고, 그러면 Firebase 구독·입력 상태가 두 벌이 된다 — 그래서 «어디에 그릴지»는 JS 로 정한다(`useIsWide` 신설. 앱에 미디어쿼리 훅이 없었다 — 있던 것은 BayPlan 의 1회성 innerWidth 뿐이라 창을 줄여도 안 바뀐다). ✅ **리스트 탭 연막검사를 build.sh 에 넣었다** — 우측 칼럼과 2단 래퍼가 실제로 그려지는지 매 빌드 확인한다. 이번에 상세 렌더를 함수로 들어내면서 **두 번 엉뚱한 자리에 넣었고**(초기 반환 블록 안 · 다른 컴포넌트 안) 둘 다 빌드는 통과했다. 그려 봐야 잡히는 부류다.
