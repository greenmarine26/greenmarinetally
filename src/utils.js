...
export const APP_NOTE = '세관 규격 42HR 은 40HC 리퍼로 읽습니다 EDI 45RE 45R8 과 규격 불일치 알림이 더는 뜨지 않습니다'
  ...
  if (!rows.some((r) => r.crane)) {
    ...
    if (nos.length >= 2) {
      ...
      for (const m of [...byMin.keys()].sort((a, b) => a - b)) {
        ...
        if (hs.length === nos.length) hs.forEach((h, i) => { hatchCrane[h] = nos[i]; });
