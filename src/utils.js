...
export const APP_VERSION = 'TallyOne 2.13';   // 검수사 «전 화면입니다. 시간이 많이 걸리니 **항목별로 나눠서** 해주세요 꼼꼼하게» — 항목 1/11: 정리 + 디자인 토큰 + 아이콘 규격. 전수 실태(2026-08-23): 테두리 색 188종·배경 bg-slate-* 37종·임의 글자 14종(9~11px 에 1,114회)·모서리 18종·**아이콘 19종**(한 파일에 5~8종 혼용). 검수사 «폰은 **아이콘 크기를 맞추면 한줄이면 될껄 크기가 틀려 다음줄을 만들기도** 합니다» → index.css 에 .ico-s(14)/.ico(16)/.ico-l(20) 세 종 + .row-1(한 줄 보장) 신설. tailwind.config theme.extend 에 색(ink/line/act/dim)·글자 5단·모서리 4종·터치높이·maxWidth app(1500px) 고정. 죽은 사본 2(src/BayMatrixManagerModal·ShipMatrixBuilderModal)와 고아 3(BulkStowage·BulkAsc·BayDictLibraryWidget) 제거 — 두 벌 고치는 사고 예방. 화면 코드는 아직 안 건드렸다(다음 항목부터).
