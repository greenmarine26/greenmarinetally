...
export const APP_VERSION = 'TallyOne 2.28';   // **수석 권을 5장에서 29장으로 다시 썼다** — 검수사 «항목당 1장 정도입니다. 이걸로 앱을 사용 가능 할까요?» · «메트릭스만 설명해도 여러장이 나올것입니다» · «또 왜 그렇게 해야 되는지도 설명해야 합니다» · «꼭 지켜야 할것도 설명되어야 합니다». 까닭 — «수석은 한번 실수로 검수 작업에 영향을 주고 또 많은 자료를 잃을수가 있기 때문». 블록에 칸 둘을 더했다 — 「왜 이렇게 하나」(남색)와 「⛔ 한 번 잘못 누르면 잃는 것」(붉은색). 경고와 섞으면 안 읽으므로 따로 둔다. 칸도 여섯으로 — 「먼저 읽을 것」을 맨 앞에 신설해 되돌아오는 것과 안 오는 것을 표로 갈랐다. 베이매트릭스는 7장 — 무엇이고 왜 수석만 만드나 / 찾기 / .def / PDF·복제 / 손으로 다듬기 / 확정과 보정중 / 자료 잃는 다섯 가지. 내용은 전부 저장소 실물에서 옮겼다(버튼 이름·확인 문구·경고 문장). 빌드가 칸마다 2장 이상·매트릭스 5장 이상·왜/잃는것 렌더를 확인한다 — 한 장짜리로 되돌아가면 멈춘다. · 앞선 내력은 git log 참조(2.27 매뉴얼 두 권 · 2.27-01 수석 권 잠김 표시).
