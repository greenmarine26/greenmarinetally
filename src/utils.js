...
export const APP_VERSION = 'TallyOne 2.30';   // **검수앱 매뉴얼도 수석 권과 같은 깊이로 + 미르가 그것을 가르친다** — 검수사 «검수앱도 똑같이 자세하게 설명해주세요» · «메뉴얼 만들면서 생긴 지식을 미르에게 인식 시켜 주세요 미르가 교관이 될수 있도록». ① 공용 권 73장 **전부**에 「왜 이렇게 하나」를 넣고 위험한 14곳에 「⛔」를 달았다(수석 권과 같은 칸). 쓰면서 **틀린 설명이 드러나 고쳤다** — 종전 매뉴얼은 리퍼 온도·사진이 완료를 막는 것처럼 읽혔으나 실제로 앱이 막는 것은 검수원 이름·갱(호기)·X-RAY 봉인 **셋뿐**이다. 실번호 중복도 자동 가이드에서만 막고 컨 상세는 경고, 리스트는 검사조차 안 한다. 완료 취소가 두 갈래이고 뜻이 다르다는 것, 해치·사진 보고는 **카톡이 실제로 나가 회수할 수 없다**는 것, 갱이 곧 인건비 근거라는 것도 새로 적었다. ② 미르가 매뉴얼을 **가르친다** — 종전엔 `dos`·`warns` 를 검색용으로만 읽고 답에는 제목·자리·머리글만 냈다. 이제 왜·순서·조심할 것·⛔ 까지 편다. 수석 권도 색인하되 검수원에게는 «있다»까지만 알리고 **하는 법은 안 편다.** 동의어는 절반만 센다(«리퍼 온도» 가 «리퍼(냉동·냉장)» 로 가던 것) · 검수원이 물으면 수석 항목은 뒤로 민다. ⚠ **매뉴얼이 단일 소스다** — nlSearch 에 지식을 복사해 두지 않는다. 빌드가 전 블록의 「왜」와 실제 가르침을 확인한다. · 앞선 내력은 git log 참조(2.29 로고).
