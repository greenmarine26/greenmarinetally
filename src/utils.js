...
export const APP_VERSION = 'TallyOne 2.19';   // 홈 항차 카드 하단 버튼을 **한 규격**으로. 검수사 — «양하완료 선적완료 버튼 크기가 틀리고 지금 처리는 두줄로 보입니다. 같은 4자씩인데 지금 처리만 두줄이라 다른것보다 박스 높이가 틀립니다». **원인이 셋이었다.** ①「지금 처리」만 `minHeight:48`, 나머지는 44 — **4px 을 코드가 직접 벌려 놨다.** ②패딩도 px-2.5 / px-3 로 달랐다. ③**셋 다 줄바꿈 금지가 없었다** — 폭이 모자라면 두 줄이 되고, 두 줄이 되는 순간 그 버튼만 키가 커진다. ⇒ `ACT_BTN` 한 줄로 묶었다(높이 48 고정 · whitespace-nowrap · 폰 flex-1 균등 분할 / PC w-full 세로). **색만 이어 붙이고 높이·패딩·글자 크기는 개별 버튼에서 다시 주지 않는다** — 그렇게 준 것이 이번 어긋남의 원인이다. 곁들여 ⓐ완료 상태 글자를 «⬇ 양하 ✓»→«⬇ 양하 완료 ✓» 로 맞췄고(3자/4자로 폭이 달라 보였다), ⓑ수집기 응답 문구를 18자→8자로 잘랐다(길면 그 버튼만 넘친다), ⓒ「검수 완료·수석 대기」 뱃지도 같은 줄에 서니 같은 높이로 맞췄다. ⚠ `minHeight:44` 문자열은 홈 화면에 **다섯 벌**이 있었다(통합검색·수석 버튼·예보 모달). 치환 assert 가 «기대 4 실제 5» 로 잡아 냈고, 네 버튼만 title 로 특정해 지웠다 — **문자열이 같다고 같은 것이 아니다.**
