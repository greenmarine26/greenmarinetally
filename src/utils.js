...
export const APP_VERSION = 'TallyOne 2.11';   // 검수사 시안(Tallyone-Mobile-Iphone-Galaxy) 승인 «폰용도 마음에 듭니다» — 폰 로그인을 바텀시트형으로: 히어로(초록 글로우·숨쉬는 점·«그린마린 검수팀 · 오늘 N척 작업중») + rounded-t-32 시트(손잡이·제목·오늘/내일 선박 칩) + 검수원 카드 72px(아바타 48px·선택 시 초록 세로바와 체크 원) + 하단 고정 바(입력 56px·버튼 60px 초록). ★ 시안에 없던 것 셋을 더했다 — ①실명단·실시간 상태 ②**🔒 보호 배지**(비밀번호를 물을 이름을 미리 알린다 — 모르고 누르면 «왜 비밀번호 창이 뜨지»가 된다) ③오늘·내일 작업 선박. ⛔ 기능 불변 — 게이트·잠금·신뢰기기·직접입력·notice·돌아가기 그대로. PC(lg)는 2.10 2단 화면 유지(시트는 껍데기만).
