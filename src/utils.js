...
export const APP_VERSION = 'TallyOne 2.33-01';   // **미르에게 얼굴과 인격을** — 검수사 «이제껏 미르의 대답은 너무 딱딱해» + 미르 그림·잡담 시안(Cozy-Ai-Friend) 제공. ①말투: 답 데이터는 그대로, 종결어미만 친구체(«353대입니다»→«353대야») — 원본 문자열 100여 곳은 안 건드리고 **출구 한 겹**(SearchPanel·GlobalSearchPage localAnswer)에서 mirTone 변환, 화면·음성 동일(검수사 선택). 실답변 141종 전수 통과·데이터 변형 0. ②잡담: 검수사 대본 그대로 이식(mirChat.js) — 업무 인텐트 전부 침묵일 때만 마지막 그물, 업무 냄새(숫자·컨번호·베이 등)가 나면 물러선다. 잡담이 답하면 무응답 신고도 자연히 안 나감. 일반 폴백(«더 들려줘»)은 일부러 안 가져옴 — 못 알아들은 업무 질문이 잡담에 먹히면 미르가 영영 못 배운다. ③얼굴: 검수사가 만들어 온 안전모 미르(mir-face.png 66KB)를 🐱·🤖 자리 4곳에. ④매뉴얼 동봉(0-B).
