...
export const APP_NOTE = '규격이 EDI · 선사리스트 · 세관리스트에서 다르면 알려 드리고, 양하·선적할 때 실물을 보고 맞는 것을 고르실 수 있습니다. 지난 항차는 자료를 다시 올리거나 자료 재처리를 한 번 눌러 주세요'
...
export function craneBaysByTime(voyage, craneNos = null) {
  ...
  const tooClose = (bays) => {
    const ns = [...bays.keys()].map((b) => parseInt(b, 10) || 0).sort((a, b) => a - b);
    return ns.some((n, i) => i > 0 && n - ns[i - 1] <= 2);
  };
  const ordered = [...at2bays.entries()].sort((a, b) => a[0] - b[0]);
  let pick = null;
  for (const [at, bays] of ordered) {
    if (bays.size !== nos.length || tooClose(bays)) { if (bays.size > 1) out.skipped++; continue; }
    out.pairs++; pick = [at, bays];                      // 가장 늦은 성한 바구니가 «지금 자리»다
  }
  if (!pick) { out.why = `호기 ${nos.length}대가 같은 시각에 함께 찍힌 자료가 없다`; return out; }
  const [at, bays] = pick;
  out.at = at;
  [...bays.keys()].sort((a, b) => (parseInt(a, 10) || 0) - (parseInt(b, 10) || 0)).forEach((bay, i) => {
    const no = nos[i], m = bays.get(bay);
    out.byBay[bay] = no;
    out.byNo[no] = { bay, at, mode: m.mode, cn: m.cn, row: m.row, tier: m.tier };
  });
  return out;
}
