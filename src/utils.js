...
export const APP_VERSION = 'TallyOne 2.27';   // **사용 매뉴얼 전면 개정 — 두 권으로 나누고 기능마다 화면 그림을 넣었다** (검수사 지시 2026-08-24). 종전 매뉴얼은 글만 있었고, 7/26 V9.11 이후 2주를 멈춰 있는 동안 앱은 스무 판을 갔다. ① 두 권 — 검수사 공용(기능 사전 14칸)과 수석검수사(5칸). 수석 전용 서류인 마감 텔리와 베이매트릭스 만들기를 공용에서 뺐다. ② 화면 그림 35장 — 스크린샷이 아니라 앱과 같은 토큰으로 코드가 그린다. 판마다 낡지 않고 용량이 0 이다. ③ 빠져 있던 것을 썼다 — X-RAY 조회 탭과 세관봉인 확인서, PC 2단 리스트, 오늘의 본인, 평택 다음 항, 리퍼 칩 풀/엠티, 화물 치수 다섯 칸과 초과 판정, 환적은 시프팅이 아니라는 것, 콘앱. 빌드가 두 권을 눌러서 열어 보고, 수석 전용이 공용으로 되돌아오면 멈춘다. · 앞선 내력은 git log 참조(2.26 X-RAY 양식 → -01~-11).
