...
export const APP_NOTE = '동방 배도 수석 보드에 호기별 작업 베이가 뜹니다 — 같은 시각에 내려온 두 베이로 1호기·2호기를 갈랐습니다';   // 3.38
...
export function craneBaysByTime(voyage, craneNos = null) {
  ...
  //  ⚠ **붙어 있는 베이 짝은 한 크레인 자리다**(재감사 지적 2026-09-09).
  //    바구니 크기만 보면, 한 크레인이 한 분에 02·03 을 내리고 다른 호기가 그 분에 침묵했을 때
  //    크기가 우연히 맞아 통과하고 03 에 남의 호기가 붙는다(실측 OBWH 2735E — 1호기가 02·03 을 오갔다).
  //    베이 짝은 «짝수+뒤홀수»(2+3 · 6+7)로 고정이므로 **번호 차가 1 이하면 같은 자리**다 — 그런 바구니는 버린다.
  //    (실측 성한 바구니의 두 베이 최소 간격은 8베이였다 — 이 문지기가 정상 바구니를 지우지 않는다.)
  const tooClose = (bays) => {
