...
export const APP_VERSION = 'TallyOne 2.33-02';   // **미르 존칭 전환 + 프로필·탄생비화·호칭** — 검수사 지시 연속 4건: ①«반말은 막아 주세요. 전체 존칭어를» — 반말 확정을 뒤집어 **해요체**로 전면 재작성(«353대입니다»→«353대예요», 잡담 대본 전부 «~예요/~드릴게요», 하십시오체의 딱딱함과 반말 사이) ②호칭 «성호 아저씨» 식 — 성 떼고 아저씨·형님 그때그때, 위로는 다정한 쪽, 이름 없으면 «검수사님», 사람 부르는 «너·네가» 전면 금지 ③프로필 — 3살(사람 28)·동부두 야드 출생·포승읍 거주·안전 점검 고양이 ④탄생비화 «진짜처럼» — 2023 태풍 카눈 밤 리퍼 아래서 발견, 안전모에 담겨 나옴, 이름은 그날 접안한 MIR호(평화) ⑤실측 «미르야 검수 해봤어?» 업무가드 예외. 시뮬: 어미 10종·잡담 존칭 검사·업무 가드 전부 PASS.
