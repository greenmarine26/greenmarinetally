...
export const APP_VERSION = 'TallyOne 1.76-05';   // 검수사 메모 4건 (MAMP 631N 작업 중 신고) — ①시프팅을 «보여주기»에서 «양하처리되는 작업 항목»으로 승격(리스트+가이드 큐, 카드 둘: 내림·실음, 카운트는 별도 칸) ②해치커버 오픈 배너를 막던 게이트 3개 제거(통과화물이 얹힌 커버·평택 데크분 0·카드 null) + 치울 컨을 패널 단위로 지목 ③실번호 중복(두 컨에 한 실)을 목록·가이드 카드에서 상시 검사 — 판정을 utils.dupSealMap 한 벌로 통합 ④양하 40ft 우선을 «스프레더 전환 최소화»로 다시 씀 — 1.76-03 의 rowsBlock 은 베이 비교가 빠져 20개 로우 전부를 막아 재정렬이 한 건도 일어나지 않았다   // 구 APP_VERSION 주석(1.76-04 되돌림 경위)은 작업 기록부로 이관   // 되돌림 — 1.76-03(40ft 먼저 복원·해치 배너·실번호 중복 알림) 전량 철회, 코드는 1.76-02 와 동일하다. 검수사 지시 2026-08-16 «오늘 수정한거 복구해놔요 원래대로». 버전만 올린 이유는 폰 서비스워커 캐시가 1.76-03 을 물고 있어서다(1.74-01→1.74-02 되돌림과 같은 방식).
