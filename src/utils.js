...
export const APP_VERSION = 'TallyOne 2.26-08';   // **출력 방식 셋을 다 지원한다** (검수사 확정 2026-08-24 — PDF 가 기본이지만 인쇄도 되고 엑셀로도 받아져야 한다, 기존처럼 전부). 새 창 위에 «PDF 저장 / 인쇄» · «엑셀(CSV) 받기» · «닫기» — 검수 리스트·VGM(M6.71)과 **같은 벌**이다. 인쇄할 때는 버튼이 숨는다. CSV 에는 머리 여섯 칸(선박명·항차·운항선사·입항일자·양륙항·호출부호·MRN)까지 담는다 — 표만 있으면 어느 배 것인지 모르는 종이가 된다. · **표기 정정 동반**: 세관 목록에 오른 것은 **전부 X-RAY 대상**이고 「화물구분」은 그 안의 검사 유형이다 (검수사 — 20대 전부가 XRAY 이고 구분만 다르다). 종전 부제 «전체 20대 · X-RAY 5대» 는 다섯 대만 대상인 것처럼 읽혔다. ⇒ 총 대수를 앞에 두고 구분은 내역으로 전부 나열, 카드 첫 장은 «X-RAY 대상». ★ 이 정정으로 2.26 의 판단이 옳았음이 확인됐다 — 앱 전반의 X-RAY 표시를 «파일에 있으면 대상» 그대로 둔 것이 맞다. 화물구분으로 좁혔다면 106대(64항차 358행 중)를 대상에서 빼는 사고가 났다. ⛔ **빌드 사고를 배포 직전에 잡았다** — APP_VERSION 줄 주석에 따옴표를 넣었더니 build.sh 의 sed 가 주석 속 문자열을 버전으로 잡아 sw.js 와 콘앱 캐시키가 통째로 엉뚱한 값이 됐다. ⇒ build.sh 가 뽑은 버전이 형식(이름 + 숫자.숫자[-숫자])에 안 맞으면 **거기서 멈춘다**. 조용히 나가면 캐시가 통째로 어긋난다.  · 앞선 내력은 git log 참조(2.26 X-RAY 양식 신설 → -01 머리 6칸 → -02 인쇄 별도문서 → -03 여백·폰트 → -04 중앙정렬 → -05 운항선사 → -06 머리표 폭).
