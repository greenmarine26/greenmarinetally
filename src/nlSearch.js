...
export function parseNaturalQuery(text) {
  ...
  //  3.43-02: 씰·봉인의 파손·손상은 데미지(컨 손상)가 아니라 씰 대처법(mirKnowledge)이다 — «씰 파손 대처법» 이 «등록된 데미지 없음» 으로 답했다(검수사 실측).
  //    씰 창만 지우고 나머지를 본다 — «컨테이너 파손됐고 씰도 잘렸어» 는 데미지다(감사: 통째로 지우면 데미지를 잃었다).
  const _tNoSeal = t.replace(/(씰|봉인|(?:^|[^가-힣])실)[^]{0,10}(파손|손상|잘[려렸림린]|끊[겼어긴김]|절단|훼손)/g, '');
  if (/데미지|파손|손상/.test(_tNoSeal)) result.dmgQuery = true;
  ...
  const _strong = new RegExp(`도움말|사용법|사용\\s*방법|매뉴얼|대처\\s*(?:법|방법)|처리\\s*(?:법|방법)|조치\\s*(?:법|방법)|어떻게\\s*(?:대처|조치|처리|해야|하나|하지|하죠|할까)|무슨\\s*버튼|어느\\s*버튼|버튼\\s*(?:어디|어느)|(?:어디서|어디에서|어디에)\\s*(?:${_actV})|어떻게\\s*(?:${_actV})|보는\\s*법|하는\\s*법|쓰는\\s*법`);
  ...
  if (/(?<!스)위치|어디(?!랑|와)|어딨|where/i.test(t)) result.posQuery = true;   // 2.59: «스위치 카고»의 위치 오탐 차단(자격 기출 채점 실측)
  ...
  //   ⚠ 갈래는 def(뜻) 하나만 새로 가린다 — 위치·개수·목록은 posQuery·isStat·listQuery 가 이미 그 역할이다.
  {
    ...
    const _busy = !!(result.digits || result.bay || hasCountFollowCtx || result.isStat
      ...
      || result.weightSum || (result.listQuery && !/종류|부위|명칭|포인트|항목|체크|검수|절차|과정|자재|흐름/.test(t)) || result.bayDistQuery   /* 2.60: «종류 알려줘»의 «알려줘»가 listQuery 를 켜 지식 답을 막았다 — 종류·부위·명칭이 있으면 지식 질문이다(«FR 목록»엔 이 낱말이 없어 목록 유지) */
    ...
    const _howTail = /어떻게|어떡|어찌|무슨\s*방법|방법(?:이|을|은|으로|좀)?(?:\s|\?|$)|절차|과정|(?:읽|보|하)는\s*법|도\s*(?:돼|되나|될까)(?:요)?\s*\?*\s*$|언제\s*(?:해|하는|봐|보는)/;   // 2.59: «셀 넘버 읽는 법» 류 · 2.60-01: 허가 질문(«40 위에 20 올려도 돼») — 물리 규칙·판단 답이 how 몫. bay 붙은 «커버 열어도 돼»는 _howBusy(bay)가 지켜 종전 경로(mirEyes 단계·제동) 유지
...
const _WHY_BY_CODE = {
  ...
  dg8_hold:      '고려해운 규정입니다 — 클래스 8(부식성)은 홀드에 실으면 안 되고 갑판 적재입니다(2026-09-03 공문). 선적 계획에서 그 자리를 빼고 선사·본선에 확인하세요.',
...
const _ALERT_TERMS = {
  ...
  //  ⚠ _alertNorm 이 숫자를 지운다 — «클래스 8» 은 «클래스» 로만 남는다. 그래서 낱말에 숫자를 넣지 않는다.
  //  ⚠ 감사 지적(3.4-01): 묶음1에 «급», 묶음2에 «홀드·갑판·데크» 만 두면 «홀드 작업 급해»·«데크 취급 주의» 같은
  //    평범한 말을 이 경고가 삼킨다(answerAboutAlert 는 검색 파서보다 먼저 돈다). 묶음1을 **이 경고에만 쓰는 말**로 좁힌다.
  dg8_hold:          [['클래스', '부식', 'CLASS', '고려해운', '규정'], ['홀드', '선창', '갑판', '데크', '위반', '경고', 'KMTC']],
