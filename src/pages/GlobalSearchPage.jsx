...
import { answerFeedback, answerCollector, answerTallyPending, answerArchiveStats, answerOverlaps, answerDataArrival, answerHatchStatus, answerGangSplit, answerTotalMoves, answerFirstStart, answerXrayShifts, answerShiftBriefing, isDataArrivalQuery, answerLoadOutlook, isLoadOutlookQuery } from '../chiefAnswers.js';   // 1.69: 수석 통계·이력·계산(96~100)
...
export default function GlobalSearchPage({ voyages, onOpenContainer, portMisData, terminalWork, heartbeat, isChief = true, initialQuery = '' }) {   // 1.69: heartbeat — 수집기 상태 즉답 · 1.69-01: 검수원 진입(홈 검색) — isChief로 수석 전용 통계만 거른다
  ...
  const localAnswer = useMemo(() => {
    ...
    // 1.91: 배 지정 선적 계획 전망
    if (isLoadOutlookQuery(debouncedQuery) && shipCtx) {
      try { const a = answerLoadOutlook(shipCtx.v, shipCtx.info.vslFull || shipCtx.info.vsl); if (a) return a; } catch (e) { /* 아래로 */ }
    }
