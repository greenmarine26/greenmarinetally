...
const CONTENT = {
  ...
  tips: [
    {
      title: '🆕 M6.94.5 (2026-05-27) — DXQD 매트릭스 빌더 정의 반영 안 됨 (4중 결함 수정)',
      examples: [
        { q: '🔥 사용자 보고', a: 'DXQD (XIN QUN DAO, callsign H3OI) 항차 매트릭스 빌더로 18개 베이 저장 후 카고플랜에 반영 안 됨. F12로 확인 결과 master_user_bay_dict_v1 안에 두 entry 분리 존재: dict["DXQD"]는 PDF 자동 파싱본 (sourceFile: "DXQD2615E.pdf", source 마킹 없음), dict["H3OI"]는 매트릭스 빌더 저장본 (source: "user", _userOwned: true, recordCount: 18). 카고플랜이 DXQD 키로 룩업 → PDF 자동본 우선 매칭 → user entry 영원히 미참조.' },
        { q: '🐛 근본 원인 (4중 결함)', a: '(R1) shipMatrixBuilder.extractShipMetaFromVoyage: info.code 없으면 callsign 첫 4자(H3OI) 사용 → PDF 흐름(DXQD)과 다른 키. (R2) matrixToBayDictEntry: callsign: "" 하드코딩, 호출처 사후 보강 의존. (R3) addToUserBayDict: dict[entry.code]=entry만, 같은 배 다른 키 entry 식별자 cross-fill 누락. (R4 ⭐) lookupUserBayDict: 6단계 매칭이 source 검증 없이 첫 hit 반환 → PDF 자동본이 user entry 가림 (원칙 ① 위반).' },
        ...
        { q: '📋 시뮬레이션 검증 (9/9 PASS + 19/19 PASS)', a: '실데이터(사용자 master_user_bay_dict_v1) 기반 sim_real_import.mjs 9/9 PASS: T1 (DXQD 룩업 → H3OI user entry, 핵심 버그 수정 직접 검증), T2~T7 (기존 매칭 케이스 보존), C1~C2 (cross-fill 작동). 다선박 확장 sim_multi_ship.mjs 19/19 PASS: 5가지 시나리오 (PDF+빌더 분리/빌더만/PDF만/.def업로드/Firebase 동기화) 모두 동일 적용 + 교차 영향 격리 검증.' },
        { q: '🔧 변경 파일 (3개)', a: 'src/data/userBayDict.js (lookupUserBayDict 2-Phase + addToUserBayDict cross-fill), src/shipMatrixBuilder.js (matrixToBayDictEntry callsign 인자 + sourceVersion M6.94.5), src/components/ShipMatrixBuilderModal.jsx (handleSave callsign 직접 전달). 추가: utils.js APP_VERSION + sw.js + public/sw.js VERSION 모두 M6.94.5 동기화.' },
        { q: '⚙ React Hook 규칙 (M6.93.18 사고 재발 방지)', a: '수정한 React 컴포넌트는 ShipMatrixBuilderModal.jsx 1개. 변경 위치는 handleSave 이벤트 핸들러 내부 — Hook 호출 없음. 기존 Hook 배치(useState/useEffect/useRef/useMemo) 모두 컴포넌트 최상단 유지. 조기 반환문 if(!matrix) return은 Hook 선언 이후 (변경 없음).' },
