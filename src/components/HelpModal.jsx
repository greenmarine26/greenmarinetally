...
const CONTENT = {
  basic: [
    {
      ...
      examples: [
        { q: '🤖 자동 가이드 버튼', a: '앱이 크레인 순서대로 다음 컨테이너를 예측해 카드로 제시 — 검수사는 [확인]/[수정]만' },
        { q: '접안 방향 선택', a: '처음 한 번 좌현/우현 입력 (육상↔해상 순서 기준, 항차에 저장)' },
        { q: '베이 선택', a: '남은 작업이 있는 베이 그룹 버튼 선택 → 양하: 데크 위층부터 육상→해상 / 선적: 홀드 아래층부터 해상→육상' },
        { q: '수정 3회 연속', a: '플랜대로 진행되지 않음 판단 → 자동으로 ✋수동 모드 전환 (짝 없는 20ft 싱글은 양하 마지막/선적 처음)' },
      ],
    },
    {
  ...
  progress: [
    {
      ...
      examples: [
        ...
        { q: '몇 시에 끝나?',          a: '⏱ 진행 페이스로 예상 완료 시각 계산 (언제 끝나·이 속도면·퇴근 몇시 등도 인식). 음성은 동료처럼 부드럽게 안내' },
        { q: '양하신고할까?',          a: '📋 그날 이상 건을 신고 리스트 기준으로 정리 — 누락(선박에 없음)·초과(리스트에 없는데 내림)·바뀜·리씰·실오류. 컨번호 조회 시 [🚫 선박에 없음] 버튼으로 누락 처리, 양하 모드 [➕ 초과 컨 추가]로 초과 기록' },
        { q: '인수인계 자료 만들어줘',  a: '🔄 남은 작업(양하/선적·남은 베이) + 양하신고할 것 + 특이사항(리퍼 온도·위험물·FR/OT)을 한눈에 정리. 작성 후 [더 전달할 내용] 칸에 특이사항을 적으면 인계 메모로 합쳐 완성 ("인계 해야돼", "다음 검수사에게 넘겨야 해"도 인식)' },
      ],
    },
    {
  ...
  ai: [
    ...
    },
    {
      ...
      examples: [
        { q: '넌 뭐야 / 너 누구야', a: '자기소개 + 질문 예시 안내' },
        { q: '입항 언제야 / 출항 시간', a: 'PORT-MIS 입출항 시각 + 부두 (수집된 선박만)' },
        { q: '지금 몇 시야 / 오늘 무슨 요일', a: '현재 날짜·시간' },
        { q: '날씨 어때 / 비 와?', a: '평택항 현재 날씨 — 기온·바람(풍향/초속)·강수확률' },
        { q: '20번 베이 트윈 가능해 / 트윈 무게 확인', a: '트윈 쌍 무게 점검 — 합계 55톤 초과 = 불가 / 무게차 한계 초과 = 수평 불가 (PNCT 14톤·PCTC 20톤, 부두 자동 적용) / 무게 미상. 브리핑에도 자동 포함' },
        { q: '20번 베이 트윈 가능해 / 트윈 무게 확인', a: '트윈(20ft 쌍) 무게 점검 — 합계 55톤 초과는 불가(위치·무게 표시), 무게 차이 크면 수평 주의. 브리핑에도 자동 포함 (V7.93)' },
      ],
    },
    {
  ...
  tips: [
    {
      title: '🆕 V7.90-07 (2026-06-11) — 브리핑 재구성 (사용자 피드백: 평택분 + 특이사항 중심)',
      examples: [
        { q: '🐛 문제 2건', a: '① 통과화물 포함 전체 개수를 말함(7.1 평택분 원칙 위반) ② 일반 통계만 나열해 애매모호 — 브리핑의 본질은 "일반적이지 않은 사항을 검수원에게 인지"시키는 것.' },
        { q: '✅ 평택분 기준', a: '집계는 작업 대상(양하=POD평택, 선적=POL평택)만. 통과화물은 숫자에서 빼고, 대신 작업 베이에 섞여 있으면 주의사항으로: "🔁 통과화물이 작업 베이(7)에 혼재 — 내리지 말 것".' },
        { q: '✅ 특이사항 중심·행동 지향', a: '⚠ 주의사항 섹션: 리퍼(온도 미입력 — 조회 시 입력) · 위험물(클래스·베이 — 별도 취급) · X-RAY(끝4 — 양하 후 별도 처리) · FR/OT/OOG(치수·고박 확인) · 실번호 의심(요지 + "실번호 점검" 안내). 특이사항 없으면 "✅ 특이사항 없음 — 일반 화물만" 한 줄. 일반 통계는 📌 작업 한 줄로 압축.' },
        { q: '🔈 음성', a: '"양하 평택 100대 — 주의 6건 (리퍼 5, 위험물 1, 엑스레이 1), 잔여 70" — 첫 줄에 평택분·주의 핵심·잔여.' },
      ],
    },
    {
      title: '🆕 V7.90-06 (2026-06-11) — 실번호 스왑(서로 바뀜) 즉시 경고 (사용자 요청)',
      examples: [
        { q: '🐛 시나리오', a: '두 컨테이너의 실번호가 리스트에서 서로 바뀌어 있으면, 한쪽을 실물대로 수정하는 순간 다른 컨과 중복이 생김 — 이걸 그 자리에서 알려줘야 함.' },
        { q: '✅ 즉시 경고', a: '풀씰 수정·실오류 보고·리씰 등록 세 입력 모두에서 — 입력한 실번호가 이 항차 다른 컨테이너의 리스트 실번호(풀씰/엠티실/리씰)와 같으면 즉시 확인창: "OOOO의 리스트 실번호와 같습니다 — 서로 바뀌어 있을 가능성, OOOO도 실물 확인". 기록은 막지 않음(실물이 진실) — 상대 컨테이너 확인을 유도.' },
        { q: '✅ 점검 연동', a: '"실번호 점검"·브리핑의 중복 검출 사유에도 "서로 바뀌어 있을 가능성, 양쪽 모두 실물 확인" 안내 추가.' },
        { q: '🔬 검증', a: '모달 단독 마운트 실렌더 — A 풀씰에 B 실번호 입력 → 경고 문구 정확히 표시 PASS.' },
      ],
    },
    {
      title: '🆕 V7.90-05 (2026-06-11) — 실번호 오류 사전 점검 (브리핑 "주의할 점")',
      examples: [
        { q: '🔍 무엇을 잡나 (사용자 설계 4종)', a: '① 다른 컨테이너인데 같은 실번호(중복) ② 실번호 칸에 컨테이너 번호 혼입(컨번호 형식이거나 컨번호 숫자 포함) ③ 형식 특이(특수문자·비정상 짧음) ④ 자리수 부족 — 같은 형식(접두) 대부분이 7자리인데 5자리 등(엑셀 앞자리 잘림 의심). 풀씰·엠티실 모두 점검.' },
        { q: '📋 브리핑 통합', a: '"브리핑" 하면 주의할 점 섹션 자동 표시 + 음성 첫 줄에 "실번호 주의 N건". 항목별로 컨 끝4 「실번호」 — 사유.' },
        { q: '🗣 "실번호 점검" / "씰 오류 확인"', a: '단독 질문으로 전체 목록 확인. 이상 없으면 "N건 모두 이상 없음".' },
        { q: '🔬 검증', a: '4종 + 정상 10건 합성 데이터 — 6건 전부 정확 검출, 정상 건 오탐 0. 자리수 판정은 같은 접두 그룹 표본 3건 이상 + 70% 최빈일 때만(과잉 경고 방지).' },
      ],
    },
    {
