    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
          <div className="bg-ink-900 border-2 border-red-700 rounded-card max-w-md w-full p-6">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-2xl">🚨</span>
              <h2 className="text-lg font-black text-red-300">{name} 크래시</h2>
            </div>
            <div className="bg-ink-950 border border-line rounded p-3 mb-3 max-h-48 overflow-y-auto">
              <div className="text-xxs text-red-400 font-bold uppercase mb-1">에러 메시지</div>
              <div className="text-xs mono text-red-200 break-all">{errMsg}</div>
              {this.state.error?.stack && (
                <details className="mt-2">
                  <summary className="text-2xs text-dim-300 cursor-pointer">스택 트레이스</summary>
                  <pre className="text-3xs mono text-dim-400 mt-1 whitespace-pre-wrap break-all">
                    {this.state.error.stack}
                  </pre>
                </details>
              )}
            </div>
            <div className="text-xs text-dim-200 mb-3">
              화면이 사라지지 않게 격리 처리했습니다. 닫고 다시 시도하거나, 에러 메시지를 개발자에게 전달해 주세요.
            </div>
            <div className="flex gap-2">
              {this.props.onClose && (
                <button onClick={() => { this.reset(); this.props.onClose(); }}
                  className="flex-1 py-2.5 bg-ink-750 hover:bg-ink-700 text-white font-bold rounded">
                  닫기
                </button>
              )}
