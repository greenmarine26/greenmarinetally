    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-1">
        <button onClick={() => setSection('main')}
          className={`flex-1 py-2 rounded text-xs font-bold ${section === 'main' ? 'bg-amber-700 text-amber-100' : 'bg-ink-800 text-dim-300'}`}>
          📋 검수 보고서
        </button>
        <button onClick={() => setSection('errors')}
          className={`flex-1 py-2 rounded text-xs font-bold flex items-center justify-center gap-1 ${
            section === 'errors'
              ...
              : sealErrors.length > 0 ? 'bg-red-900/40 text-red-300 border border-red-800' : 'bg-ink-800 text-dim-300'
          }`}>
        ...
        <>
          <div className="bg-ink-900 border border-line rounded-pill p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm font-bold text-dim-100">검수 진행 보고서</div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center mt-2">
              <div className="bg-ink-800/50 rounded p-2">
                <div className="text-2xs text-dim-400 font-bold">전체</div>
                <div className="text-2xl font-black mono text-dim-100">{containers.length}</div>
              </div>
              <div className="bg-emerald-900/30 rounded p-2">
                <div className="text-2xs text-emerald-400 font-bold">완료</div>
                <div className="text-2xl font-black mono text-emerald-300">{records.length}</div>
              </div>
              <div className="bg-amber-900/30 rounded p-2">
                <div className="text-2xs text-amber-400 font-bold">미완</div>
          ...
          <div className="flex gap-1">
            <button onClick={() => setGroupBy('time')}
              className={`flex-1 py-2 rounded text-xs font-bold ${groupBy === 'time' ? 'bg-amber-700 text-amber-100' : 'bg-ink-800 text-dim-300'}`}>
              시간순
            </button>
            <button onClick={() => setGroupBy('inspector')}
              className={`flex-1 py-2 rounded text-xs font-bold ${groupBy === 'inspector' ? 'bg-amber-700 text-amber-100' : 'bg-ink-800 text-dim-300'}`}>
              검수원별
            </button>
          </div>
...
function SealErrorReport({ errors, voyageInfo, voy, mode, onExport, onCopy }) {   // V9.14: voy 스코프 버그 수리 — 실오류 1건 이상이면 ReferenceError로 앱 전체 크래시였다
  if (errors.length === 0) {
    return (
      <div className="bg-emerald-950/30 border border-emerald-800 rounded-pill p-8 text-center">
        <CheckCircle2 className="w-10 h-10 text-emerald-500 mx-auto mb-2"/>
        <div className="text-emerald-300 font-bold">실오류 없음</div>
        <div className="text-xxs text-emerald-400/70 mt-1">모든 컨테이너 원본·실제 일치</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="bg-red-950/40 border-2 border-red-700 rounded-pill p-3">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <FileWarning className="w-5 h-5 text-red-400"/>
            <div>
              <div className="font-black text-red-200">🚨 세관 신고 대상</div>
              <div className="text-2xs text-red-300/70">항차: {voyageInfo?.vsl} {voy} · 모드: {mode === 'discharge' ? '양하' : '선적'}</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-3xl font-black mono text-red-300">{errors.length}</div>
            <div className="text-2xs text-red-400">건</div>
          </div>
        </div>
        ...
        <div className="text-xxs text-red-300/70 mb-2 font-bold">신고 양식 (원번호 → 실제번호)</div>
        <div className="space-y-1.5">
          {errors.map((e, i) => (
            <div key={`${e.cn}-${e.errType}`} className="bg-red-950/60 border border-red-800/60 rounded p-2 text-xxs">
              <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                <span className="text-red-400 font-black mono">{i + 1}.</span>
                <span className={`text-3xs font-black px-1.5 py-0.5 rounded ${
                ...
                <span className="text-dim-300 mono text-2xs">{e.cn}</span>
                {e.bay && <span className="text-amber-400 mono text-2xs">{e.bay}-{e.row}-{e.tier}</span>}
              </div>
              <div className="font-mono text-xs2 ml-4">
                <span className="text-dim-400">원번호:</span>
                <span className="text-dim-200 font-bold ml-1">{e.orig}</span>
                <span className="text-red-400 mx-2">→</span>
                <span className="text-dim-400">실제:</span>
                <span className="text-red-300 font-black ml-1">{e.actual}</span>
              </div>
              {e.by && (
                <div className="text-2xs text-dim-400 ml-4 mt-0.5">
  ...
  return (
    <div className="bg-ink-900 border border-line rounded-pill overflow-hidden">
      <button onClick={() => setExpanded(!expanded)}
        className="w-full px-3 py-2 bg-ink-800/50 flex items-center justify-between text-left">
        <div className="flex items-center gap-2">
          {groupBy === 'inspector'
            ...
            : <Clock className="w-4 h-4 text-dim-300"/>}
          <span className="font-bold text-sm text-dim-100">{title}</span>
          <span className="bg-emerald-900/50 text-emerald-300 text-2xs px-1.5 rounded font-black">{items.length}</span>
        </div>
        <span className="text-xs text-dim-400">{expanded ? '▾' : '▸'}</span>
      </button>
      {expanded && (
        <div className="p-2 space-y-1">
          {items.map(r => (
            <div key={r.cn} className="bg-ink-950/50 rounded px-2 py-1.5 flex items-center gap-2">
...
function UndoneSection({ containers, compMap }) {
  ...
  if (undone.length === 0) return (
    <div className="bg-emerald-950/30 border border-emerald-800 rounded-pill px-3 py-2 text-xs2 text-emerald-300 font-bold">
      ✅ 미완 0건 — 전량 완료
    </div>
  );
  return (
    <div className="bg-amber-950/30 border border-amber-800/60 rounded-pill overflow-hidden">
      <button onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-3 py-2.5 text-left" style={{ minHeight: 44 }}>
        <span className="text-sm2 font-bold text-amber-200">⏳ 미완 {undone.length}건 {open ? '접기' : '보기'}</span>
        <span className="text-amber-400 text-xs">{open ? '▲' : '▼'}</span>
      </button>
      {open && (
        <div className="px-3 pb-3 max-h-72 overflow-y-auto space-y-1">
          {undone.map(c => (
            <div key={c.cn} className="flex items-center justify-between text-xs2 bg-ink-900/70 rounded px-2 py-1.5">
              <span className="mono text-dim-100 font-bold">{c.cn}</span>
              <span className="mono text-dim-300">{[c.bay, c.row, c.tier].filter(Boolean).join('-') || '-'}</span>
            </div>
          ))}
        </div>
      )}
