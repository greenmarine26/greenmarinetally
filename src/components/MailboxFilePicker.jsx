...
export default function MailboxFilePicker({ vessel, voy, voyageKey, mode, onEdi, onList, onXray }) {
  ...
  const load = useCallback(async (h) => {
    ...
    setErr('');
    try {
      // TallyOne 1.11: mode 를 넘긴다 — `2606N(D)` / `2606N(L)` 방향별 폴더를 먼저 찾기 위해서.
  ...
  if (!root) {
    return (
      <div className="bg-ink-900 border border-amber-800/50 rounded-pill p-3">
        ...
        <div className="text-xxs text-dim-300 mb-2 leading-relaxed">
          한 번만 연결해 두면, 이 항차의 자료가 아래에 바로 뜹니다 — 파일 창에서 찾아 들어갈 필요가 없습니다.
          <br />권장: <b className="text-dim-100">MAILBOX</b> 폴더 (선박 폴더들이 들어 있는 그 폴더) —
    ...
    return (
      <div className="bg-ink-900 border border-amber-800/50 rounded-pill p-3">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={connect}
            className="bg-ink-800 border border-line text-dim-200 px-3 py-2 rounded text-xs">
            다른 폴더로 바꾸기
          </button>
        </div>
        {err && <div className="text-xxs text-red-400 mt-2">{err}</div>}
    ...
    <div className="bg-ink-900 border border-line rounded-pill p-3">
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        <button onClick={() => setOpen(!open)} className="text-sm font-bold flex items-center gap-2 text-amber-200">
          ...
          메일함 자료
          {res && res.ok && <span className="text-xxs text-dim-300 font-normal mono">{res.dirPath} · {files.length}개</span>}
          {res && res.ok && res.rootName && <span className="text-2xs text-dim-500 font-normal">📁{res.rootName}</span>}
        </button>
        <div className="flex-1" />
        <button onClick={() => load()} title="다시 읽기"
          className="p-1.5 rounded bg-ink-800 hover:bg-ink-750 text-dim-200">
          <RefreshCw className="w-3.5 h-3.5" />
        </button>
        <button onClick={connect} className="text-2xs text-dim-400 hover:text-dim-200 underline underline-offset-2">
          폴더 바꾸기
        </button>
      </div>

      {err && <div className="text-xxs text-red-400 mb-2">{err}</div>}

      {open && res && !res.ok && (
        <div className="text-xxs text-dim-300 leading-relaxed">
          <div>연결된 폴더: <b className="text-dim-100">{res.rootName || '?'}</b></div>
          ...
          {res.dirs && res.dirs.length > 0 && (
            <div className="mt-1 text-dim-400">있는 폴더: {res.dirs.slice(0, 24).join(' · ')}{res.dirs.length > 24 ? ' …' : ''}</div>
          )}
          <div className="mt-1 text-dim-400">
            [폴더 바꾸기]로 <b className="text-dim-200">MAILBOX</b>를 고르면 선박·항차를 알아서 찾아갑니다.
          </div>
        </div>
      )}

      {open && res && res.ok && files.length === 0 && (
        <div className="text-xxs text-dim-400">이 항차 폴더에 아직 자료가 없습니다.</div>
      )}
      ...
      {open && res && res.ok && files.length > 0 && (
        <div className="space-y-1 max-h-72 overflow-y-auto">
          {files.map((f) => {
            ...
            return (
              <div key={f.name}
                className={`flex items-center gap-2 px-2 py-1.5 rounded border ${anyDone ? 'bg-ink-800/30 border-line' : 'bg-ink-800/60 border-line'}`}>
                <div className="flex-1 min-w-0">
                  <div className={`text-xs2 truncate ${anyDone ? 'text-dim-300' : 'text-dim-100'}`}>
                    {anyDone && <span className="text-emerald-500 mr-1">✓</span>}{f.name}
                  </div>
                  <div className="text-2xs text-dim-400 mono">{HM(f.at)} · {KB(f.size)}</div>
                </div>
                  ...
                  {TARGETS.map(([t, label, strong, weak]) => {
                    ...
                    return (
                      <button key={t} onClick={() => send(f, t)} disabled={!!busy}
                        title={rec ? '추천' : ''}
                        className={`px-2 py-1 rounded text-2xs font-bold border transition disabled:opacity-40 ${
      ...
      {open && (
        <div className="text-2xs text-dim-500 mt-2 leading-tight">
