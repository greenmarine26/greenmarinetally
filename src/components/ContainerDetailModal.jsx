...
export default function ContainerDetailModal({ variant = 'modal', c, comp, isXray, xraySeal, mode, voyageKey, voyageInfo, inspector, onClose, sealMode, allContainers = [], workBay = null, workTier = null, onStartSwap = null, records = null, shiftCns = null }) {   // 3.2-01: records·shiftCns — 통과분 문지기 재료(리스트 등재·시프팅은 작업분)
  ...
  const xRegTouched = useRef(false);   // 3.9 재감사: 사람이 체크를 눌렀을 때만 봉인자를 건드린다(패널 변형은 재마운트가 없어 앞 컨 값이 남는다 — 편집 열 때 되돌린다)
          ...
          {mode === 'loading' && (
            <div className="mt-3 pt-3 border-t border-line">
              <div className="text-2xs text-dim-400 font-bold uppercase mb-2 flex items-center justify-between">
                {/* 2.94-06: **선적확인이 눌려야 실물이다**(검수사 확정 2026-08-31).
                    누르기 전에는 «실체»라 부르지 않는다 — 그 말이 앱과 사람 모두를 속였다. */}
                <span>{c._comp ? '실린 자리 (선적확인됨)' : '실을 자리 (아직 안 실림)'}</span>
                {!editingActualPos && (c.bay_actual || c.row_actual || c.tier_actual) ? (
                  ...
                  <button onClick={() => {
                    setActualBay(c.bay_actual || c.bay_assign || c.bay || '');
                    setActualRow(c.row_actual || c.row_assign || c.row || '');
                    setActualTier(c.tier_actual || c.tier_assign || c.tier || '');
              ...
              {!editingTmp ? (
                <div className="flex items-center gap-2">
                  {c.mkcon ? (
                    ...
                    <span className="text-sm font-bold text-red-300 animate-pulse">⚠️ 자료에 설정온도 없음 — 현장에서 확인해 입력하세요</span>
                  )}
            ...
            {editingXSeal ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setEditingXSeal(false); setXSealVal(xraySeal?.seal || ''); setXEsealVal(xraySeal?.eseal || ''); setXRegister(xRegisterInit); xRegTouched.current = false; }} className="px-3 py-2 bg-ink-750 text-dim-200 rounded">취소</button>
                </div>
              </div>
            ) : (
              <button onClick={() => { setEditingXSeal(true); setXSealVal(xraySeal?.seal || ''); setXEsealVal(xraySeal?.eseal || ''); setXRegister(xRegisterInit); xRegTouched.current = false; }} className="w-full text-left space-y-0.5">
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-base font-bold ${xraySealerOf(xraySeal, comp, voyageInfo) ? 'text-emerald-300' : 'text-dim-500 italic'}`}>
                    {xraySealerOf(xraySeal, comp, voyageInfo) || '미등록'}
                  </span>
                </div>
              </button>
            )}
