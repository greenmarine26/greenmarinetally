...
export default function PrintableBayDetail({
  ...
  const filteredPages = useMemo(() => {
    ...
    if (printMode === 'single' && selectedKey) {
      return allPages.filter(p => p.key === selectedKey);
    }
    return [];
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          {printMode === 'single' && (
            <select value={selectedKey || ''} onChange={(e) => setSelectedKey(e.target.value)}
              className="px-2 py-1 rounded text-xs bg-slate-800 text-slate-100 border border-slate-700">
              <option value="">— 베이 선택 —</option>
              {allPages.map(p => {
                const dispBay = (n) => n >= 100 ? String(n) : String(n).padStart(2, '0');
                let label;
                ...
                return <option key={p.key} value={p.key}>{label}</option>;
              })}
            </select>
          )}
        </div>
