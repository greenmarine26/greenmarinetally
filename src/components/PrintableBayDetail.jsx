      ...
      <div className="bd-cell-lines">
        ...
        {_oogDir(c) && (
          <svg className="bd-oog" viewBox="0 0 3 2" preserveAspectRatio="none" aria-hidden="true">
            {/* ── 2.98-06: 카스피 FR 표기법 — 도형이 곧 뜻이다 (검수사 확정 2026-08-31) ──
                · 좌·우 삼각형   = 그쪽으로 폭이 나갔다. 꼭짓점은 **옆 칸 중앙**까지.
                    종전 0/3 은 옆 칸 끝까지 뻗어 FR 두 대가 나란히 서면 사이에서 **X 자로 교차**했다.
                    원본은 `> <` 로 마주보되 닿지 않는다.
                · 위 삼각형 + 그것을 **가로지르는 직사각형** = 높이가 있으니 **연장 스프레더**(다리 긴 것)를 써라.
                    검수사 원문 — *"위쪽 삼각형을 가로질러 스프레더 같이 생긴 직사각형이 있습니다.
                    의미는 높이가 있으니 연장스프레더를 사용하라는것입니다. 다리가 긴"*
                    *"직사각형이 옆으로가 아니고 90도 회전시켜서 한쪽끝이 격자위부분에 걸쳐야 합니다"*
                    *"위로가 아니고 아래 격자경계선에"*
                    → **세로**로 세우고 **아래 끝(y=1)을 셀 윗변(격자 경계선)에** 붙인다.
                      거기서 위로 올라가며 삼각형 꼭짓점을 가로지른다. */}
            {_oogDir(c).includes('W') && (<>
              <polygon points="1,1 0.5,1.5 1,2" />
              <polygon points="2,1 2.5,1.5 2,2" />
            </>)}
            {_oogDir(c).includes('H') && (<>
              <polygon points="1,1 1.5,0.3 2,1" />
              <rect x="1.26" y="0.15" width="0.48" height="0.85" />
            </>)}
