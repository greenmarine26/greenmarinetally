...
export async function ocrReeferTemps(file, geminiApiKey = '') {
  ...
  //  ★ 3.32 — **양식을 못 박지 않는다.** 배마다 표가 다르다(검수사 실물 2026-09-08 SWBT 2614N).
  //    3.25 까지의 프롬프트는 SITC 양식 하나를 «표 구조는 보통 이렇습니다» 로 못 박았다 —
  //      NO. | CONTAINER NUMBER | SLOT | PLUG-IN DATE | POL | POD | SET TEMP | 시각격자
  //    그런데 SAWASDEE BALTIC(Sinokor) 실물은 열 순서가 다르고 **없던 열이 하나 더** 있다 —
  //      NO. | LOCATION | CONTAINER NO. | SETT. TEMP. | PLUG IN TEMP. | POL | POD | REMARK | 시각격자
  //    ① 설정온도가 POD **앞**에 있고 ② `PLUG IN TEMP` 칸에 손글씨 숫자가 들어 있는데
  //    프롬프트는 «손글씨가 인쇄값 위에 덧쓰여 있으면 손글씨를 우선» 이라고만 해서
  //    그 칸을 실측으로 집을 여지가 그대로 있었다. ③ REMARK 칸에도 손으로 적은 **자리 번호**
  //    (180184 = 18-01-84)가 있어 온도로 오독될 수 있다.
  //    ⇒ 열 순서를 가정하지 말고 **열 제목을 읽어** 칸을 정하게 하고, 실측이 아닌 칸을 이름으로 막는다.
  const prompt = `이 이미지는 리퍼(냉동·냉장) 컨테이너 온도 점검 기록표입니다.
배마다 양식이 다릅니다 — **열 순서를 가정하지 말고 열 제목을 읽어** 어느 칸이 무엇인지 정하십시오.

칸의 뜻
- 컨테이너 번호 — CONTAINER NO. / CNTR NO. / CONTAINER NUMBER / 컨테이너번호
- set(설정온도) — SET TEMP / SETT. TEMP. / SETTING / 설정온도 / 지시온도 열. 행마다 하나뿐이고 보통 인쇄돼 있습니다.
  "RF-18.0C" "RF 14.0C" 처럼 앞뒤에 글자가 붙어 있어도 **숫자만** 읽습니다.
- act(실제온도) — 날짜·시각으로 나뉜 **격자**에 사람이 손으로 적은 관측값입니다.
  격자에서 **가장 마지막에 값이 적힌 칸**(오른쪽 끝 · 여러 날짜면 마지막 날짜의 마지막 값)을 씁니다.
  중간값·평균을 쓰지 않습니다.

⛔ **PLUG IN TEMP · PLUG-IN · 꽂을 때 온도 열은 실제온도가 아닙니다.** 그 칸에 손글씨가 있어도 act 로 쓰지 마십시오.
⛔ REMARK · 비고 · LOCATION · SLOT 열의 숫자(자리 번호 «180184» 같은 여섯 자리)는 온도가 아닙니다. 버리십시오.
⛔ 격자가 통째로 비어 있으면 act 는 빈 문자열입니다. **set 을 복사해 넣지 마십시오.**

다음 JSON 형식으로만 응답하세요. 다른 설명 없이 JSON만:
{"items":[{"cn":"컨테이너번호","set":"설정온도","act":"실제온도"}]}

규칙
- cn 은 영문 4자 + 숫자 7자. 표에 "HALU 8503321" 처럼 **띄어 적혀 있어도 붙여서** 읽습니다.
  그 형식이 아니면 그 행은 버립니다.
- 온도는 숫자만 남깁니다. 영하는 반드시 음수로 (-18, -18.0, -2.5, -0.5).
  "18-" "18 MINUS" "△18" 처럼 적혀 있어도 -18 로 읽습니다. ℃ · C · 도 같은 단위 글자는 뺍니다.
  0 은 0 그대로 둡니다(빈칸과 다릅니다).
- 손글씨의 앞자리가 칸 경계를 넘어 적혀 "14.0" 이 "4.0" 처럼 보이는 줄이 있습니다.
  같은 열의 위아래 값과 set 값을 견주어 자연스러운 쪽으로 읽으십시오.
- 인쇄된 설정온도 위에 손으로 고쳐 적었으면 **손글씨를 씁니다** — 다만 이것은 설정온도 칸 안에서만 하는 말입니다.
