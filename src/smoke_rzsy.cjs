// 신규 취항선 SMYA(SMC YANTAI, 구 RZSY — 2026-09-02 터미널 배정목록 선박코드로 정정) 등재 검사 — .def 사전 조회 + 기존 배 회귀
...
(async () => {
  ...
  console.log('[1] SMYA(구 RZSY) 19베이가 전부 조회되는가');
  const BAYS = ['01','02','03','05','06','07','09','10','11','13','14','15','17','18','19','21','22','23','25'];
  let n = 0;
  for (const b of BAYS) {
    const e = dict.getDefBayEntry('SMYA', b);
  ...
  ok(dict.getDefBayEntry('SMYA','04') === null, '없는 베이 04 는 null (있는 척하지 않는다)');
  ok(dict.getDefBayEntry('SMYA','20') === null, '없는 베이 20 은 null');

  console.log('[2] getBayOverride 경로로도 같은 답이 나오는가');
  const g = ov.getBayOverride('SMYA','15');
