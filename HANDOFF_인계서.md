...
## V7.96-02 — 베이상세 편집 렌더 버그 수정 (찌부러짐)

**증상**: V7.96 베이 편집 모달에서 BayBoxV2가 높이 0으로 찌부러져 셀이 한 줄로 뭉개짐(컴/폰 모두). 작업 불가.
**원인**: BayBoxV2는 `flex:1 1 0` 기반이라 **높이가 정해진 부모** 안에서만 펼쳐짐(카고플랜은 `.cpv2-page` 195mm 고정 높이가 줌). 모달 `.bde-box`가 높이를 안 줘서 flex 체인이 0으로 무너짐.
**수정**:
- `.bde-box`에 `height:min(72vh,620px); display:flex; flex-direction:column` 부여 → bay-section flex:1이 펼쳐짐.
- trio 래퍼 div에 `flex:1; min-height:0` → 두 BayBoxV2가 높이 분배.
- 편집 모드 셀/라벨 폰트 키움(드래그·식별).
- 모바일(≤680px) 미디어쿼리: 임시창고를 하단 띠로(가로 칩), 베이박스 상단 채움, 넓은 베이는 스테이지 가로 스크롤.

**검증(이번엔 실제 PNG 렌더로 확인 — 지침서 절대원칙 준수)**:
- 크롬 헤드리스 렌더 → 단독 베이/페어 trio/폰(390px) 3종 모두 PNG로 눈 확인. 셀 마크·X-RAY(★)·평택색·통과(회색)·선택칸(파란테두리)·임시창고 정상.
- **이전 V7.96 실패 원인 = 마크업 검사(draggable 속성 유무)만 하고 시각 렌더를 PNG로 안 봤음.** 시각 렌더는 반드시 PNG 확인 후 ZIP — 재확인.
