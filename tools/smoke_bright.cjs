...
if (bundle && fs.existsSync(bundle)) {
  ...
  //  3.53-03(2.41-01 패치): 「화면」이라는 낱말만으로는 조작이 아니다 — 매뉴얼 질문은 흘려보내고, 의도어(활용형)는 받는다.
  NO.push('베이 화면 어디야', '홈 화면으로 어떻게 가', '수석 화면 볼 수 있어?');
  YES.push('눈이 아픈데', '화면 때문에 눈이 아파', '화면이 침침해');
  YES.push('화면이 눈부셔', '눈이 시려');   // 감사 지적 ①② — 활용형
