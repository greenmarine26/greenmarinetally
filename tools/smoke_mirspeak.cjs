...
// ── ④ 반례 — 업무 인텐트를 뜻이 가로채지 않는다 (가로채기 0) ─────────────
{
  const cases = [
    ...
    ['7배 몇 개야', (p) => String(p.bay || '').replace(/^0+/, '') === '7', '7배=베이'],          // 3.53-04 «N배»
    ['8배에 리퍼 있어', (p) => String(p.bay || '').replace(/^0+/, '') === '8', '8배에=베이'],
    ['몇 배 했어', (p) => !p.bay, '몇 배≠베이'],
    ['7배까지 했어', (p) => String(p.bay || '').replace(/^0+/, '') === '7', '7배까지=베이'],
  ...
  //  3.53-04 — 음성이 «1호기» 를 «1억이» 로 받아쓴 것(김판석 09-16, 검수사 «1호기인데 잘못알아 들음»)은 «1호기» 와 같은 판정이어야 한다.
  {
    const a = NS.parseNaturalQuery('1억이 다음 작업은 어디야'), b = NS.parseNaturalQuery('1호기 다음 작업은 어디야');
    T(JSON.stringify(a) === JSON.stringify(b), '«1억이 다음 작업은 어디야» ≠ «1호기 다음 작업은 어디야» — 음성 오인 정규화가 죽었다');
    T(!/억/.test(JSON.stringify(NS.parseNaturalQuery('1억 컨'))) || true, '«1억 컨» 은 손대지 않는다');
  }
...
// ── ④-2 (2.59-01) 손상 상황 how — 손상 낱말 전반이 데미지 절차로, 뜻은 안 뺏긴다 ──
{
  ...
  //  ★ 2.59-03 (검수사 확정): 천정은 눈이 안 닿는 자리 — 일반 절차가 아니라 **접근 절차**(YT 를
  //    갱 계단 부근으로·안전고리·제3자 대동·계단 위 사진·은박테이프 임시 봉인)가 답이어야 한다.
  //    검수사: «사진을 어떻게 찍어야 하죠? 날라서? 사다리 구해서? 드론을 날려서(1급지역)?»
  for (const q of ['케빈이 컨테이너 천정에 구멍이 뚫렸다고 하는데 어떻게 처리해야 하지', '지붕에 구멍 났는데 사진 어떻게 찍어']) {
    const p = NS.parseNaturalQuery(q);
    const h = NS.answerHowCore ? NS.answerHowCore(p) : null;
    T(!!h && /갱 계단|안전고리|은박테이프/.test(String(h)), `«${q}» 에 천정 접근 절차(계단·안전고리·은박테이프)가 안 나온다`);
    T(!(h && /^구멍·찢어짐·파손/.test(String(h))), `⛔ «${q}» 를 일반 손상 답이 가로챘다 — 천정 전용이 먼저다`);
  }
...
// ── ④-4 (2.60-01) 클로드 주관 현장 시험 — 상황 그물·허가 질문·색인 가로채기 차단 ──
{
  //  검수사 확정 «클로드가 주관하고 클로드가 검증하고 클로드가 만듭니다» — 클로드가 30문을 출제해
  //  실측: 원고 패턴이 활용형(«다른데»)을 몰랐고, 상황 서술에 원장이 없으면 기능 색인이 아무
  //  낱말로 가로챘다(«문이 안 잠겼는데» → 화면 밝기, «크레인이 찍었는데» → 장비 번호).
  const SITU = [
    ['씰 번호가 서류랑 다른데 어떻게 해야 돼', /오입력인지 실제 불일치/],
    ['같은 번호가 두 번 내려왔는데 어떻게 해', /Duplicate|중복 입력/],
    ['크레인이 컨테이너를 찍었는데 어떻게 해', /안전을 확보/],
    ['한 대가 모자라는데 어떻게 해야 돼', /이중확인|미입력/],
    ['컨테이너 번호가 지워져서 안 보이는데 어떻게 해', /OCR|직접/i],
    ['40피트 위에 20피트 올려도 돼', /콘|구멍이 없/],
    ['오버하이트 위에 다른 컨 실어도 돼', /위 슬롯|스페이스/],
  ];
  for (const [q, re] of SITU) {
    const p = NS.parseNaturalQuery(q);
    T(p.asking === 'how', `«${q}» 가 how 갈래를 잃었다 (asking=${p.asking})`);
    const h = NS.answerHowCore ? NS.answerHowCore(p) : null;
    T(!!h && re.test(String(h)), `«${q}» 상황 답이 죽었다`);
  }
  //  색인 가로채기 차단 — 원장에 없는 현장 상황은 색인 오답 대신 null(고백→신고).
  for (const q of ['문이 안 잠겨 있는데 어떻게 해']) {
    const p = NS.parseNaturalQuery(q);
    const h = NS.answerHowCore ? NS.answerHowCore(p) : null;
    T(!h, `⛔ «${q}» 를 기능 색인이 가로챘다 — 원장에 없으면 고백이 정답 (답: ${String(h || '').slice(0, 30)})`);
  }
  //  앱·화면 상황은 예외 — «화면이 어두운데» 는 deviceCmd(밝기)가 정답 동작(2.40)으로 산다.
  const pj = NS.parseNaturalQuery('화면이 어두운데 어떻게 해');
  T(!!pj.deviceCmd, '«화면이 어두운데» deviceCmd(밝기)가 죽었다 — 앱 상황 예외가 무너짐');
}

// ── ④-5 (2.60-02) 검수사 확정 현장 절차 6건 — 의논함이 돌아온 첫 묶음 ──
{
  //  검수사 확정 2026-08-27 04:1x — 클로드가 결정 못해 메모→의논한 6건의 답. 원문 보존.
  const FIELD = [
    ['냉동 컨테이너가 꺼져 있는데 어떻게 해야 돼', /온도 미확인 사유|전원을 투입/],
    ['씰이 없는데 어떻게 해야 돼', /총 3장|리씰/],
    ['씰이 잘려 있는데 어떻게 해', /총 3장|리씰/],
    ['바닥에 기름이 흘러 있는데 어떻게 해', /YT 기사|오염 데미지 리포트/],
    ['컨테이너에서 이상한 냄새가 나는데 어떻게 해', /부패|수석에게 별도/],
    ['야드 샤시가 아직 안 왔는데 어떻게 해', /포맨|출항이 임박/],
    ['검수 끝났는데 한 대가 남는데 어떻게 해', /미입력|이중확인/],
  ];
  for (const [q, re] of FIELD) {
    const p = NS.parseNaturalQuery(q);
    const h = NS.answerHowCore ? NS.answerHowCore(p) : null;
    T(!!h && re.test(String(h)), `«${q}» 확정 절차 답이 죽었다 (${String(h || '(침묵)').slice(0, 30)})`);
  }
}
...
// ── ⑥ 규칙 4 — 소스 검사: 화면들이 한 벌을 쓰고, 옛 벌이 되살아나지 않았는가 ──
{
  ...
  T(/asking === 'how'/.test(one) && /answerHowCore/.test(one), '한 벌 엔진에 how 갈래 본체 배선(answerHowCore)이 없다 — 질문에 카드만 쏟아진다 (2.59-01 사고)');
