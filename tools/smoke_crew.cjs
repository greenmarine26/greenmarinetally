...
//  ── ⑦ 배선 — 소스 문자열 ──
{
  ...
  T(/crewAnswer: \(cq\) =>/.test(rd('src/pages/VoyagePage.jsx')) && /crewAnswer: briefCtx\?\.crewAnswer/.test(rd('src/pages/VoyagePage.jsx')) && /c\.crewAnswer = \(cq\) =>/.test(rd('src/mir.js')), '조회 답 클로저(crewAnswer)가 화면에 안 실린다');
