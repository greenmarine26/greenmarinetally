...
// ★ 3.53-12 — 종전 2.70 은 «외부 합계와 앱 기록의 차이를 대수로 깎는다»(twGap)였다.
...
T(/gang: c\.gangBrief \? c\.gangBrief\(\) : null/.test(read('src/mir.js')) && /c\.gangShift = \(n\) =>/.test(read('src/mir.js')), '한 벌 엔진(mirAnswer) 갱 배선이 없다 — 브리핑 갱 줄·갱 배분 클로저');
...
T(/answerGangShift\(v, de/.test(read('src/mir.js')) && /p\.gangQuery && c\.gangShift/.test(read('src/mir.js')), '한 벌 엔진(mirAnswer) 갱 분기가 없다');
