...
//   ⚠ 콘앱 ctx 는 cone.html 의 mirAsk 를 그대로 베낀다 — 콘앱이 재료를 하나라도 빼먹으면 여기서 갈린다(2.49 실측: 재료 하나가 늘 null).
const fs = require('fs');
...
const vk = FX.voyageKey, v = FX.voyage, info = v.info, ss = FX.shipSpeed, pf = FX.pilotForecast;
let n = 0, bad = 0;
...
const tallyCtx = () => ({ app: 'tally', smallTalkLast: true, execDevice: true, modeChoice: 'both', countFallback: true, inspector: '연막', isChief: true, chiefData: null, heartbeat: null, portMisData: {}, pilotForecast: pf, shipSpeed: ss, voyages: { [vk]: v }, flat: cs,
...
//  콘앱 — cone.html mirAsk 그대로(shipSpeed·pilotForecast)
...
const coneCtx = (extra) => Object.assign({ app: 'cone', containers: cs, cone: { rows: [], dischRows: [], stowRows: [] }, execDevice: true, shiftN: 0, mode: 'discharge', modeLabel: '양하', info, compMap: comp, shiftMap: null, voyage: coneVoyage, vsl: info.vsl || '', vslFull: info.vslFull || '', pier: info.pier || '',
  shipSpeed: ss, pilotForecast: pf, voyageKey: vk, records: { discharge: D.records || {}, loading: L.records || {} }, _trace: {} }, extra || {});
...
//  ② 내용 — 3.53-12: 두 앱 다 **완료 기록 한 숫자**로 답한다. 외부 합계는 재료에 없다
{
...
check('cone.html 에 터미널 합계 자료를 읽는 자리가 없다(3.53-13 · 2.50-02)', !new RegExp('terminal' + '_work|terminal' + 'Work').test(H));
...
(async () => {
  ...
  const stateStub = { voyageKey: vk, pilotForecast: null };
  let mc = null, err = '';
  try {
  ...
  if (mc) {
    ...
    const ctx2 = { app: 'cone', containers: cs, cone: { rows: [], dischRows: [], stowRows: [] }, execDevice: true, shiftN: 0, mode: 'discharge', modeLabel: '양하', info: mc.info, compMap: comp2, shiftMap: null, voyage: voyage2, vsl: mc.info.vsl || '', vslFull: mc.info.vslFull || '', pier: mc.info.pier || '', shipSpeed: (mc && mc.shipSpeed) || null, pilotForecast: (mc && mc.pilotForecast) || null, voyageKey: vk, records: { discharge: mc.recD || {}, loading: mc.recL || {} }, _trace: {} };
